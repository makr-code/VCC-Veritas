    ...
    for i, row in enumerate(rows):
        ...
        try:
            ...
            if not validator.is_valid_brandenburg(lat, lon):
                ...
                if i < 10:
                    print(
                        f"{bimschg_id:<20} {ort:<20} {ostwert:>12.1f} {nordwert:>12.1f} {lat:>12.6f} {lon:>12.6f} {'⚠️ WGS84':<10}"
                    )
                continue
...
def validate_wka():
    ...
    print("-" * 120)
    print(
        f"{'Anlagen-ID':<15} {'Ort':<20} {'UTM Ost':>12} {'UTM Nord':>12} {'WGS84 Lat':>12} {'WGS84 Lon':>12} {'Status':<10}"
    )
    print("-" * 120)
    ...
    for i, row in enumerate(rows):
        ...
        try:
            ...
            if not validator.is_valid_brandenburg(lat, lon):
                ...
                if i < 10:
                    print(
                        f"{wka_id:<15} {ort:<20} {ostwert:>12.1f} {nordwert:>12.1f} {lat:>12.6f} {lon:>12.6f} {'⚠️ WGS84':<10}"
                    )
                continue
...
if __name__ == "__main__":
    ...
    try:
        ...
        print(f"{'─'*40}")
        print(
            f"GESAMT:   {bimschg_valid+wka_valid:>6} / {bimschg_total+wka_total:>6} gültig ({(bimschg_valid+wka_valid)/(bimschg_total+wka_total)*100:>5.1f}%)"
        )
