...
def calculate_recall_at_k(results: List[Dict], relevant_doc_ids: List[str], k: int = 10) -> float:
    ...
    retrieved_relevant = sum(1 for result in results[:k] if result.get("doc_id", "") in relevant_doc_ids)
...
class EvaluationRunner:
    ...
    async def evaluate_query(self, query: GroundTruthQuery, k: int = 10) -> Dict[str, Any]:
        ...
        results_dicts = [{"doc_id": r.doc_id, "score": r.score, "content": r.content} for r in results]
    ...
    def _aggregate_metrics(self, all_metrics: List[Dict]) -> Dict[str, Any]:
        ...
        aggregated = {
            ...
            "latency_p95": sorted(m["latency_ms"] for m in valid_metrics)[int(len(valid_metrics) * 0.95)]
            if len(valid_metrics) > 20
            else max(m["latency_ms"] for m in valid_metrics),
        }
...
def compare_results(baseline: Dict, hybrid: Dict):
    ...
    all_categories = set(list(baseline.get("by_category", {}).keys()) + list(hybrid.get("by_category", {}).keys()))
...
async def run_evaluation():
    ...
    hybrid = HybridRetriever(dense_retriever=uds3_adapter, sparse_retriever=hybrid_bm25, config=None)
    ...
    filtered_queries = [q for q in GROUND_TRUTH_DATASET if q.expected_top1 in available_doc_ids]
