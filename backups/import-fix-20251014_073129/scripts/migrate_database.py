...
class DatabaseMigrator:
    ...
    def get_applied_migrations(self) -> List[Migration]:
        ...
        self.cursor.execute("SELECT version, name, checksum, applied_at FROM schema_migrations ORDER BY version")

        migrations = []
        for row in self.cursor.fetchall():
            migrations.append(Migration(version=row[0], name=row[1], checksum=row[2], applied_at=row[3]))
    ...
    def get_available_migrations(self) -> List[Migration]:
        ...
        for filepath in sorted(self.migrations_dir.glob("*.sql")):
            ...
            migrations.append(Migration(version=version, name=name, filename=str(filepath), checksum=checksum))
...
"""
    ...
    def migrate(self, dry_run: bool = False) -> int:
        ...
        for migration in pending:
            ...
            try:
                # Execute migration
                self.cursor.executescript(up_sql) if self.db_type == "sqlite" else self.cursor.execute(up_sql)

                # Record migration
                self.cursor.execute(
                    "INSERT INTO schema_migrations (version, name, checksum) VALUES (?, ?, ?)"
                    if self.db_type == "sqlite"
                    else "INSERT INTO schema_migrations (version, name, checksum) VALUES (%s, %s, %s)",
    ...
    def rollback(self, steps: int = 1) -> int:
        ...
        for migration in reversed(to_rollback):
            ...
            try:
                # Execute rollback
                self.cursor.executescript(down_sql) if self.db_type == "sqlite" else self.cursor.execute(down_sql)
...
def main():
    ...
    parser.add_argument("command", choices=["create", "migrate", "rollback", "status"], help="Migration command")
    ...
    parser.add_argument("--db-type", default=os.getenv("DB_TYPE", "sqlite"), help="Database type (sqlite, postgresql, mysql)")
