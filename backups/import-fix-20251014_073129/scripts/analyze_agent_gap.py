...
class AgentAnalyzer:
        ...
        for agent in self.agents:
            for tool in agent.tools_used:
                if tool not in self.tools:
                    self.tools[tool] = ToolInfo(name=tool, type=tool_types.get(tool, "unknown"))
    ...
    def _generate_summary(self) -> Dict[str, Any]:
        ...
        # Tool statistics
        most_used_tools = sorted(self.tools.values(), key=lambda t: t.usage_count, reverse=True)[:5]
        ...
        return {
            ...
            "most_used_tools": [{"name": t.name, "usage_count": t.usage_count, "type": t.type} for t in most_used_tools],
            "code_statistics": {
    ...
    def _generate_recommendations(self) -> List[str]:
        ...
        if critical_agents:
            recs.append(
                f"⚠️ CRITICAL: Migrate {len(critical_agents)} critical agents first: {', '.join(a.name for a in critical_agents[:3])}"
            )
...
class ReportGenerator:
    ...
    def _generate_markdown(self, output_path: Path):
        ...
        md.append(
            f"- **Test Coverage:** {s['test_coverage']['percentage']}% ({s['test_coverage']['agents_with_tests']}/{s['total_agents']} agents)"
        )
        md.append("")
        ...
        for priority, count in sorted(
            s["priority_counts"].items(), key=lambda x: {"CRITICAL": 0, "HIGH": 1, "MEDIUM": 2, "LOW": 3}.get(x[0], 99)
        ):
