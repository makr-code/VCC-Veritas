        ...
        if response.status_code == 200:
            ...
            if collections:
                for coll in collections:
                    ...
                    try:
                        detail_response = requests.get(f"{base_url}/api/v1/collections/{coll_id}", timeout=5)
                        if detail_response.status_code == 200:
                            details = detail_response.json()
                            # Try to count documents
                            count_response = requests.post(f"{base_url}/api/v1/collections/{coll_id}/count", timeout=5)
                            ...
                            try:
                                get_response = requests.post(
                                    f"{base_url}/api/v1/collections/{coll_id}/get", json={"limit": 3}, timeout=5
                                )
            ...
            for coll in collections:
                ...
                try:
                    ...
                    query_response = requests.post(
                        ...
                        json={"query_texts": ["BGB Taschengeld § 110"], "n_results": 5},
                        timeout=10,
                    )
