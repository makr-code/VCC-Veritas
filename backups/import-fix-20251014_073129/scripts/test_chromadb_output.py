...
def main():
    ...
    if isinstance(results, dict):
        for key in results:
            print(
                f"  - {key}: {type(results[key])}, length={len(results[key]) if hasattr(results[key], '__len__') else 'N/A'}"
            )
    ...
    if isinstance(results2, dict):
        for key in results2:
            print(
                f"  - {key}: {type(results2[key])}, length={len(results2[key]) if hasattr(results2[key], '__len__') else 'N/A'}"
            )
