...
class QueryExpander:
    ...
    def __init__(self, config: Optional[QueryExpansionConfig] = None):
        ...
        if not self._ollama_available:
            logger.warning(
                "⚠️ Query Expansion läuft ohne Ollama (httpx fehlt) - "
    ...
    async def expand(
        ...
        logger.info(
            f"🔍 Query Expansion: '{query[:50]}...' → {len(expanded_queries)} Varianten ({duration:.0f}ms)"
        )
...
class MultiQueryGenerator:
    ...
    async def generate_multi_perspective(
        ...
        for perspective in perspectives:
            ...
            prompt = (
                f"Du bist Experte für Baurecht und technische Normen. "
