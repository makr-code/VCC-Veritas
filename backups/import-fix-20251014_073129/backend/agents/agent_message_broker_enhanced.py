...
class MessageWorker:
    ...
    async def _process_single_message(self):
        ...
        try:
            priority, msg_id, message = await asyncio.wait_for(self.broker._message_queue.get(), timeout=1.0)
    ...
    async def _collect_batch(self) -> List:
        ...
        while len(batch) < self.broker.config.batch_size:
            ...
            try:
                ...
                item = await asyncio.wait_for(self.broker._message_queue.get(), timeout=remaining if remaining > 0 else 0.1)
                batch.append(item)
            ...
            try:
                ...
                for i, worker in enumerate(self.workers):
                    if not worker.is_healthy:
                        logger.warning(f"⚠️ Worker-{worker.worker_id} unhealthy " f"(Errors: {worker.errors}), restarting...")
