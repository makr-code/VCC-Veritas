...
from framework.orchestration_controller import (
...
async def test_manual_interventions():
    ...
    # Test skip step
    print(f"\n[Skip Step]")
    intervention_id = await controller.skip_step(
        plan["plan_id"],
        "step_2",
        user="test_user"
    )
    ...
    print(f"  ✓ Step 2 marked as skipped")
    
    # Test add step
    print(f"\n[Add Step]")
    new_step = {"step_id": "step_4", "action": "validate"}
    intervention_id = await controller.add_step(
        plan["plan_id"],
        new_step,
        insert_after="step_1",
        user="test_user"
    ...
    # Test remove step
    print(f"\n[Remove Step]")
    intervention_id = await controller.remove_step(
        plan["plan_id"],
        "step_3",
        user="test_user"
    )
...
async def test_checkpoints():
    ...
    restored = await controller.restore_from_checkpoint(
        plan["plan_id"],
...
async def test_cancellation():
    ...
    print(f"  ✓ Cancellation processed correctly")
