...
class IntelligentMultiAgentPipeline:
        ...
        try:
            ...
            try:
                self.rag_service = RAGContextService(
                    database_api=None, uds3_strategy=self.uds3_strategy  # Wird von UDS3 verwaltet
                )
    ...
    def _run_agent_task_sync(
        ...
        mock_result["priority_score"] = round(task.priority_score, 2)
        mock_result["execution_stage"] = task.stage

        duration = time.time() - start_time

        trace = {
            "agent": task.agent_type,
            "status": mock_result.get("status", "completed"),
            "stage": task.stage,
            "priority": mock_result["priority_score"],
        ...
        return {"task": task, "result": mock_result, "trace": trace, "duration": duration, "status": trace["status"]}
    ...
    def _generate_mock_agent_result(self, agent_type: str, query: str) -> Dict[str, Any]:
        """Generiert Mock-Ergebnis für Agent (für Testing)"""
    ...
    def _extract_sources_from_results(
        ...
        for doc in rag_context.get("documents", []):
            sources.append(
                {"title": doc.get("title", "Unbekanntes Dokument"), "type": "document", "relevance": doc.get("relevance", 0.7)}
            )
