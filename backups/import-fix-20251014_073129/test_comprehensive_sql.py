...
async def test_comprehensive_sql():
    ...
    try:
        ...
        cursor.executemany(
            "INSERT INTO users (id, username, email, status, department, salary) VALUES (?, ?, ?, ?, ?, ?)", users_data
        )
        ...
        cursor.executemany("INSERT INTO documents (id, user_id, title, category, file_size) VALUES (?, ?, ?, ?, ?)", docs_data)
        ...
        cursor.executemany("INSERT INTO comments (id, document_id, user_id, content) VALUES (?, ?, ?, ?)", comments_data)
        ...
        for test in test_cases:
            ...
            request = DatabaseQueryRequest(
                query_id=f"test_{passed + failed + 1}", sql_query=test["sql"], database_path=db_path
            )
