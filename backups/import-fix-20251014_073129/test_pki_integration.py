...
from backend.services.pki_client import PKIConnectionError, VeritasPKIClient, get_pki_client

# Configure logging
logging.basicConfig(level=logging.WARNING, format="%(levelname)s: %(message)s")  # Suppress debug logs
...
def test_pki_connection() -> bool:
    ...
    try:
        client = VeritasPKIClient(base_url="https://localhost:8443", verify_ssl=False)  # Development mode
    ...
    subject = {"CN": "test.veritas.local", "O": "VCC", "OU": "VERITAS Integration Test"}

    try:
        result = client.request_certificate(subject=subject, valid_days=30, cert_type="client")
    ...
    try:
        verify_result = client.verify_certificate(
            cert_pem=test_cert, check_revocation=False  # Don't check revocation for new cert
        )
...
def test_connection_failure():
    ...
    try:
        ...
        client = VeritasPKIClient(base_url="https://localhost:9999", verify_ssl=False)  # Wrong port!
