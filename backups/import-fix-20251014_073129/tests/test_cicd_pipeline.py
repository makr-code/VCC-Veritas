    ...
    def _validate_ci_workflow(self) -> bool:
        ...
        try:
            ...
            result = WorkflowValidationResult(
                workflow_name="CI Workflow", valid=len(errors) == 0, errors=errors, warnings=warnings, info=info
            )
    ...
    def _validate_cd_workflow(self) -> bool:
        ...
        try:
            ...
            if "jobs" not in workflow:
                ...
                jobs = workflow["jobs"]
                expected_jobs = ["setup", "build", "deploy-staging", "deploy-production", "post-deployment"]
            ...
            result = WorkflowValidationResult(
                workflow_name="CD Workflow", valid=len(errors) == 0, errors=errors, warnings=warnings, info=info
            )
            self.results.append(result)
    ...
    def _validate_release_workflow(self) -> bool:
        ...
        try:
            ...
            if "jobs" not in workflow:
                ...
                jobs = workflow["jobs"]
                expected_jobs = ["create-release", "build-artifacts", "publish-pypi", "update-docs", "notify"]
            ...
            result = WorkflowValidationResult(
                workflow_name="Release Workflow", valid=len(errors) == 0, errors=errors, warnings=warnings, info=info
            )
            self.results.append(result)
