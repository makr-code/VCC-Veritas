    ...
    async def _handle_analysis(self, message: AgentMessage) -> Dict[str, Any]:
        ...
        return {
            "result": {
                "total_cost": 1500000,
                "currency": "EUR",
                "breakdown": {"land": 500000, "construction": 800000, "permits": 100000},
            },
            "confidence": 0.92,
            "agent": "financial",
            "processing_time_ms": 50,
...
class Phase4IntegrationTests:
    ...
    async def test_request_response(self):
        ...
        self.test_results.append({"test": "Request/Response", "success": success, "latency_ms": elapsed_ms})
    ...
    async def test_publish_subscribe(self):
        ...
        self.test_results.append({"test": "Publish/Subscribe", "success": success, "latency_ms": elapsed_ms})
    ...
    async def test_broadcast(self):
        ...
        await self.env_agent.send_broadcast(
            payload={"announcement": "System-Update in 5 Minuten", "action": "save_state", "timestamp": time.time()},
        ...
        if success:
            ...
            logger.info(
                f"   Broker Stats: {broker_stats['messages_delivered']} delivered, {broker_stats['messages_failed']} failed"
            )
        else:
        ...
        self.test_results.append({"test": "Broadcast", "success": success, "latency_ms": elapsed_ms})
    ...
    async def test_context_sharing(self):
        ...
        self.test_results.append({"test": "Context-Sharing", "success": success, "latency_ms": elapsed_ms})
    ...
    async def test_concurrent_requests(self):
        ...
        for i in range(10):
            task = self.env_agent.send_request(
                recipient=self.fin_agent.identity, payload={"query": f"Cost analysis {i}", "request_id": i}, timeout=10.0
            )
            tasks.append(task)
    ...
    async def test_performance_benchmark(self):
        ...
        for i in range(50):
            task = self.env_agent.send_request(
                recipient=self.fin_agent.identity, payload={"query": f"Benchmark request {i}"}, timeout=15.0
            )
            tasks.append(("request", task))

        # 30 Events
        for i in range(30):
            task = self.env_agent.publish_event(topic="rag_context_updates", payload={"benchmark_event": i})
            tasks.append(("event", task))

        # 20 Broadcasts
        for i in range(20):
            task = self.env_agent.send_broadcast(payload={"benchmark_broadcast": i})
