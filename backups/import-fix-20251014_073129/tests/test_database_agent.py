...
def test_database():
    ...
    try:
        ...
        cursor.executemany("INSERT INTO users (id, username, email, status, role) VALUES (?, ?, ?, ?, ?)", test_users)
        ...
        cursor.executemany(
            "INSERT INTO documents (id, title, category, content, user_id) VALUES (?, ?, ?, ?, ?)", test_documents
        )
...
class TestDatabaseAgent:
    ...
    async def test_simple_select(self, database_agent, test_database):
        ...
        request = DatabaseQueryRequest(query_id="test_001", sql_query="SELECT * FROM users", database_path=test_database)
    ...
    async def test_select_with_where(self, database_agent, test_database):
        ...
        request = DatabaseQueryRequest(
            query_id="test_002", sql_query="SELECT * FROM users WHERE status = 'active'", database_path=test_database
        )
    ...
    async def test_pragma_query(self, database_agent, test_database):
        ...
        request = DatabaseQueryRequest(query_id="test_004", sql_query="PRAGMA table_info(users)", database_path=test_database)
    ...
    async def test_insert_blocked(self, database_agent, test_database):
        ...
        request = DatabaseQueryRequest(
            query_id="test_005", sql_query="INSERT INTO users (username) VALUES ('hacker')", database_path=test_database
        )
    ...
    async def test_update_blocked(self, database_agent, test_database):
        ...
        request = DatabaseQueryRequest(
            query_id="test_006", sql_query="UPDATE users SET role = 'admin' WHERE id = 1", database_path=test_database
        )
    ...
    async def test_delete_blocked(self, database_agent, test_database):
        ...
        request = DatabaseQueryRequest(
            query_id="test_007", sql_query="DELETE FROM users WHERE id = 1", database_path=test_database
        )
    ...
    async def test_max_results_limit(self, database_agent, test_database):
        ...
        request = DatabaseQueryRequest(
            query_id="test_008", sql_query="SELECT * FROM users", database_path=test_database, max_results=2  # Limit auf 2
        )
    ...
    async def test_schema_extraction(self, database_agent, test_database):
        ...
        request = DatabaseQueryRequest(
            query_id="test_009", sql_query="SELECT * FROM users LIMIT 1", database_path=test_database, include_schema=True
        )
    ...
    async def test_database_not_found(self, database_agent):
        ...
        request = DatabaseQueryRequest(
            query_id="test_010", sql_query="SELECT * FROM users", database_path="/nonexistent/database.db"
        )
    ...
    async def test_query_statistics(self, test_database):
        ...
        for i in range(5):
            request = DatabaseQueryRequest(
                query_id=f"test_stats_{i}", sql_query="SELECT * FROM users", database_path=test_database
            )
    ...
    async def test_query_caching(self, database_agent, test_database):
        ...
        request = DatabaseQueryRequest(query_id="test_cache_001", sql_query="SELECT * FROM users", database_path=test_database)
...
class TestDatabaseAgentIntegration:
    ...
    async def test_full_workflow(self, test_database):
        ...
        # 1. Create Agent
        config = DatabaseConfig(max_results=10, enable_query_cache=True, log_all_queries=False)
        agent = create_database_agent(config)
