...
def sample_messages() -> List[Dict]:
    ...
    return [
        {"role": "user", "content": "Was ist VERITAS?", "timestamp": "2025-10-09 14:20:00", "attachments": []},
        {
            "role": "assistant",
            "content": "# VERITAS\n\nVERITAS ist ein **intelligentes RAG-System**.",
            "timestamp": "2025-10-09 14:20:05",
            "metadata": {"confidence": 92, "duration": 2.3, "sources_count": 3},
            "sources": [
        ...
        },
        {
            ...
            "attachments": [{"name": "architecture.pdf", "size": 1234567, "path": "/tmp/arch.pdf"}],
        },
        {
            "role": "assistant",
            ...
            "sources": [{"title": "Agent Documentation", "page": 8, "relevance": 0.92}],
        },
    ]
...
def sample_feedback_stats() -> Dict:
    ...
    return {
        ...
        "positive_ratio": 80.0,
        "average_rating": 4.2,
        "categories": {"accuracy": 45, "completeness": 38, "relevance": 42, "performance": 25},
    }
...
def sample_files(temp_export_dir):
    ...
    for filename, content, mime_type in test_files:
        ...
        files.append({"name": filename, "path": str(file_path), "size": len(content), "mime_type": mime_type})
...
def mock_feedback_api(mocker):
    ...
    api_mock.get_stats.return_value = {"total_feedback": 150, "positive_ratio": 80.0}
    return api_mock
...
def large_message_set() -> List[Dict]:
    ...
    for i in range(100):
        # User message
        messages.append(
            {"role": "user", "content": f"Test query {i+1}", "timestamp": f"2025-10-09 14:{i:02d}:00", "attachments": []}
        )
        # Assistant message
        messages.append(
            {
                ...
                "metadata": {"confidence": 85 + (i % 15), "duration": 1.5 + (i % 5) * 0.5, "sources_count": 3 + (i % 5)},
                "sources": [{"title": f"Source {j+1}", "page": j + 1, "relevance": 0.9 - j * 0.1} for j in range(3)],
            }
        )
    return messages
...
def pytest_configure(config):
    ...
    config.addinivalue_line("markers", "slow: marks tests as slow (deselect with '-m \"not slow\"')")
    config.addinivalue_line("markers", "integration: marks tests as integration tests")
    config.addinivalue_line("markers", "ui: marks tests requiring UI (Tkinter)")
