...
async def test_hybrid_retriever_vector_search_warning_once():
    ...
    assert (
        "vector_search" in log_output_init or "search_documents" in log_output_init
...
async def test_sparse_retriever_bm25_empty_warning_once():
    ...
    assert "BM25 Index ist leer" in log_output_first, "Warning sollte beim ersten search() geloggt werden"
    ...
    assert (
        "BM25 Index ist leer" not in log_output_subsequent
...
async def test_hybrid_retrieval_multiple_queries_no_spam():
    ...
    assert warning_count == 0, f"0 Warnings erwartet bei 10 Queries, aber {warning_count} gefunden:\n{log_output}"
...
def test_hybrid_retriever_flags_initialized():
    ...
    assert retriever1._vector_search_available == True, "Flag sollte True sein wenn vector_search existiert"
    ...
    assert retriever2._vector_search_available == False, "Flag sollte False sein wenn vector_search nicht existiert"
...
def test_sparse_retriever_flag_initialized():
    ...
    assert retriever._empty_index_warning_shown == False, "Flag sollte initial False sein"

    # Nach erstem search() sollte Flag True sein
    retriever.search(query="Test")

    assert retriever._empty_index_warning_shown == True, "Flag sollte nach erstem search() True sein"
...
async def test_performance_cached_flags_vs_hasattr():
    ...
    assert cached_time < 1.0, f"100 Queries sollten < 1s dauern, aber {cached_time}s gemessen"
