...
try:
    ...
    from protocols.agent_message import AgentIdentity, AgentMessage, MessagePriority, MessageType, create_request_message
...
class PerformanceBenchmark:
    ...
    async def test_baseline_vs_optimized(self):
        ...
        config_baseline = BrokerConfiguration(num_workers=1, enable_batching=False)
        throughput_baseline, latency_baseline = await self.measure_throughput(config_baseline, num_messages)

        # Optimized: 5 Workers, Batching
        print(f"  Running OPTIMIZED (5 workers, batching, {num_messages} messages)...")
        config_optimized = BrokerConfiguration(num_workers=5, enable_batching=True, batch_size=20)
        throughput_optimized, latency_optimized = await self.measure_throughput(config_optimized, num_messages)
    ...
    async def test_worker_scaling(self):
        ...
        for num_workers in worker_counts:
            ...
            config = BrokerConfiguration(num_workers=num_workers, enable_batching=True, batch_size=20)

            throughput, latency = await self.measure_throughput(config, num_messages)
            results_scaling.append({"workers": num_workers, "throughput": throughput, "latency": latency})

            print(f"    -> {throughput:.1f} msg/s, {latency:.1f}ms latency")

        self.results.append({"test": "Worker Scaling", "results": results_scaling})
        ...
        for i, result in enumerate(results_scaling):
            if i == 0:
                ...
                print(
                    f"    {result['workers']} worker(s):  {result['throughput']:.1f} msg/s ({scaling_factor:.1f}x, {efficiency:.0f}% efficiency)"
                )
        print()
    ...
    async def test_batch_size_optimization(self):
        ...
        for batch_size in batch_sizes:
            ...
            config = BrokerConfiguration(num_workers=5, enable_batching=True, batch_size=batch_size)

            throughput, latency = await self.measure_throughput(config, num_messages)
            results_batching.append({"batch_size": batch_size, "throughput": throughput, "latency": latency})
        ...
        self.results.append(
            {"test": "Batch Size Optimization", "results": results_batching, "optimal_batch_size": optimal["batch_size"]}
        )
    ...
    async def test_latency_under_load(self):
        ...
        for i in range(num_messages):
            ...
            message = create_request_message(sender=agent_a, recipient=agent_b, payload={"data": i})
        ...
        self.results.append(
            {"test": "Latency under Load", "p50_latency_ms": p50, "p95_latency_ms": p95, "p99_latency_ms": p99}
        )
    ...
    async def test_concurrent_requests(self):
        ...
        for i in range(num_concurrent):
            message = create_request_message(sender=agent_a, recipient=agent_b, payload={"request_id": i})
        ...
        self.results.append(
            {"test": "Concurrent Requests", "num_concurrent": num_concurrent, "elapsed_sec": elapsed, "throughput": throughput}
        )
    ...
    async def measure_throughput(self, config: BrokerConfiguration, num_messages: int) -> tuple[float, float]:
        """
    ...
    async def create_test_agent(self, agent_id: str, broker: AgentMessageBroker) -> AgentIdentity:
        ...
        identity = AgentIdentity(
            agent_id=agent_id, agent_type="test", agent_name=f"Test Agent {agent_id}", capabilities=["test"]
        )
    ...
    def print_summary(self):
        ...
        if test1:
            ...
            print(
                f"   Improvement: {test1['improvement_factor']:.1f}x ({test1['baseline_throughput']:.1f} -> {test1['optimized_throughput']:.1f} msg/s)"
            )
        ...
        if test4:
            ...
            print(
                f"   P95: {test4['p95_latency_ms']:.1f}ms ({('< 200ms [PASS]' if test4['p95_latency_ms'] < 200 else '>= 200ms [FAIL]')})"
            )
