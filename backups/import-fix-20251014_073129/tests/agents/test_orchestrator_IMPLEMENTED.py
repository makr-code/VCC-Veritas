...
class TestAgentOrchestrator:
    ...
    def mock_agent_coordinator(self):
        ...
        coordinator.dispatch_agent = AsyncMock(return_value={"status": "success", "result": {"data": "test result"}})
    ...
    def mock_pipeline_manager(self):
        ...
        manager.get_pipeline = Mock(return_value={"pipeline_id": "pipeline_123", "status": "active"})
        return manager
    ...
    def orchestrator_instance(self, schema_dir, mock_agent_coordinator, mock_pipeline_manager):
        ...
        orchestrator = AgentOrchestrator(
            schema_dir=schema_dir, agent_coordinator=mock_agent_coordinator, pipeline_manager=mock_pipeline_manager
        )
        return orchestrator
    ...
    def test_add_pipeline_to_storage(self, orchestrator_instance):
        ...
        pipeline = QueryPipeline(
            pipeline_id=pipeline_id, query_id=query_id, query_text="Test query", schema_name="test_schema"
        )
    ...
    def test_orchestration_query_complexity_detection(self):
        ...
        # Complex query
        complex_query = (
            "Analyze environmental impact of construction project considering all regulations, costs, and social factors"
        )
    ...
    def test_pipeline_creation_performance(self, orchestrator_instance):
        ...
        for i in range(10):
            pipeline = QueryPipeline(
                pipeline_id=f"perf_test_{i}", query_id=f"query_{i}", query_text=f"Test query {i}", schema_name="test_schema"
            )
