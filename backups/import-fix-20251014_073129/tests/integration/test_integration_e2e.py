...
class TestUploadQueryFeedbackExportWorkflow:
    ...
    def test_complete_workflow(self, mock_backend, mock_uds3, tmp_path):
        ...
        feedback_result = mock_backend.submit_feedback(message_id=message_id, rating=1, category="helpful")  # Positive
    ...
    def test_workflow_with_multiple_queries(self, mock_backend, mock_uds3, tmp_path):
        ...
        messages = []
        queries = ["Was ist das BImSchG?", "Welche Hauptziele verfolgt das BImSchG?", "Was sind die wichtigsten Paragrafen?"]
        ...
        for i, query in enumerate(queries):
            ...
            # Store messages
            messages.append({"role": "user", "content": query, "timestamp": datetime.now()})
            messages.append(
                {
                    "role": "assistant",
                    "content": response["answer"],
...
class TestErrorRecovery:
    ...
    def test_partial_workflow_recovery(self, mock_backend, mock_uds3, tmp_path):
        ...
        messages.append({"role": "user", "content": "Query 1", "timestamp": datetime.now()})
        messages.append({"role": "assistant", "content": response1["answer"], "timestamp": datetime.now()})
...
class TestConcurrentOperations:
    ...
    def test_concurrent_feedback_submissions(self, mock_backend):
        ...
        def submit_feedback(msg_id):
            result = mock_backend.submit_feedback(message_id=f"msg_{msg_id}", rating=1, category="helpful")
