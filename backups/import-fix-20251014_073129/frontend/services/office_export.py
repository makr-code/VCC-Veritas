...
class OfficeExportService:
    ...
    def export_to_word(
        ...
        # Metadata Header
        doc.add_paragraph(f"Exportiert am: {datetime.now().strftime('%d.%m.%Y %H:%M')}", style="Subtitle")
        doc.add_paragraph(f"Anzahl Messages: {len(chat_messages)}", style="Subtitle")
        doc.add_paragraph("")  # Spacing

        # Messages
        for i, msg in enumerate(chat_messages, 1):
            self._add_message_to_word(doc, msg, i, include_metadata=include_metadata, include_sources=include_sources)
    ...
    def _add_message_to_word(self, doc: Document, message: Dict, index: int, include_metadata: bool, include_sources: bool):
    ...
    def export_to_excel(
        self, chat_messages: List[Dict], feedback_stats: Optional[Dict] = None, filename: Optional[str] = None
    ) -> Path:
        """
    ...
    def _create_sources_sheet(self, ws, messages: List[Dict]):
        ...
        for i, msg in enumerate(messages, 1):
            ...
            for source in sources:
                row = [i, source.get("title", "Unbekannt"), source.get("page", ""), source.get("relevance", "")]
                ws.append(row)
    ...
    def filter_messages_by_date(self, messages: List[Dict], days: int = 7) -> List[Dict]:
        """
