...
class ExportDialog:
    ...
    def __init__(self, parent: tk.Tk, on_export: Callable[[Dict], None], supported_formats: List[str] = [".docx", ".xlsx"]):
        """
    ...
    def _build_ui(self):
        ...
        title_label = tk.Label(title_frame, text="📤 Chat-Export", font=("Segoe UI", 16, "bold"), fg="white", bg="#4472C4")
        ...
        # 1. Format Selection
        format_frame = tk.LabelFrame(content_frame, text="📄 Dateiformat", font=("Segoe UI", 10, "bold"), padx=10, pady=10)
        ...
        for fmt in self.supported_formats:
            ...
            rb = tk.Radiobutton(format_frame, text=label, variable=self.format_var, value=fmt, font=("Segoe UI", 9))
            rb.pack(anchor=tk.W, pady=2)

        # 2. Time Period
        period_frame = tk.LabelFrame(content_frame, text="⏰ Zeitraum", font=("Segoe UI", 10, "bold"), padx=10, pady=10)
        ...
        for value, label in periods:
            rb = tk.Radiobutton(period_frame, text=label, variable=self.period_var, value=value, font=("Segoe UI", 9))
            rb.pack(anchor=tk.W, pady=2)

        # 3. Options
        options_frame = tk.LabelFrame(content_frame, text="⚙️ Optionen", font=("Segoe UI", 10, "bold"), padx=10, pady=10)
        ...
        cb_sources = tk.Checkbutton(
            options_frame, text="📚 Quellenverzeichnis einschließen", variable=self.include_sources_var, font=("Segoe UI", 9)
        )
        cb_sources.pack(anchor=tk.W, pady=2)

        # 4. Filename (optional)
        filename_frame = tk.LabelFrame(
            content_frame, text="📝 Dateiname (optional)", font=("Segoe UI", 10, "bold"), padx=10, pady=10
        )
        filename_frame.pack(fill=tk.X, pady=(0, 10))

        filename_entry = tk.Entry(filename_frame, textvariable=self.custom_filename_var, font=("Segoe UI", 9))
...
def show_export_dialog(
    parent: tk.Tk, on_export: Callable[[Dict], None], supported_formats: Optional[List[str]] = None
) -> None:
    """
    Zeigt Export-Dialog
