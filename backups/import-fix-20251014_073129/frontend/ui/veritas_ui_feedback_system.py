...
class MessageFeedbackWidget:
        ...
        try:
            with open(filename, "w", newline="", encoding="utf-8") as csvfile:
                ...
                for fb in self.feedback_list:
                    writer.writerow([fb.get("rating", ""), fb.get("detailed_feedback", ""), fb.get("timestamp", "")])
    ...
    def create_feedback_buttons(self):
        ...
        self.feedback_entry = tk.Entry(self.feedback_input_frame, bd=1, relief="solid", width=25, highlightthickness=0)
    ...
    def _bookmark_answer(self):
        ...
        try:
            ...
            if content:
                ...
                bookmark = {"content": content, "timestamp": datetime.datetime.now().isoformat()}
    ...
    def _show_save_dialog(self):
        ...
        selected = simpledialog.askstring(
            "Exportformat wählen", "Bitte Exportformat eingeben (CSV, PDF, JSON, Word):", initialvalue="CSV"
        )
        if selected:
    ...
    def _toggle_feedback(self, is_positive: bool):
        ...
        self.feedback_buttons[is_positive].config(bg=positive_color if is_positive else negative_color)
    ...
    def _add_feedback_status_display(self, parent_container):
        ...
        # Status-Label
        status_label = tk.Label(status_frame, text=f"{icon} {display_text}")
    ...
    def _show_feedback_details(self):
        ...
        title_label = tk.Label(header_frame, text=f"📝 Feedback-Verlauf ({len(self.feedback_list)} Einträge)")
        title_label.pack()
        ...
        # Schließen-Button
        close_btn = tk.Button(dialog, text="Schließen", relief="flat", bd=0, padx=20, pady=8, command=dialog.destroy)
    ...
    def _create_feedback_item(self, parent, feedback: Dict, index: int):
        ...
        header_label = tk.Label(header_frame, text=f"#{index} {icon} {rating_text}")
        ...
        if detail_text:
            detail_label = tk.Label(item_frame, text=detail_text, anchor="w", justify="left", wraplength=450)
...
class FeedbackManager:
    ...
    def add_message_feedback(self, message_id: str, message_data: Dict):
        ...
        feedback_widget = MessageFeedbackWidget(
            parent_display=self.chat_window.chat_display, message_data=message_data, save_callback=self.chat_window.save_chat
        )
