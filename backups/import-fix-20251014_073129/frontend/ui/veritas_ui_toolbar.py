...
class ChatToolbar(ttk.Frame):
    ...
    def _create_hamburger_button(self, parent):
        ...
        hamburger_btn = ttk.Button(parent, text="☰", command=self._show_hamburger_menu, width=3)
            ...
            if response.status_code == 200:
                ...
                if user_chats:
                    for chat in user_chats:
                        title = chat.get("title") or f"Chat {chat.get('session_id')}"
                        submenu.add_command(
                            label=title, command=lambda chat_id=chat.get("session_id"): self._open_and_continue_chat(chat_id)
                        )
                else:
    ...
    def _open_and_continue_chat(self, chat_id):
        ...
        if app and app.chat_windows:
            other_chats = [
                w
                for w in app.chat_windows.values()
            ...
            if other_chats:
                ...
                for chat_window in other_chats[:5]:
                    ...
                    menu.add_command(label=f"   → {chat_name}", command=lambda w=chat_window: self._switch_to_chat(w))
                menu.add_separator()
    ...
    def _create_user_profile_section(self, parent_frame):
        ...
        self.user_main_btn = ttk.Button(user_frame, text=user_text, command=self._edit_user_profile, width=10)
        self.user_main_btn.pack(side="left")

        # Dropdown-Button
        self.user_dropdown_btn = ttk.Button(user_frame, text="▼", command=self._show_user_menu, width=2)
    ...
    def _create_chat_actions(self, parent):
        ...
        # Chat exportieren
        export_btn = ttk.Button(parent, text="📋", width=3, command=self._export_chat)
        export_btn.pack(side="left", padx=(0, 2))
        Tooltip(export_btn, "Chat als Textdatei exportieren")

        # Chat löschen
        clear_btn = ttk.Button(parent, text="🗑️", width=3, command=self._clear_chat)
        clear_btn.pack(side="left", padx=(0, 2))
        Tooltip(clear_btn, "Chat-Verlauf löschen")

        # Detaillierte Quellen
        sources_btn = ttk.Button(parent, text="📚", width=3, command=self._show_detailed_sources)
    ...
    def _create_api_status_button(self, parent):
        ...
        if accent_style:
            self.api_status_btn = ttk.Button(parent, text="🔴", width=3, style=accent_style, command=self._check_api_status)
        else:
            self.api_status_btn = ttk.Button(parent, text="🔴", width=3, command=self._check_api_status)
    ...
    def _load_user_info(self):
        ...
        try:
            ...
            if os.path.exists(user_file):
                ...
                # Standard-Benutzer
                return {"name": "Benutzer", "email": "benutzer@veritas.local", "avatar": "👤", "theme": "dark"}
        except Exception as e:
    ...
    def _show_user_menu(self):
        ...
        # Benutzer-Info
        menu.add_command(label=f"👤 {self.current_user['name']}", state="disabled")
        menu.add_command(label=f"📧 {self.current_user['email']}", state="disabled")
        menu.add_separator()

        # Profil bearbeiten
        menu.add_command(label="✏️ Profil bearbeiten", command=self._edit_user_profile)
    ...
    def _show_settings(self):
        """Zeigt Einstellungen."""
        messagebox.showinfo("Einstellungen", "Einstellungen-Dialog ist noch nicht implementiert.")

    def _show_info(self):
        """Zeigt Programm-Informationen."""
        messagebox.showinfo("Info", "Veritas Chat v3.1.0\n\n" "RAG-basiertes Chat-System mit\n" "Multi-Datenbank Support")
