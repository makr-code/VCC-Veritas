...
def end_entity_csr():
    ...
    csr = generate_csr(private_key, common_name="endentity.example.com", organization="Test Corp")
...
def intermediate_ca_csr():
    ...
    csr = generate_csr(private_key, common_name="Intermediate CA", organization="Test Corp")
    return csr, private_key
...
class TestRootCA:
    ...
    def test_initialize_root_ca_custom_key_size(self, ca_service):
        ...
        ca_id, _, _ = ca_service.initialize_root_ca(common_name="Test Root CA", validity_days=3650, key_size=3072)

        assert ca_id is not None

    def test_initialize_root_ca_saved_to_disk(self, ca_service):
        """Test Root CA is saved to disk"""
        ca_id, _, _ = ca_service.initialize_root_ca(common_name="Test Root CA", validity_days=3650)

        # Check certificate file
        cert_path = os.path.join(ca_service.ca_storage_path, "ca_certificates", f"{ca_id}.pem")
        assert os.path.exists(cert_path)

        # Check key file
        key_path = os.path.join(ca_service.ca_storage_path, "ca_keys", f"{ca_id}_key.pem")
...
class TestCSRSigningEndEntity:
    ...
    def test_sign_csr_end_entity_custom_validity(self, ca_service, end_entity_csr):
        ...
        cert_id, cert_pem = ca_service.sign_csr(csr_pem=csr_pem, validity_days=730, is_ca=False, ca_id=ca_id)  # 2 years
    ...
    def test_sign_csr_nonexistent_ca(self, ca_service, end_entity_csr):
        ...
        with pytest.raises(RuntimeError) as exc_info:
            ca_service.sign_csr(csr_pem=csr_pem, validity_days=365, is_ca=False, ca_id="nonexistent_ca")
    ...
    def test_sign_invalid_csr(self, ca_service):
        ...
        with pytest.raises(RuntimeError):
            ca_service.sign_csr(csr_pem=b"invalid_csr", validity_days=365, is_ca=False, ca_id=ca_id)
...
class TestCSRSigningIntermediateCA:
    ...
    def test_sign_csr_intermediate_ca(self, ca_service, intermediate_ca_csr):
        ...
        # Initialize Root CA
        root_ca_id, _, _ = ca_service.initialize_root_ca(common_name="Test Root CA", validity_days=3650, key_size=4096)

        # Sign intermediate CA CSR
        csr_pem, _ = intermediate_ca_csr
        cert_id, cert_pem = ca_service.sign_csr(csr_pem=csr_pem, validity_days=1825, is_ca=True, ca_id=root_ca_id)  # 5 years
    ...
    def test_intermediate_ca_path_length(self, ca_service, intermediate_ca_csr):
        ...
        # Sign intermediate CA CSR
        csr_pem, _ = intermediate_ca_csr
        cert_id, cert_pem = ca_service.sign_csr(csr_pem=csr_pem, validity_days=1825, is_ca=True, ca_id=root_ca_id)
...
class TestCRLGeneration:
    ...
    def test_generate_crl_empty(self, ca_service):
        ...
        crl_pem = ca_service.generate_crl(ca_id=ca_id, revoked_certs=[])
    ...
    def test_generate_crl_with_revoked_cert(self, ca_service, end_entity_csr):
        ...
        # Generate CRL
        revoked_certs = [
            {"serial_number": 1, "revocation_date": cert_info["revoked_at"], "reason": "keyCompromise"}  # Simplified
        ]
    ...
    def test_generate_crl_multiple_revoked(self, ca_service):
        ...
        revoked_certs = [
            {"serial_number": 1, "revocation_date": datetime.now(timezone.utc).isoformat(), "reason": "keyCompromise"},
            {"serial_number": 2, "revocation_date": datetime.now(timezone.utc).isoformat(), "reason": "superseded"},
            {"serial_number": 3, "revocation_date": datetime.now(timezone.utc).isoformat(), "reason": "cessationOfOperation"},
        ]
    ...
    def test_generate_crl_saved_to_disk(self, ca_service):
        ...
        # Check CRL file
        crl_path = os.path.join(ca_service.ca_storage_path, "crl", f"{ca_id}_crl.pem")
...
class TestCertificateChainValidation:
    ...
    def test_verify_self_signed_certificate(self, ca_service):
        ...
        is_valid, message = ca_service.verify_certificate_chain(cert_pem=ca_cert_pem, ca_id=ca_id)
    ...
    def test_verify_invalid_certificate(self, ca_service):
        ...
        is_valid, message = ca_service.verify_certificate_chain(cert_pem=b"invalid_cert", ca_id=ca_id)
    ...
    def test_verify_nonexistent_ca(self, ca_service):
        ...
        is_valid, message = ca_service.verify_certificate_chain(cert_pem=ca_cert_pem, ca_id="nonexistent_ca")
...
class TestCAStatistics:
    ...
    def test_statistics_multiple_cas(self, ca_service):
        ...
        ca_ids = []
        for i in range(3):
            ca_id, _, _ = ca_service.initialize_root_ca(common_name=f"Root CA {i}", validity_days=3650)
            ca_ids.append(ca_id)
...
class TestCAServiceIntegration:
    ...
    def test_full_ca_workflow(self, ca_service, end_entity_csr):
        ...
        # 5. Generate CRL
        cert_info = ca_service.cert_manager.get_certificate(cert_id)
        revoked_certs = [{"serial_number": 1, "revocation_date": cert_info["revoked_at"], "reason": "keyCompromise"}]
        crl_pem = ca_service.generate_crl(ca_id, revoked_certs)
        assert crl_pem is not None
    ...
    def test_two_tier_pki(self, ca_service, intermediate_ca_csr, end_entity_csr):
        ...
        int_ca_id, int_ca_cert = ca_service.sign_csr(csr_pem=int_csr_pem, validity_days=1825, is_ca=True, ca_id=root_ca_id)

        # Save intermediate CA to CA storage (for signing)
        int_ca_cert_path = os.path.join(ca_service.ca_storage_path, "ca_certificates", f"{int_ca_id}.pem")
        int_ca_key_path = os.path.join(ca_service.ca_storage_path, "ca_keys", f"{int_ca_id}_key.pem")
        ...
        ee_cert_id, ee_cert_pem = ca_service.sign_csr(csr_pem=ee_csr_pem, validity_days=365, is_ca=False, ca_id=int_ca_id)
...
class TestCAServiceErrors:
    ...
    def test_initialize_ca_invalid_key_size(self, ca_service):
        ...
        with pytest.raises(ValueError):
            ca_service.initialize_root_ca(common_name="Test Root CA", validity_days=3650, key_size=1024)  # Too small
    ...
    def test_sign_csr_invalid_validity(self, ca_service, end_entity_csr):
        ...
        with pytest.raises(ValueError):
            ca_service.sign_csr(csr_pem=csr_pem, validity_days=-1, is_ca=False, ca_id=ca_id)  # Invalid
