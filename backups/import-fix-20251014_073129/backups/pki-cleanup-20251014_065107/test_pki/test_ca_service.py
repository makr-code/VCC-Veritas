...
from pki.crypto_utils import generate_csr, generate_key_pair
from pki.exceptions import CANotInitializedError, InvalidCSRError
...
class TestCSRSigning:
    ...
    def test_sign_csr_basic(self, initialized_ca):
        ...
        csr_pem = generate_csr(private_key_pem=private_key, common_name="test.veritas.local")
    ...
    def test_sign_csr_with_validity(self, initialized_ca):
        ...
        csr_pem = generate_csr(private_key_pem=private_key, common_name="validity.test")
    ...
    def test_sign_multiple_csrs(self, initialized_ca):
        ...
        for i in range(5):
            private_key, _ = generate_key_pair()
            csr_pem = generate_csr(private_key_pem=private_key, common_name=f"test{i}.local")
...
class TestCRL:
    ...
    def test_get_crl_with_revoked(self, initialized_ca):
        ...
        private_key, _ = generate_key_pair()
        csr_pem = generate_csr(private_key_pem=private_key, common_name="revoke.test")
        cert = initialized_ca.sign_csr(csr_pem)

        # Widerrufe Zertifikat
        initialized_ca.revoke_certificate(cert["serial_number"], reason="key_compromise")
...
class TestCertificateRevocation:
    ...
    def test_revoke_with_reason(self, initialized_ca):
        ...
        private_key, _ = generate_key_pair()
        csr_pem = generate_csr(private_key_pem=private_key, common_name="revoke_reason.test")
        cert = initialized_ca.sign_csr(csr_pem)

        result = initialized_ca.revoke_certificate(cert["serial_number"], reason="superseded")
...
class TestChainVerification:
    ...
    def test_verify_valid_chain(self, initialized_ca):
        ...
        # Erstelle und signiere Zertifikat
        private_key, _ = generate_key_pair()
        csr_pem = generate_csr(private_key_pem=private_key, common_name="verify.test")
        cert = initialized_ca.sign_csr(csr_pem)
    ...
    def test_verify_revoked_certificate(self, initialized_ca):
        ...
        # Erstelle Zertifikat
        private_key, _ = generate_key_pair()
        csr_pem = generate_csr(private_key_pem=private_key, common_name="verify_revoked.test")
        cert = initialized_ca.sign_csr(csr_pem)
        ...
        is_valid = initialized_ca.verify_chain(cert["cert_pem"], include_crl_check=True)
...
class TestSignedCertificates:
    ...
    def test_get_signed_certificates_exclude_revoked(self, initialized_ca):
        ...
        # Ohne revoked
        active_certs = initialized_ca.get_signed_certificates(include_revoked=False)
        assert len(active_certs) == 1

        # Mit revoked
        all_certs = initialized_ca.get_signed_certificates(include_revoked=True)
