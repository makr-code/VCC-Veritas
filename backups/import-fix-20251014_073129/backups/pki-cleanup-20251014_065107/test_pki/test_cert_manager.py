...
import pytest
from pki.cert_manager import CertificateManager, CertificateStatus, CertificateType
from pki.exceptions import CertificateNotFoundError, InvalidCertificateError
...
class TestCertificateListing:
    ...
    def test_list_certificates_filter_by_status(self, cert_manager):
        ...
        valid_certs = cert_manager.list_certificates(status=CertificateStatus.VALID)
        revoked_certs = cert_manager.list_certificates(status=CertificateStatus.REVOKED)
    ...
    def test_list_certificates_filter_by_type(self, cert_manager):
        ...
        cert_manager.create_certificate("server.test", cert_type=CertificateType.SERVER)
        cert_manager.create_certificate("client.test", cert_type=CertificateType.CLIENT)

        server_certs = cert_manager.list_certificates(cert_type=CertificateType.SERVER)
        client_certs = cert_manager.list_certificates(cert_type=CertificateType.CLIENT)
...
class TestCertificateRevocation:
    ...
    def test_revoke_certificate_with_reason(self, cert_manager):
        ...
        cert_manager.revoke_certificate(cert["cert_id"], reason="key_compromise")
