...
from .crypto_utils import calculate_fingerprint, generate_csr, generate_key_pair
from .exceptions import CertificateExpiredError, CertificateNotFoundError, CertificateRevokedError, InvalidCertificateError
...
class CertificateManager:
    ...
    def create_certificate(
        ...
        if len(self._mock_certs) >= MOCK_MAX_CERTIFICATES:
            raise InvalidCertificateError(f"Maximum certificates reached: {MOCK_MAX_CERTIFICATES}")
...
-----END CERTIFICATE-----"""
    ...
    def get_certificate_by_fingerprint(self, fingerprint: str) -> Optional[Dict[str, Any]]:
        """
    ...
    def renew_certificate(self, cert_id: str, validity_days: int = CERT_VALIDITY_DAYS) -> Dict[str, Any]:
        """
    ...
    def get_statistics(self) -> Dict[str, Any]:
        ...
        valid = len([c for c in self._mock_certs.values() if c["status"] == CertificateStatus.VALID.value])
        expired = len([c for c in self._mock_certs.values() if c["status"] == CertificateStatus.EXPIRED.value])
        revoked = len([c for c in self._mock_certs.values() if c["status"] == CertificateStatus.REVOKED.value])

        # Zertifikate nach Typ
        by_type = {}
        for cert_type in CertificateType:
            count = len([c for c in self._mock_certs.values() if c["cert_type"] == cert_type.value])
