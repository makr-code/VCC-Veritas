...
class CAService:
    ...
    **Mode:** PRODUCTION (Hard Fail on Errors)
    ...
    def __init__(self, ca_storage_path: str = "./ca_storage", cert_manager: Optional[CertificateManager] = None):
        """
        ...
        try:
            ...
            if not self.ca_config_file.exists():
                self._save_ca_config({"root_ca_id": None, "intermediate_cas": [], "initialized": False})
    ...
    def initialize_root_ca(
        self, common_name: str, validity_days: int = 3650, key_size: int = 4096, **subject_attributes  # 10 years
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            if subject_attributes.get("organizational_unit"):
                subject_attrs.append(
                    x509.NameAttribute(NameOID.ORGANIZATIONAL_UNIT_NAME, subject_attributes["organizational_unit"])
                )
    ...
    def sign_csr(
        self, csr_pem: bytes, validity_days: int = 365, is_ca: bool = False, ca_id: str = "root_ca"
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            if is_ca:
                ...
                builder = builder.add_extension(
                    x509.BasicConstraints(ca=True, path_length=0), critical=True  # path_length=0 = can't sign other CAs
                )
                builder = builder.add_extension(
                    x509.KeyUsage(
                        digital_signature=True,
    ...
    def generate_crl(self, ca_id: str = "root_ca", revoked_certs: Optional[List[Dict[str, Any]]] = None) -> bytes:
        """
            ...
            if revoked_certs:
                for revoked_cert in revoked_certs:
                    ...
                    if reason:
                        ...
                        revoked_cert_builder = revoked_cert_builder.add_extension(x509.CRLReason(reason_flag), critical=False)
    ...
    def verify_certificate_chain(self, cert_pem: bytes, ca_id: str = "root_ca") -> Dict[str, Any]:
        """
        ...
        try:
            ...
            # Verify signature
            try:
                ca_cert.public_key().verify(cert.signature, cert.tbs_certificate_bytes, cert.signature_hash_algorithm)
            except Exception as e:
            ...
            return {"valid": len(errors) == 0, "errors": errors, "chain": chain}
