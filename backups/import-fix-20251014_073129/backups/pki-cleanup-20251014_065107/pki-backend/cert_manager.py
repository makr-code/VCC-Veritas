...
class CertificateManager:
    ...
    **Mode:** PRODUCTION (Hard Fail on Errors)
    ...
    def create_certificate(
        ...
        **subject_attributes,
        ...
        try:
            ...
            # Build subject name
            subject_attrs = [x509.NameAttribute(NameOID.COMMON_NAME, subject_name)]
            ...
            if issuer_cert_id and issuer_key_pem:
                ...
                issuer_cert_obj = x509.load_pem_x509_certificate(
                    issuer_cert["certificate_pem"].encode(), backend=default_backend()
                )
                builder = builder.issuer_name(issuer_cert_obj.subject)

                # Load issuer key for signing
                signing_key = serialization.load_pem_private_key(issuer_key_pem, password=None, backend=default_backend())
            else:
    ...
    def list_certificates(self, include_revoked: bool = False, is_ca: Optional[bool] = None) -> List[Dict[str, Any]]:
        """
    ...
    def validate_certificate(self, cert_id: str) -> Dict[str, Any]:
        ...
        errors = []
        warnings = []
        checks = {"exists": False, "not_expired": False, "not_revoked": False, "valid_signature": True}  # Simplified for now
