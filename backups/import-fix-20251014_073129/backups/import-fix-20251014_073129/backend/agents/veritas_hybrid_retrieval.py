...
class HybridRetrievalConfig:
    ...
    expansion_strategies: List[str] = field(default_factory=lambda: ["synonym", "context"])
...
class HybridRetriever:
    ...
    def __init__(
        ...
        rrf_config = RRFConfig(
            ...
            weights={"dense": self.config.dense_weight, "sparse": self.config.sparse_weight},
        )
        self.rrf = get_rrf(rrf_config)

        # Check Sparse availability
        self._sparse_available = self.config.enable_sparse and self.sparse_retriever.is_available()

        if not self._sparse_available:
            logger.warning("⚠️ Sparse Retrieval nicht verfügbar - verwende Dense-Only Modus")
        ...
        if not self._vector_search_available:
            ...
            if self._has_search_documents:
                logger.warning("⚠️ Dense Retriever hat keine vector_search Methode - verwende search_documents Fallback")
            else:
                logger.warning("⚠️ Dense Retriever hat keine vector_search Methode - Dense Retrieval deaktiviert")

        # Query Expansion initialisieren (optional)
        self._query_expansion_available = self.config.enable_query_expansion and QUERY_EXPANSION_AVAILABLE
        ...
        if self._query_expansion_available:
            try:
                ...
                expansion_config = QueryExpansionConfig(num_expansions=self.config.num_query_expansions, strategies=strategies)
    ...
    async def retrieve(
        ...
        enable_query_expansion = (
            enable_query_expansion if enable_query_expansion is not None else self.config.enable_query_expansion
        )
        ...
        if enable_query_expansion and self._query_expansion_available:
            try:
                expanded = await self.query_expander.expand(query, num_expansions=self.config.num_query_expansions)
                ...
                if query_expansion_applied:
                    logger.debug(f"🔍 Query Expansion: '{query[:50]}...' → {len(queries_to_search)} Varianten")
        ...
        if all_sparse_results and self.config.enable_fusion:
            ...
            fused_docs = self.rrf.fuse_two(all_dense_results, all_sparse_results, top_k=top_k)
            ...
            retrieval_time = time.time() - start_time
            expansion_info = f" + {len(queries_to_search)} Queries" if query_expansion_applied else ""
            logger.debug(f"🔍 Dense-Only Retrieval{expansion_info}: {len(hybrid_results)} Docs in {retrieval_time*1000:.1f}ms")

        return hybrid_results

    async def _retrieve_dense(self, query: str, params: Dict[str, Any]) -> List[Dict[str, Any]]:
        """
        ...
        try:
            ...
            if self._vector_search_available:
                results = await self.dense_retriever.vector_search(query=query, top_k=self.config.dense_top_k, **clean_params)
                ...
                results = await self.dense_retriever.search_documents(
                    query=query, limit=self.config.dense_top_k, **clean_params
                )
            else:
    ...
    async def _retrieve_sparse(self, query: str, params: Dict[str, Any]) -> List[Dict[str, Any]]:
        """
        ...
        try:
            ...
            return [
                {"doc_id": result.doc_id, "content": result.content, "score": result.score, "metadata": result.metadata}
                for result in results
            ]
    ...
    def _convert_fused_to_hybrid(self, fused_docs: List[FusedDocument]) -> List[HybridResult]:
    ...
    def _convert_dense_to_hybrid(self, dense_docs: List[Dict[str, Any]]) -> List[HybridResult]:
...
def create_hybrid_retriever(
    dense_retriever: Any, corpus: Optional[List[Dict[str, Any]]] = None, config: Optional[HybridRetrievalConfig] = None
) -> HybridRetriever:
    """
    ...
    return HybridRetriever(dense_retriever=dense_retriever, sparse_retriever=sparse_retriever, config=config)
