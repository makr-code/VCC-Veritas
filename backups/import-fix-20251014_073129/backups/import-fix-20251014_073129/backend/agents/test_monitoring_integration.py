...
from framework.agent_monitoring import AgentMonitor, HealthStatus, MonitoringService
...
def test_health_checks():
    ...
    # Create monitor
    monitor = AgentMonitor(agent_id="test_agent_health", agent_type="TestAgent")
    ...
    for i in range(5):
        monitor.record_step_execution(
            step_id=f"step_{i+1}", duration=0.1 + i * 0.02, status="success", quality_score=0.85 + i * 0.02
        )
        time.sleep(0.01)
    ...
    for i in range(3):
        monitor.record_step_execution(step_id=f"failed_{i+1}", duration=0.5, status="failed", quality_score=0.3, retry_count=2)
...
def test_prometheus_metrics():
    ...
    monitor = AgentMonitor(agent_id="prom_test_agent", agent_type="PrometheusTest")

    # Record executions
    for i in range(3):
        monitor.record_step_execution(
            step_id=f"step_{i+1}", duration=0.15, status="success" if i < 2 else "failed", quality_score=0.9 if i < 2 else 0.4
        )
