...
class AgentExecutionPlan:
    ...
    def to_dict(self) -> Dict[str, Any]:
        return {
            "parallel_agents": [{"subquery_id": sq_id, "assignment": a.to_dict()} for sq_id, a in self.parallel_agents],
            "sequential_agents": [{"subquery_id": sq_id, "assignment": a.to_dict()} for sq_id, a in self.sequential_agents],
...
Antworte NUR mit dem JSON-Array, kein zusätzlicher Text!"""
    ...
    def __init__(self, ollama_client: VeritasOllamaClient):
        ...
        self.stats = {"queries_decomposed": 0, "avg_subqueries_per_query": 0.0, "dependency_cycles_detected": 0}

    async def decompose_query(
        self, query_text: str, user_context: Dict[str, Any], complexity_hint: Optional[str] = None
    ) -> List[SubQuery]:
        """
        ...
        try:
            ...
            prompt = self.DECOMPOSITION_PROMPT.format(
                query_text=query_text, user_context=json.dumps(user_context, indent=2, ensure_ascii=False)
            )

            ollama_request = OllamaRequest(
                model="llama3.2:latest", prompt=prompt, temperature=0.3, max_tokens=1500  # Niedriger für konsistente Struktur
            )
            ...
            self.stats["avg_subqueries_per_query"] = (
                total_subqueries / self.stats["queries_decomposed"] if self.stats["queries_decomposed"] > 0 else 0.0
            )
...
class AgentSelector:
    ...
    def __init__(self):
        ...
        self.stats = {"selections_performed": 0, "avg_confidence_score": 0.0, "agent_usage_counts": defaultdict(int)}
    ...
    async def select_agents(self, subquery: SubQuery, rag_context: Optional[Dict[str, Any]] = None) -> AgentSelection:
        ...
        for agent_type, capabilities in self.agent_capability_map.items():
            ...
            match_score = self._calculate_capability_overlap(subquery.required_capabilities, capability_strs)

            if match_score > 0.3:  # Threshold
                matching_caps = list(set(subquery.required_capabilities) & set(capability_strs))
                matches.append(
                    AgentAssignment(
        ...
        logger.info(
            f"✅ Agent Selected: {selected[0].agent_type if selected else 'None'} (Conf: {selected[0].confidence_score if selected else 0.0:.2f})"
        )

        return AgentSelection(subquery_id=subquery.id, selected_agents=selected, fallback_agents=fallbacks)

    def _calculate_capability_overlap(self, required: List[str], available: List[str]) -> float:
        """
    ...
    def _boost_agent(self, matches: List[AgentAssignment], agent_type: str, boost_value: float, reason_suffix: str):
...
Antworte NUR mit der finalen Antwort, kein zusätzlicher Text!"""
    ...
    def __init__(self, ollama_client: VeritasOllamaClient):
        self.ollama_client = ollama_client
        self.stats = {"syntheses_performed": 0, "conflicts_detected": 0, "avg_confidence_score": 0.0}

    async def synthesize_results(self, agent_results: List[AgentResult], original_query: str) -> SynthesizedResult:
        """
        ...
        try:
            ...
            for i, agent_result in enumerate(deduplicated, 1):
                rag_context_parts.append(f"[{i}] {agent_result.agent_type}: {agent_result.response_text}")
            ...
            json_prompts = JSONCitationFormatter.get_json_prompt_template(enable_rich_media=enable_rich_media)
            ...
            user_prompt = json_prompts["user_template"].format(
                query=original_query, rag_context=rag_context, agent_results=agent_results_text, source_list=source_list
            )
            ...
            logger.info(
                f"[JSON SYNTHESIS] Prompt: {len(full_prompt)} Zeichen, " f"Quellen: {len(deduplicated)}, Format: JSON → IEEE"
            )
            ...
            return SynthesizedResult(
                ...
                metadata={"agent_count": len(deduplicated), "synthesis_timestamp": datetime.now(timezone.utc).isoformat()},
            )
    ...
    def _resolve_conflicts(self, agent_results: List[AgentResult], conflicts: List[Dict[str, Any]]) -> List[AgentResult]:
        """
        ...
        sorted_results = sorted(agent_results, key=lambda r: r.confidence_score, reverse=True)
    ...
    def _simple_aggregation_fallback(self, agent_results: List[AgentResult], original_query: str) -> SynthesizedResult:
...
class SupervisorAgent:
    ...
    async def create_agent_plan(
        self, subqueries: List[SubQuery], rag_context: Optional[Dict[str, Any]] = None
    ) -> AgentExecutionPlan:
        """
        ...
        return AgentExecutionPlan(
            ...
            metadata={"total_subqueries": len(subqueries), "plan_created_at": datetime.now(timezone.utc).isoformat()},
        )
