...
class FeedbackDatabase:
    ...
    def get_stats(self, days: int = 30) -> Dict[str, Any]:
        ...
        with sqlite3.connect(self.db_path) as conn:
            ...
            recent_feedback = [
                {"message_id": row[0], "rating": row[1], "category": row[2], "comment": row[3], "timestamp": row[4]}
    ...
    def get_feedback_list(self, limit: int = 50, offset: int = 0, rating_filter: Optional[int] = None) -> List[Dict[str, Any]]:
        """
...
@router.get("/list")
async def get_feedback_list(limit: int = 50, offset: int = 0, rating: Optional[int] = None):
    """
    Holt Liste von Feedbacks mit Pagination
    ...
    **Response:**
    ...
    """
    try:
        feedback_list = feedback_db.get_feedback_list(limit=limit, offset=offset, rating_filter=rating)

        return {"total": len(feedback_list), "limit": limit, "offset": offset, "feedback": feedback_list}
...
async def feedback_health():
    ...
    try:
        ...
        return {"status": "healthy", "database": "connected", "today_feedback": stats["total_feedback"]}
    except Exception as e:
