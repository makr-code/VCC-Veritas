        ...
        if available:
            ...
            try:
                ...
                return {
                    ...
                    "status": "healthy",
                    "health_info": health,
                    "config": {"host": config.host, "port": config.port, "ssl": config.use_ssl},
                    "stats": stats,
                }
            except Exception as e:
                await adapter.close()
                return {"available": False, "status": "unhealthy", "error": str(e)}
        else:
            return {"available": False, "status": "unavailable", "error": "ThemisDB adapter initialization failed"}
...
def check_uds3() -> Dict[str, Any]:
    ...
    try:
        ...
        if available:
            ...
            return {"available": False, "status": "unavailable", "error": "UDS3 adapter initialization failed"}
...
async def check_adapter_selection() -> Dict[str, Any]:
    ...
    """
    try:
        from backend.adapters import get_adapter_type, get_database_adapter
        ...
        return {"selected": True, "adapter_type": str(adapter_type), "adapter_class": adapter_name}
