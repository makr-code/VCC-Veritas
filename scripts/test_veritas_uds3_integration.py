...
from backend.agents.veritas_uds3_hybrid_agent_v2 import SearchResult, UDS3HybridSearchAgent
...
async def test_basic_hybrid_search():
    ...
    try:
        ...
        results = await agent.hybrid_search(
            query=query, top_k=10, weights={"vector": 0.5, "graph": 0.5}, search_types=["vector", "graph"]
        )
...
async def test_vector_only_search():
    ...
    try:
        ...
        results = await agent.vector_search(query=query, top_k=5)
...
async def test_graph_only_search():
    ...
    try:
        ...
        results = await agent.graph_search(query=query, top_k=5)
...
async def test_custom_weights():
    ...
    try:
        ...
        results = await agent.hybrid_search(
            query=query, top_k=5, weights={"vector": 0.2, "graph": 0.8}, search_types=["vector", "graph"]
        )
