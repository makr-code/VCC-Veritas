...
class DBFToSQLiteConverter:
    ...
    def convert_dbf_to_sqlite(
        self, dbf_path: str, sqlite_path: str, table_name: Optional[str] = None, overwrite: bool = False
    ) -> Dict[str, Any]:
        """
...
def main():
    ...
    for dbf_file in dbf_files:
        ...
        try:
            ...
            result = converter.convert_dbf_to_sqlite(
                dbf_path=str(dbf_file), sqlite_path=str(sqlite_path), table_name=table_name, overwrite=True
            )
    ...
    for result in results:
        if result["success"]:
            ...
            print(
                f"   req = DatabaseQueryRequest(query_id='test', sql_query='SELECT * FROM {result['table_name']} LIMIT 5', database_path='{result['sqlite_path']}'); "
            )
