...
class OllamaModelBenchmark:
    ...
    async def benchmark_model(self, model_name: str, model_details: Dict, num_runs: int = 3) -> ModelBenchmarkResult:
        ...
        result = ModelBenchmarkResult(model_name=model_name, available=True)
    ...
    async def _call_ollama(self, model: str, prompt: str) -> str:
        ...
        payload = {"model": model, "prompt": prompt, "stream": False, "options": {"temperature": 0.7, "num_predict": 50}}
    ...
    def print_summary(self, results: List[ModelBenchmarkResult]):
        ...
        # Sort by score
        sorted_results = sorted([r for r in results if r.available], key=lambda x: x.score, reverse=True)
        ...
        for i, result in enumerate(sorted_results, 1):
            ...
            print(
                f"{i:<6} {result.model_name:<25} {latency_str:<12} {quality_str:<10} {score_str:<8} {result.recommended_for:<12}"
            )
        ...
        if sorted_results:
            ...
            production_candidates = [r for r in sorted_results if r.avg_latency_ms < 500 and r.quality_score >= 6.0]
    ...
    def get_best_model(self, results: List[ModelBenchmarkResult]) -> Optional[str]:
        ...
        production = [r for r in available if r.avg_latency_ms < 500 and r.quality_score >= 6.0]
