...
IGNORE_PATTERNS = ["__pycache__", "*.pyc", ".git", ".vscode", "node_modules", "venv", ".pytest_cache"]
...
def analyze_file(file_path: Path) -> Dict[str, List[Tuple[int, str, str]]]:
    ...
    results = {"endpoints": [], "response_fields": []}
...
def create_backup():
    ...
    try:
        shutil.copytree(FRONTEND_DIR, backup_path, ignore=shutil.ignore_patterns(*IGNORE_PATTERNS))
...
def create_report(analysis_results: Dict[Path, Dict], migration_results: Dict[Path, Tuple[int, int]], output_path: Path):
...
def main():
    ...
    parser = argparse.ArgumentParser(description="VERITAS Frontend Migration - API v2 → v3")
    parser.add_argument("--analyze", action="store_true", help="Nur analysieren, keine Änderungen")
    parser.add_argument("--dry-run", action="store_true", help="Dry-Run: Zeige was geändert würde")
    parser.add_argument("--migrate", action="store_true", help="Live-Migration durchführen")
