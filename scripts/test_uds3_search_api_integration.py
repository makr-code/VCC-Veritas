...
async def test_veritas_agent():
    ...
    try:
        ...
        results = await agent.hybrid_search(query="Was regelt § 58 LBO BW?", top_k=5, weights={"vector": 0.5, "graph": 0.5})
        ...
        vector_results = await agent.vector_search(query="Photovoltaik", top_k=3)
        print(f"✅ Found {len(vector_results)} vector results")
        ...
        graph_results = await agent.graph_search(query="Energiegesetz", top_k=3)
        print(f"✅ Found {len(graph_results)} graph results")
        ...
        custom_results = await agent.hybrid_search(query="Photovoltaik", top_k=5, weights={"graph": 0.8, "vector": 0.2})
