...
async def test_hybrid_search(query: str):
    ...
    results = await agent.hybrid_search(query=query, top_k=5)
    ...
    if strategy.vector_backend is not None:
        ...
        results_vector = await agent.vector_search_only(query=query, top_k=5)
    ...
    if strategy.relational_backend is not None:
        ...
        results_keyword = await agent.keyword_search_only(query=query, top_k=5)
    ...
    results_custom = await agent.hybrid_search(query=query, top_k=5, weights={"vector": 0.7, "keyword": 0.2, "graph": 0.1})
    ...
    results_filtered = await agent.hybrid_search(query=query, top_k=5, filters={"document_type": "regulation"})
