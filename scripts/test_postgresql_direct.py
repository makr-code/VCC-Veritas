    ...
    try:
        ...
        if any("document" in str(t[0]).lower() for t in tables):
            ...
            for table in tables:
                ...
                if "document" in table_name.lower():
                    print(f"\n   Tabelle: {table_name}")
                    # Show first few rows
                    cur.execute(sql.SQL("SELECT * FROM {} LIMIT 3").format(sql.Identifier(table_name)))
                    rows = cur.fetchall()
        ...
        for table in tables:
            ...
            try:
                ...
                cur.execute(
                    sql.SQL(
                        "SELECT column_name FROM information_schema.columns WHERE table_name = %s AND data_type IN ('text', 'character varying')"
                    ),
                    (table_name,),
                )
                ...
                if text_columns:
                    ...
                    for col in text_columns[:3]:  # Limit to first 3 text columns
                        search_query = sql.SQL("SELECT COUNT(*) FROM {} WHERE {} ILIKE %s OR {} ILIKE %s").format(
