...
class OfficeRequestMetadata(BaseModel):
    ...
    mode: str = Field(
        default="ask",
        description="Query Mode: ask|agent|edit|plan"
    )
    
    scope: str = Field(
        default="selection",
        description="Scope: selection|document"
    )
    
    host: str = Field(
        default="word",
        description="Office Host: word|excel|powerpoint|outlook"
    )
    
    user_context: Optional[Dict[str, Any]] = Field(
        default_factory=dict,
        description="User Context (selection_length, document_name, etc.)"
    )
...
class OfficeRequestContent(BaseModel):
    ...
    query: str = Field(
        ...
        min_length=1,
        max_length=10000,
        description="User-Frage"
    )
    
    context: Optional[str] = Field(
        None,
        description="Markdown-formatierter Kontext (Selection/Document)"
    )
...
class OfficeAPIRequest(BaseModel):
    ...
    metadata: OfficeRequestMetadata = Field(
        default_factory=OfficeRequestMetadata,
        description="Request Metadata (Mode, Scope, Host)"
    )
    
    content: OfficeRequestContent = Field(
        ...,
        description="Request Content (Query, Context, History)"
    )
...
class OfficeResponseMetadata(BaseModel):
    ...
    processing_time_ms: Optional[int] = Field(
        None,
        description="Processing Time (Millisekunden)"
    )
    
    model: Optional[str] = Field(
        None,
        description="LLM Model (z.B. 'llama3.2')"
    )
...
class OfficeResponseContent(BaseModel):
    ...
    answer: str = Field(
        ...,
        description="Markdown-formatierte Antwort mit [1], [2], [3] Citations"
    )
    
    format: str = Field(
        default="markdown",
        description="Content Format (immer 'markdown')"
    )
    
    citations: List[OfficeCitation] = Field(
        default_factory=list,
        description="Quellen-Liste"
    )
...
class OfficeResponseError(BaseModel):
    ...
    retry_after_ms: Optional[int] = Field(
        None,
        description="Retry After (Millisekunden)"
    )
...
class OfficeAPIResponse(BaseModel):
    ...
    status: str = Field(
        ...,
        description="Status: success|error|partial"
    )
    
    metadata: OfficeResponseMetadata = Field(
        default_factory=OfficeResponseMetadata,
        description="Response Metadata"
    )
    
    content: Optional[OfficeResponseContent] = Field(
        None,
        description="Response Content (nur bei success/partial)"
    )
    
    error: Optional[OfficeResponseError] = Field(
        None,
        description="Error Info (nur bei error)"
    )
...
def map_unified_to_office_response(
    ...
    for src in unified_response.sources:
        citations.append(OfficeCitation(
            document_id=src.id,
