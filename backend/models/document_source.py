...
class RelevanceScore:
    ...
    def __post_init__(self):
        ...
        for score_name in ["semantic", "keyword", "graph", "hybrid"]:
            ...
            if not (0.0 <= score <= 1.0):
                raise ValueError(f"{score_name} score must be in range [0.0, 1.0], got {score}")

    def calculate_hybrid(self, semantic_weight: float = 0.5, keyword_weight: float = 0.3, graph_weight: float = 0.2) -> float:
        """
        ...
        self.hybrid = (
            self.semantic * semantic_weight + self.keyword * keyword_weight + self.graph * graph_weight
        ) / total_weight
    ...
    def to_dict(self) -> Dict[str, float]:
        ...
        return {"semantic": self.semantic, "keyword": self.keyword, "graph": self.graph, "hybrid": self.hybrid}
...
class DocumentSource:
    ...
    def to_citation(self) -> "SourceCitation":
        ...
        """
        return SourceCitation(source=self, confidence=self.get_confidence(), excerpt=self.get_excerpt(max_length=150))
...
class SearchResult:
    ...
    def get_top_k(self, k: int = 5) -> List[DocumentSource]:
        ...
        return sorted(self.documents, key=lambda d: d.relevance_score.hybrid, reverse=True)[:k]

    def filter_by_confidence(self, min_confidence: CitationConfidence = CitationConfidence.MEDIUM) -> List[DocumentSource]:
        ...
        return [doc for doc in self.documents if confidence_values[doc.get_confidence()] >= min_value]
    ...
    def to_dict(self) -> Dict[str, Any]:
        """Convert to dictionary"""
        return {"documents": [doc.to_dict() for doc in self.documents], "metadata": self.metadata.to_dict()}
    ...
    def from_dict(cls, data: Dict[str, Any]) -> "SearchResult":
        ...
        documents = [DocumentSource.from_dict(doc_data) for doc_data in data["documents"]]
...
def create_mock_document(document_id: str, title: str, content: str, relevance: float = 0.8) -> DocumentSource:
    """
...
if __name__ == "__main__":
    ...
    hybrid = score.calculate_hybrid(semantic_weight=0.5, keyword_weight=0.3, graph_weight=0.2)
    ...
    citation = SourceCitation(
        source=doc, confidence=CitationConfidence.HIGH, page_number=42, excerpt="Ein Bauantrag in Stuttgart erfordert..."
    )
    ...
    metadata = SearchResultMetadata(
        query="Bauantrag Stuttgart", search_method="hybrid", total_documents=2, filtered_count=2, execution_time_ms=125.5
    )

    result = SearchResult(documents=[doc, doc2], metadata=metadata)
