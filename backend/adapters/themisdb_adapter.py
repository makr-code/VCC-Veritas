...
class ThemisDBAdapter:
    ...
    def __init__(self, config: Optional[ThemisDBConfig] = None):
        ...
        self.base_url = f"{'https' if self.config.use_ssl else 'http'}://" f"{self.config.host}:{self.config.port}"
    ...
    async def vector_search(
        self, query: str, top_k: int = 5, collection: str = "documents", threshold: float = 0.0, **kwargs
    ) -> List[Dict[str, Any]]:
        """
        ...
        try:
            ...
            if documents:
                ...
                logger.debug(f"✅ ThemisDB Vector Search: {len(documents)} docs, " f"{latency_ms:.1f}ms, query: {query[:50]}")
            else:
                self._stats["empty_results"] += 1
                logger.debug(f"ℹ️ ThemisDB Vector Search: 0 results, {latency_ms:.1f}ms")
    ...
    async def execute_aql(self, query: str, bind_vars: Optional[Dict[str, Any]] = None) -> List[Dict[str, Any]]:
        """
        ...
        """
        try:
            response = await self.client.post("/api/aql/query", json={"query": query, "bindVars": bind_vars or {}})
            response.raise_for_status()
    ...
    async def insert_document(self, collection: str, document: Dict[str, Any], key: Optional[str] = None) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            response = await self.client.post(f"/api/document/{collection}", json=payload)
            response.raise_for_status()
            return response.json()
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        return {
            **self._stats,
            "avg_latency_ms": (
                self._stats["total_latency_ms"] / self._stats["total_queries"] if self._stats["total_queries"] > 0 else 0.0
            ),
            "success_rate": (
                self._stats["successful_queries"] / self._stats["total_queries"] if self._stats["total_queries"] > 0 else 0.0
            ),
        }
