...
class ReRankingService:
    ...
    def _load_model(self) -> None:
        ...
        self.model = CrossEncoder(self.config.model_name, max_length=self.config.max_length)
    ...
    async def rerank_documents(
        self, query: str, documents: List[Dict[str, Any]], top_k: Optional[int] = None, return_scores: bool = True
    ) -> List[Dict[str, Any]]:
        """
        ...
        try:
            ...
            if self.config.score_threshold is not None:
                doc_score_pairs = [(doc, score) for doc, score in doc_score_pairs if score >= self.config.score_threshold]
            ...
            logger.info(f"✅ Re-Ranking abgeschlossen: {len(documents)} → {len(top_docs)} Dokumente " f"({duration_ms:.1f}ms)")
    ...
    def _compute_relevance_scores(self, query: str, documents: List[Dict[str, Any]]) -> List[float]:
        """
    ...
    def _get_cache_key(self, query: str, doc_ids: List[str]) -> str:
        ...
        # Use SHA-256 for cache keys (avoid weak MD5 usage flagged by Bandit B324)
        return hashlib.sha256(key_str.encode()).hexdigest()

    def _apply_cached_ranking(
        self, documents: List[Dict[str, Any]], cached_scores: List[Tuple[str, float]], top_k: int
    ) -> List[Dict[str, Any]]:
...
def get_reranking_service(config: Optional[ReRankingConfig] = None, force_reload: bool = False) -> ReRankingService:
    """
...
async def rerank_documents_simple(query: str, documents: List[Dict[str, Any]], top_k: int = 5) -> List[Dict[str, Any]]:
    """
