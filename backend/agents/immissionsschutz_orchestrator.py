...
class ImmissionsschutzOrchestrator:
    ...
    def _finalize_workflow(self, workflow: WorkflowResult):
        ...
        self.logger.info(
            f"Workflow {workflow.workflow_id} finalized: {workflow.status.value} "
            f"({workflow.success_rate:.1f}% success)"
        )
    ...
    async def comprehensive_analysis(
        ...
        try:
            # Step 1: Basis-Daten
            entity_result = await self._execute_step(
                workflow, 0,
                self.db_agent.get_complete_entity,
                bst_nr, anl_nr, True
            )
            ...
            compliance_report = await self._execute_step(
                workflow, 1,
                self.immi_agent.generate_compliance_report,
                bst_nr, anl_nr
            )
            
            # Step 3: Risiko-Analyse
            risiko_analyse = await self._execute_step(
                workflow, 2,
                self.immi_agent.calculate_risk_score,
                bst_nr, anl_nr
            )
            
            # Step 4: Grenzwerte
            grenzwert_status = await self._execute_step(
                workflow, 3,
                self.immi_agent.check_grenzwerte,
                bst_nr, anl_nr
            )
            ...
            workflow.result_data = {
                ...
                "grenzwert_ueberschreitungen": len([
                    p for kategorie in (grenzwert_status or {}).values() 
                    for p in kategorie if p.ist_kritisch
                ]) if grenzwert_status else 0
            }
    ...
    async def _aggregate_comprehensive_analysis(
        ...
        if risiko_analyse:
            ...
            bewertungs_teile.append(
                f"{risiko_emoji} Risiko: {risiko_analyse.risiko_klasse.value} "
                f"({risiko_analyse.risiko_score:.0%})"
            )
        ...
        if grenzwert_status:
            ...
            if kritische:
                ...
                for pruefung in kritische[:2]:
                    handlungsempfehlungen.append(
                        f"Grenzwertüberschreitung {pruefung.messart} beheben"
                    )
    ...
    async def compliance_workflow(
        ...
        try:
            ...
            base_compliance = await self._execute_step(
                workflow, 0,
                self.db_agent.analyze_compliance,
                bst_nr, anl_nr
            )
            ...
            auflagen_task = self._execute_step(
                workflow, 2,
                self.db_agent.check_auflagen_status,
                bst_nr, anl_nr
            )
            ...
            result = await self._execute_step(
                workflow, 4,
                self._aggregate_compliance_workflow,
                base_compliance,
                verfahren,
                auflagen,
                maengel
    ...
    async def maintenance_planning(
        ...
        try:
            ...
            ueberwachung_result = await self._execute_step(
                workflow, 3,
                self.db_agent.query_entity,
                EntityType.UEBERWACHUNG,
                {"bst_nr": bst_nr, "anl_nr": anl_nr},
                50
            )
    ...
    async def emission_monitoring(
        ...
        try:
            # Step 1: Messungen
            messungen = await self._execute_step(
                workflow, 0,
                self.db_agent.query_messungen,
                bst_nr, anl_nr, None, None, 200
            )
            
            # Step 2: Grenzwerte
            grenzwerte = await self._execute_step(
                workflow, 1,
                self.immi_agent.check_grenzwerte,
                bst_nr, anl_nr
            )
            ...
            for messart, _ in top_messarten:
                task = self._execute_step(
                    workflow, 2,
                    self.immi_agent.analyze_trend,
                    bst_nr, anl_nr, messart, 90
                )
            ...
            result = await self._execute_step(
                workflow, 4,
                self._create_emission_monitoring_report,
                messungen,
                grenzwerte,
                trends,
                messreihen_result
            ...
            if result:
                workflow.result_data = result
                workflow.summary = f"{result.get('messungen_total', 0)} Messungen, {result.get('ueberschreitungen', 0)} Überschreitungen"
                workflow.recommendations = result.get('empfehlungen', [])[:5]
    ...
    async def _create_emission_monitoring_report(
        self,
        messungen,
        grenzwerte,
        trends,
