...
class PipelineFactory:
    ...
    async def create_pipeline(self, max_workers: int = 5, enable_rag: bool = True, enable_supervisor: bool = False):
        """
        ...
        await pipeline._initialize_request_scoped_resources(enable_rag=enable_rag, enable_supervisor=enable_supervisor)

        # Factory Stats aktualisieren
        self.pipelines_created += 1
        self.pipelines_active += 1

        logger.info(f"✅ Pipeline-Instanz erstellt (#{self.pipelines_created}, " f"aktiv: {self.pipelines_active})")
...
def create_pipeline_factory(ollama_client, uds3_strategy, agent_registry, progress_manager=None) -> PipelineFactory:
    """
...
def get_pipeline_factory() -> Optional[PipelineFactory]:
    ...
    if _pipeline_factory is None:
        raise RuntimeError("Pipeline Factory nicht initialisiert! " "Rufe zuerst create_pipeline_factory() auf.")
...
async def create_request_scoped_pipeline(max_workers: int = 5, enable_rag: bool = True, enable_supervisor: bool = False):
    """
    ...
    return await factory.create_pipeline(max_workers=max_workers, enable_rag=enable_rag, enable_supervisor=enable_supervisor)
