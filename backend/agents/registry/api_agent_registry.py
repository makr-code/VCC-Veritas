...
try:
    ...
    class UDS3PolyglotManager:
        ...
        def __init__(self, *args, **kwargs):
            logger.warning("⚠️ UDS3PolyglotManager not available - using mock")
            pass
...
class AgentCapability(Enum):
    ...
    # Visualization & Generation (NEW - 2025-12-04)
    CHART_GENERATION = "chart_generation"  # Chart/Graph Erstellung
    PRESENTATION_CREATION = "presentation_creation"  # Präsentationen (VDL)
    IMAGE_GENERATION = "image_generation"  # AI Bildgenerierung
    VISUAL_DESIGN = "visual_design"  # Visuelles Design
...
class SharedResourcePool:
        ...
        with self._lock:
            if not self._database_api and _database_api_available:
                try:
                    ...
                    self._uds3_strategy = UDS3PolyglotManager(backend_config=backend_config, enable_rag=True)
    ...
    def cache_external_api_result(self, cache_key: str, result: Any, ttl: int = 300):
        ...
        with self._lock:
            self._external_api_cache[cache_key] = {"result": result, "cached_at": time.time(), "ttl": ttl}
...
class AgentRegistry:
    ...
    def register_agent(
        ...
        with self._lock:
            ...
            logger.info(
                f"✅ Agent registriert: {agent_type} (Cap: {max_concurrent_instances}, Capabilities: {len(capabilities)})"
            )
            return True
    ...
    def get_agent_instance(self, agent_type: str, query_id: str = None) -> Optional[Any]:
        ...
        with self._lock:
            ...
            # Cap-Check
            active_count = len(
                [inst for inst in self._active_instances.values() if inst.agent_registration.agent_type == agent_type]
            )
...
def register_agent_type(agent_type: str, agent_class: Type, capabilities: Set[AgentCapability], **kwargs) -> bool:
    """
