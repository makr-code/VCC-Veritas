...
class AgentCoordinator:
    ...
    def start(self):
        ...
        self.coordinator_thread = threading.Thread(target=self._coordinator_loop, name="AgentCoordinator-Main", daemon=True)
        self.coordinator_thread.start()

        # Starte Monitor-Thread
        self.monitor_thread = threading.Thread(target=self._monitor_loop, name="AgentCoordinator-Monitor", daemon=True)
    ...
    def submit_query(self, query_data: Dict[str, Any], priority: int = 1) -> str:
        ...
        self.gui_adapter.send_agent_update(
            AgentMessageType.QUERY_START, "coordinator", {"query_id": query_id, "query_data": query_data}
        )
    ...
    def _process_query(self, query_item: Dict[str, Any]):
        ...
        try:
            ...
            # GUI Update
            self.gui_adapter.send_agent_update(
                AgentMessageType.QUERY_COMPLETE, "coordinator", {"query_id": query_id, "result": final_result}
            )
            ...
            # Fehler-Ergebnis
            error_result = {"query_id": query_id, "error": str(e), "processing_time": time.time() - query_item["submitted_at"]}
            ...
            # GUI Update
            self.gui_adapter.send_agent_update(
                AgentMessageType.QUERY_ERROR, "coordinator", {"query_id": query_id, "error": str(e)}
            )
    ...
    def _execute_agent(self, agent_type: str, query_data: Dict[str, Any], query_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            # GUI Update
            self.gui_adapter.send_agent_update(
                AgentMessageType.AGENT_SPAWNED, agent_id, {"agent_type": agent_type, "query_id": query_id}
            )
            ...
            return {"agent_id": agent_id, "agent_type": agent_type, "error": str(e), "confidence_score": 0.0}
            ...
            def cleanup_agent():
                ...
                self.gui_adapter.send_agent_update(AgentMessageType.AGENT_TERMINATED, agent_id, {"agent_type": agent_type})
    ...
    def _terminate_idle_agents(self, agent_type: str, count: int):
        ...
        with self.agent_lock:
            for agent_id, agent_info in list(self.active_agents.items()):
                if agent_info["agent_type"] == agent_type and agent_info["status"] == "idle" and terminated < count:
