...
- Layer 2: UDS3 Search API (uds3.search) - High-level search interface ✅
...
Usage (NEW - RECOMMENDED ⭐):
    ...
    # Agent uses strategy.search_api internally (no manual instantiation)
...
Note: Agent accesses Search API via strategy.search_api property (lazy-loaded)
      No manual UDS3SearchAPI() instantiation required anymore!
"""
...
# Import UDS3 Search API types (for type hints only)
try:
    from uds3.search import SearchQuery
    from uds3.search import SearchResult as UDS3SearchResult
    from uds3.search import SearchType
...
class UDS3HybridSearchAgent:
    ...
    def __init__(self, strategy):
        ...
        Note: Accesses Search API via strategy.search_api property (NEW ⭐)
              No manual UDS3SearchAPI() instantiation required!
        """
        self.strategy = strategy

        # Check if UDS3 Search API is available
        if not UDS3_SEARCH_API_AVAILABLE:
            logger.error("❌ UDS3 Search API not available - install uds3.search module")
            raise ImportError("UDS3 Search API required (uds3.search)")

        # Access Search API via property (lazy-loaded by strategy) ✅
        # OLD: self.search_api = UDS3SearchAPI(strategy)
        # NEW: Use strategy.search_api property
        self.search_api = strategy.search_api

        logger.info("✅ UDS3HybridSearchAgent initialized (using strategy.search_api property)")
