...
try:
    from .veritas_api_agent_chemical_data import ChemicalDataAgent, ChemicalSubstance
...
class StandardsSearchResponse:
    ...
    def get_mandatory_standards(self) -> List[TechnicalStandard]:
        ...
        return [
            std for std in self.standards if any(req.compliance_level == ComplianceLevel.MANDATORY for req in std.requirements)
        ]
...
class TechnicalStandardsAgent:
    ...
    def __init__(self, config: TechnicalStandardsConfig = None):
        ...
        self.logger.info(
            f"✅ Technical Standards Agent initialized with {len(self.config.enabled_organizations)} organizations"
        )
    ...
    async def _search_by_organization(
        self, org: StandardsOrganization, search_term: str, max_results: int
    ...
    async def _assess_compliance(self, standard: TechnicalStandard, product_scope: str) -> ComplianceAssessment:
        ...
        assessment = ComplianceAssessment(
            ...
            priority_actions=["Sofortige Behebung sicherheitskritischer Mängel"],
...
async def main():
    ...
    # Agent erstellen
    config = TechnicalStandardsConfig(cache_enabled=True, min_relevance_threshold=0.4)
