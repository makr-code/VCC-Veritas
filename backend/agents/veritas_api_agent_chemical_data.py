...
try:
    from .veritas_api_agent_atmospheric_flow import AtmosphericFlowAgent, EmissionSource, FlowModelType
...
class ChemicalDataConfig:
    ...
    enabled_databases: List[str] = field(
        default_factory=lambda: ["pubchem", "chemspider", "echa_chem", "gestis", "nist_webbook"]
    )
    ...
    # SDS-Konfiguration
    sds_providers: List[str] = field(default_factory=lambda: ["sigma_aldrich", "merck", "fisher_scientific", "vwr"])
...
class ChemicalDataAgent:
    ...
    async def _search_by_cas(self, cas_number: str) -> Optional[ChemicalSubstance]:
        ...
        mock_cas_data = {
            ...
            "71-43-2": {"name": "Benzol", "formula": "C6H6", "mw": 78.114, "state": PhysicalState.LIQUID},  # Benzol
            "108-88-3": {"name": "Toluol", "formula": "C7H8", "mw": 92.140, "state": PhysicalState.LIQUID},  # Toluol
            "67-64-1": {"name": "Aceton", "formula": "C3H6O", "mw": 58.080, "state": PhysicalState.LIQUID},  # Aceton
            "64-17-5": {"name": "Ethanol", "formula": "C2H6O", "mw": 46.069, "state": PhysicalState.LIQUID},  # Ethanol
        }
        ...
        if cas_number in mock_cas_data:
            ...
            substance.identifiers.append(
                ChemicalIdentifier(
                    identifier_type=ChemicalIdentifierType.CAS_NUMBER, value=cas_number, verified=True, source="mock_database"
                )
            )
    ...
    async def _search_by_structure(
        self, structure: str, identifier_type: ChemicalIdentifierType
    ) -> Optional[ChemicalSubstance]:
        ...
        if cas in property_data:
            for prop_name, (value, unit, temp) in property_data[cas].items():
                substance.physical_properties.append(
                    PhysicalProperty(
                        property_name=prop_name, value=value, unit=unit, temperature_c=temp, source="mock_database"
                    )
                )
    ...
    async def _add_mock_ghs_classification(self, substance: ChemicalSubstance):
        ...
        ghs_data = {
            ...
            "71 - 43-2": [  # Benzol
                ...
                (
                    GHSHazardClass.SPECIFIC_TARGET_ORGAN_TOXICITY,
                    "1",
                    "H372",
                    "Schädigt Organe bei längerer oder wiederholter Exposition",
                ),
            ],
        }
    ...
    async def _get_safety_data_sheet(self, substance: ChemicalSubstance) -> Optional[SafetyDataSheet]:
        ...
        sds.section_9_physical_chemical = {
            prop.property_name: f"{prop.value} {prop.unit}" for prop in substance.physical_properties
        }
...
async def main():
    ...
    # Agent erstellen
    config = ChemicalDataConfig(cache_enabled=True, min_quality_threshold=0.5)
