...
class UDS3VectorSearchAdapter:
    ...
    async def vector_search(self, query: str, top_k: int = 5, threshold: float = 0.0, **kwargs) -> List[Dict[str, Any]]:
        """
        ...
        try:
            ...
            result = self.uds3.query_across_databases(
                vector_params={"query_text": query, "top_k": top_k, "threshold": threshold},
                graph_params=None,
            ...
            if documents:
                ...
                logger.debug(f"✅ Vector Search: {len(documents)} docs, " f"{latency_ms:.1f}ms, query: {query[:50]}")
            else:
                self._stats["empty_results"] += 1
                logger.debug(f"ℹ️ Vector Search: 0 results, {latency_ms:.1f}ms, query: {query[:50]}")
    ...
    def _extract_field(self, obj: Any, field_names: List[str], default: Any = None) -> Any:
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        avg_latency = self._stats["total_latency_ms"] / total if total > 0 else 0.0

        return {
            **self._stats,
            "avg_latency_ms": avg_latency,
            "success_rate": (self._stats["successful_queries"] / total if total > 0 else 0.0),
        }
    ...
    async def vector_search(self, query: str, top_k: int = 5, threshold: float = 0.0, **kwargs) -> List[Dict[str, Any]]:
        ...
        if self._call_count == 1:
            logger.warning(
                ...
                "- gibt leere Ergebnisse zurück (UDS3 nicht verfügbar)"
            )
