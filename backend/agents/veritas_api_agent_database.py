    ...
    def detect_operation(self, sql_query: str) -> SQLOperation:
        ...
        if query_upper.startswith("WITH"):
            ...
            if "SELECT" in query_upper:
                ...
                self.logger.warning("🚫 WITH clause without SELECT detected")
                return SQLOperation.BLOCKED
...
class DatabaseAgent:
    ...
    async def execute_query(self, request: DatabaseQueryRequest) -> DatabaseQueryResponse:
        ...
        if not is_valid:
            ...
            if self.config.log_blocked_queries:
                self.logger.warning(f"🚫 Blocked Query: {request.sql_query[:100]} | Reason: {error_msg}")
        ...
        try:
            ...
            self.stats["avg_query_time_ms"] = self.stats["total_query_time_ms"] / self.stats["total_queries"]
            ...
            if query_time_ms > self.config.log_slow_queries_ms:
                ...
                self.logger.warning(f"🐌 Slow query detected: {query_time_ms}ms | " f"Query: {request.sql_query[:100]}")

            # Log All Queries (optional)
            if self.config.log_all_queries:
                self.logger.info(
                    f"📊 Query executed: {request.query_id} | " f"Time: {query_time_ms}ms | Rows: {response.row_count}"
                )
    ...
    async def _execute_sqlite_query(self, request: DatabaseQueryRequest, operation: SQLOperation) -> DatabaseQueryResponse:
        """
    ...
    async def _get_cached_response(self, request: DatabaseQueryRequest) -> Optional[DatabaseQueryResponse]:
        """
    ...
    async def _cache_response(self, request: DatabaseQueryRequest, response: DatabaseQueryResponse):
        """
        Cached Response
...
async def main():
    ...
    # Create Agent
    config = DatabaseConfig(max_results=10, enable_query_cache=True, log_all_queries=True)
    agent = create_database_agent(config)

    # Test Queries
    test_queries = [
        {"sql": "SELECT * FROM users", "description": "Select all users", "should_succeed": True},
        {"sql": 'SELECT * FROM users WHERE status = "active"', "description": "Select active users", "should_succeed": True},
        {
            "sql": "SELECT COUNT( * ) as count FROM users GROUP BY status",
            "description": "Aggregate query",
            "should_succeed": True,
        },
        {"sql": "PRAGMA table_info(users)", "description": "Get table schema", "should_succeed": True},
        {
            "sql": 'INSERT INTO users (username) VALUES ("hacker")',
            "description": "INSERT (should be blocked)",
            "should_succeed": False,
        },
        {"sql": 'UPDATE users SET status = "admin"', "description": "UPDATE (should be blocked)", "should_succeed": False},
        {"sql": "DELETE FROM users WHERE id = 1", "description": "DELETE (should be blocked)", "should_succeed": False},
        {"sql": "DROP TABLE users", "description": "DROP TABLE (should be blocked)", "should_succeed": False},
    ]
