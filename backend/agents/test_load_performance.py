...
from framework.agent_monitoring import AgentMonitor, MonitoringService

# Framework imports
from framework.base_agent import BaseAgent
from framework.orchestration_controller import OrchestrationController
from framework.quality_gate import QualityDecision, QualityGate, QualityPolicy
...
class PerformanceTester:
    ...
    async def test_concurrent_plans(self, num_plans: int) -> PerformanceMetrics:
        ...
        for i in range(num_plans):
            plan = ExecutionPlan(plan_id=f"load_test_{i}", goal=f"Load test plan {i}", agent_id=agent.agent_id)
            # Add 5 steps per plan
            for j in range(5):
                plan.add_step(
                    ExecutionStep(step_id=f"step_{i}_{j}", description=f"Step {j} of plan {i}", agent_id=agent.agent_id)
                )
            plans.append(plan)
    ...
    async def test_memory_leaks(self, iterations: int) -> PerformanceMetrics:
        ...
        for i in range(iterations):
            plan = ExecutionPlan(plan_id=f"mem_test_{i}", goal=f"Memory test {i}", agent_id=agent.agent_id)
            plan.add_step(ExecutionStep(step_id=f"step_{i}", description=f"Test step {i}", agent_id=agent.agent_id))
    ...
    async def test_quality_gate_performance(self, num_validations: int) -> PerformanceMetrics:
        ...
        policy = QualityPolicy(min_quality_score=0.7, target_quality_score=0.9, require_review_below=0.8)
    ...
    async def test_orchestration_performance(self, num_plans: int) -> PerformanceMetrics:
        ...
        async def execute_with_interventions(plan_id: str) -> Tuple[float, bool]:
            ...
            plan_start = time.time()
            try:
                plan = ExecutionPlan(plan_id=plan_id, goal=f"Orchestration test {plan_id}", agent_id=agent.agent_id)
                for j in range(3):
                    plan.add_step(
                        ExecutionStep(step_id=f"step_{plan_id}_{j}", description=f"Step {j}", agent_id=agent.agent_id)
                    )
                ...
                execution_task = asyncio.create_task(controller.execute_plan(plan.plan_id))
                ...
                if len(plan.steps) > 1:
                    await controller.skip_step(plan.plan_id, plan.steps[1].step_id, "test_user", "Performance test skip")
        ...
        for i in range(0, num_plans, batch_size):
            batch = [execute_with_interventions(f"orch_test_{j}") for j in range(i, min(i + batch_size, num_plans))]
    ...
    async def test_sustained_load(self, plans_per_minute: int, duration_minutes: int) -> PerformanceMetrics:
        ...
        async def execute_plan_task(plan_id: str) -> Tuple[float, bool]:
            ...
            plan_start = time.time()
            try:
                plan = ExecutionPlan(plan_id=plan_id, goal=f"Sustained load test {plan_id}", agent_id=agent.agent_id)
                for j in range(3):
                    plan.add_step(
                        ExecutionStep(step_id=f"step_{plan_id}_{j}", description=f"Step {j}", agent_id=agent.agent_id)
                    )
        ...
        while time.time() < end_time:
            ...
            if total_plans % (plans_per_minute // 2) == 0:
                ...
                print(f"  Progress: {elapsed:.1f}/{duration_minutes} min, " f"Plans: {total_plans}, Errors: {errors}")
    ...
    async def _create_test_agent(self) -> BaseAgent:
        ...
        class LoadTestAgent(BaseAgent):
            ...
            async def execute_step(self, step: Dict[str, Any], context: Dict[str, Any]) -> Dict[str, Any]:
                ...
                return {"status": "success", "data": {"result": "test_data"}, "quality_score": 0.85}
