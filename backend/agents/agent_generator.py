...
class AgentGenerator:
    ...
    def generate_agent(self, domain: str, capabilities: List[str] = None, custom_config: Dict[str, Any] = None) -> str:
        """
        ...
        agent_content = self._replace_template_placeholders(template_content, domain, capabilities, custom_config)
    ...
    def _replace_template_placeholders(
        self, content: str, domain: str, capabilities: List[str] = None, custom_config: Dict[str, Any] = None
    ) -> str:
...
'''
    ...
    def _generate_documentation(self, domain: str, capabilities: List[str] = None, custom_config: Dict[str, Any] = None):
        ...
        doc_content = """# {domain.capitalize()} Agent
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="VERITAS Agent Generator", epilog=f"Verfügbare Domains: {', '.join(AVAILABLE_DOMAINS)}"
    )

    parser.add_argument(
        "--domain", required=True, choices=AVAILABLE_DOMAINS, help="Agent Domain (z.B. environmental, financial)"
    )

    parser.add_argument(
        "--capabilities", help="Komma-getrennte Liste von Capabilities (z.B. DATA_ANALYSIS,EXTERNAL_API_INTEGRATION)"
    )

    parser.add_argument("--output-dir", default=".", help="Output-Verzeichnis für generierte Dateien")

    parser.add_argument("--config", help="JSON-String mit zusätzlicher Konfiguration")
    ...
    try:
        ...
        agent_path = generator.generate_agent(domain=args.domain, capabilities=capabilities, custom_config=custom_config)
        ...
        print("   3. Integriere in Agent Registry")
