...
GRENZWERTE_LUFT = {
    ...
    "NOx": {"jahresmittel": 40.0, "typ": GrenzwertTyp.LUFT_JAHRESMITTEL},
    "NO2": {
        "jahresmittel": 40.0,
        "stundenmittel": 200.0,
    ...
    "PM2.5": {"jahresmittel": 25.0, "typ": GrenzwertTyp.LUFT_JAHRESMITTEL},
    # Schwefeldioxid
    "SO2": {
        "jahresmittel": 50.0,
        "tagesmittel": 125.0,
    ...
    # Kohlenmonoxid
    "CO": {"8h_mittel": 10000.0, "typ": GrenzwertTyp.LUFT_JAHRESMITTEL},
    # Ozon
    "O3": {"8h_mittel": 120.0, "typ": GrenzwertTyp.LUFT_KURZZEITWERT},
}

GRENZWERTE_LAERM = {
    # TA Lärm - Immissionsrichtwerte in dB(A)
    "Industriegebiet": {"tag": 70.0, "nacht": 70.0, "typ_tag": GrenzwertTyp.LAERM_TAG, "typ_nacht": GrenzwertTyp.LAERM_NACHT},
    "Gewerbegebiet": {"tag": 65.0, "nacht": 50.0, "typ_tag": GrenzwertTyp.LAERM_TAG, "typ_nacht": GrenzwertTyp.LAERM_NACHT},
    "Mischgebiet": {"tag": 60.0, "nacht": 45.0, "typ_tag": GrenzwertTyp.LAERM_TAG, "typ_nacht": GrenzwertTyp.LAERM_NACHT},
    "Wohngebiet": {"tag": 55.0, "nacht": 40.0, "typ_tag": GrenzwertTyp.LAERM_TAG, "typ_nacht": GrenzwertTyp.LAERM_NACHT},
    "Kurgebiet": {"tag": 45.0, "nacht": 35.0, "typ_tag": GrenzwertTyp.LAERM_TAG, "typ_nacht": GrenzwertTyp.LAERM_NACHT},
}
...
class ImmissionsschutzAgentTestServerExtension(DatabaseAgentTestServerExtension):
    ...
    async def check_grenzwerte(
        self, bst_nr: str, anl_nr: str, messart: Optional[str] = None
        ...
        # Messungen abrufen
        messungen = await self.query_messungen(bst_nr=bst_nr, anl_nr=anl_nr, messart=messart, limit=1000)
        ...
        return {"luft": pruefungen_luft, "laerm": pruefungen_laerm}

    def _pruefe_luftgrenzwert(self, messart: str, werte: List[float]) -> Optional[GrenzwertPruefung]:
    ...
    def _pruefe_laermgrenzwert(self, messart: str, werte: List[float]) -> Optional[GrenzwertPruefung]:
    ...
    async def analyze_trend(self, bst_nr: str, anl_nr: str, messart: str, zeitraum_tage: int = 90) -> TrendAnalyse:
        """
        ...
        result = await self.query_entity(
            EntityType.MESSREIHE, filters={"bst_nr": bst_nr, "anl_nr": anl_nr, "messart": messart}, limit=10
        )

        if not result.success or not result.data:
            self.logger.warning(f"Keine Messreihen gefunden für {messart}")
            # Fallback: Einzelmessungen auswerten
            return await self._analyze_trend_from_messungen(bst_nr, anl_nr, messart, zeitraum_tage)
    ...
    async def _analyze_trend_from_messungen(self, bst_nr: str, anl_nr: str, messart: str, zeitraum_tage: int) -> TrendAnalyse:
    ...
    async def generate_compliance_report(self, bst_nr: str, anl_nr: str) -> ComplianceReport:
        """
        ...
        maengel_kritisch = sum(
            1 for m in anlage.maengel if getattr(m, "status", None) == "offen" and getattr(m, "kategorie", None) == "kritisch"
        )
        ...
        zusammenfassung = self._generate_zusammenfassung(
            base_compliance, alle_pruefungen, trend_analysen, maengel_kritisch, wartungen_geplant
        )
    ...
    def _generate_zusammenfassung(
        ...
        if kritische_pruefungen:
            parts.append(f"🔴 {len(kritische_pruefungen)} Grenzwertüberschreitungen erkannt")
        else:
    ...
    async def calculate_risk_score(self, bst_nr: str, anl_nr: str) -> RisikoAnalyse:
        """
...
def get_immissionsschutz_agent(config: Optional[TestServerConfig] = None) -> ImmissionsschutzAgentTestServerExtension:
