...
class QualityGate:
    ...
    def __init__(self, policy: QualityPolicy, review_callback: Optional[Callable[[ReviewRequest], ReviewStatus]] = None):
        ...
        logger.info("Initialized QualityGate with policy: " f"min={policy.min_quality}, target={policy.target_quality}")
    ...
    def _create_dimension_failure_result(self, quality_score: float, dimension_check: Dict[str, Any]) -> GateResult:
        ...
        return GateResult(
            ...
            quality_score=quality_score,
            meets_threshold=False,
            reasons=["Quality dimension check failed", f"Failed dimensions: {', '.join(failed_dimensions)}"],
