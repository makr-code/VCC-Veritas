...
try:
    ...
    from .dependency_resolver import DependencyError, DependencyResolver
    from .quality_gate import GateDecision, QualityGate, QualityPolicy
    ...
    from .state_machine import PlanState, StateMachine, StateTransition, StateTransitionError
except ImportError:
    from agent_monitoring import AgentMonitor
    from dependency_resolver import DependencyError, DependencyResolver
    from quality_gate import GateDecision, QualityGate, QualityPolicy
...
class BaseAgent(ABC):
    ...
    def __init__(
        ...
        if enable_monitoring:
            self.monitor = AgentMonitor(agent_id=self.agent_id, agent_type=self.get_agent_type())
    ...
    def get_agent_type(self) -> str:
        ...
        Example:
            ...
            ...     return "environmental"
        """
        raise NotImplementedError(f"{self.__class__.__name__} must implement get_agent_type()")
    ...
    def get_capabilities(self) -> List[str]:
        ...
        Example:
            ...
            ...     ]
        """
        raise NotImplementedError(f"{self.__class__.__name__} must implement get_capabilities()")
    ...
    def execute(
        ...
        state_machine = StateMachine(plan_id, on_state_change=lambda t: self._on_state_change(t))
        ...
        logger.info(f"Executing plan {plan_id}: {len(steps_to_execute)} steps " f"(range: {start_idx + 1}-{end_idx})")
        ...
        try:
            state_machine.transition_to(PlanState.RUNNING, "Execution started", metadata={"plan_id": plan_id})
        ...
        if parallel and len(steps_to_execute) > 1:
            execution_result = self._execute_parallel(plan_id, steps_to_execute, context, max_workers)
            execution_mode = "parallel"
        else:
            execution_result = self._execute_sequential(plan_id, steps_to_execute, context)
    ...
    def _execute_sequential(self, plan_id: str, steps: List[Dict[str, Any]], context: Dict[str, Any]) -> Dict[str, Any]:
        """
        ...
        for step in steps:
            ...
            try:
                ...
                result = retry_handler.execute_with_retry(
                    func=self._execute_step_internal, step_id=step_id, max_retries=max_retries, context=context, step=step
                )
                ...
                if result.get("status") == "success":
                    ...
                    failed += 1
                    errors_list.append(f"Step {step_number} ({step_id}): {result.get('error', 'Unknown error')}")
    ...
    def _execute_parallel(
        self, plan_id: str, steps: List[Dict[str, Any]], context: Dict[str, Any], max_workers: int = 4
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            logger.info(f"Parallel execution: {len(execution_plan)} groups, " f"max parallelism: {max_parallelism}")
        ...
        for group_index, step_ids in enumerate(execution_plan, 1):
            logger.info(f"Executing group {group_index}/{len(execution_plan)}: " f"{len(step_ids)} steps in parallel")
            ...
            with ThreadPoolExecutor(max_workers=min(max_workers, len(step_ids))) as executor:
                ...
                for step_id in step_ids:
                    ...
                    future = executor.submit(self._execute_single_step, plan_id, step, context)
                ...
                for future in as_completed(future_to_step):
                    ...
                    try:
                        ...
                        step_number = step.get("step_index", 0) + 1
                        errors_list.append(f"Step {step_number} ({step_id}): {str(e)}")
                        results[step_id] = {"status": "failed", "error": str(e), "quality_score": 0.0}
    ...
    def _execute_single_step(self, plan_id: str, step: Dict[str, Any], context: Dict[str, Any]) -> Dict[str, Any]:
        """
        ...
        # Create retry handler with step-specific config
        retry_config = RetryConfig(
            max_retries=max_retries, base_delay=1.0, backoff_factor=2.0, strategy=RetryStrategy.EXPONENTIAL, jitter=True
        )
        retry_handler = RetryHandler(config=retry_config)
    ...
    def _execute_step_internal(self, context: Dict[str, Any], step: Dict[str, Any]) -> Dict[str, Any]:
        """
    ...
    def _store_step_result(self, plan_id: str, step: Dict[str, Any], result: Dict[str, Any]) -> None:
        ...
        try:
            ...
            conn.commit()
            logger.debug(f"Stored result for step: {step['step_id']} " f"(status: {db_status}, retries: {retry_count})")
        finally:
            conn.close()

    def _update_plan_status(self, plan_id: str, status: str, quality_score: float, execution_time_ms: int) -> None:
...
if __name__ == "__main__":
    ...
    plan2 = create_minimal_plan(
        plan_id=f"test_parallel_{test_id_2}", research_question="Test parallel execution", schema_name="standard"
    )
    ...
    with SampleAgent() as agent:
        ...
        print(f"  Quality score: {result['total_quality_score']:.2f}")
        print(f"  Execution time: {result['execution_time_ms']}ms")
        print("  Expected: 3 groups - [A], [B,C], [D] with max parallelism 2")
