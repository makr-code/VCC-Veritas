...
class StreamingManager:
    ...
    async def register_client(self, client_id: str, websocket: Any, subscribe_to: Optional[List[str]] = None) -> None:
        """
        ...
        async with self._lock:
            connection = ClientConnection(
                client_id=client_id, websocket=websocket, subscribed_plans=set(subscribe_to) if subscribe_to else set()
            )
    ...
    async def stream_plan_started(self, plan_id: str, plan_data: Dict[str, Any]) -> None:
        """Stream plan started event."""
        event = StreamEvent(event_type=EventType.PLAN_STARTED.value, plan_id=plan_id, data=plan_data)
        await self.stream_event(event)

    async def stream_step_started(self, plan_id: str, step_id: str, step_data: Dict[str, Any]) -> None:
        """Stream step started event."""
        event = StreamEvent(event_type=EventType.STEP_STARTED.value, plan_id=plan_id, step_id=step_id, data=step_data)
        await self.stream_event(event)

    async def stream_step_progress(self, plan_id: str, step_id: str, progress: float, message: str) -> None:
    ...
    async def stream_step_completed(self, plan_id: str, step_id: str, result: Dict[str, Any]) -> None:
        """Stream step completed event."""
        event = StreamEvent(event_type=EventType.STEP_COMPLETED.value, plan_id=plan_id, step_id=step_id, data=result)
        await self.stream_event(event)

    async def stream_quality_check(self, plan_id: str, step_id: str, quality_result: Dict[str, Any]) -> None:
        """Stream quality check event."""
        event = StreamEvent(event_type=EventType.QUALITY_CHECK.value, plan_id=plan_id, step_id=step_id, data=quality_result)
        await self.stream_event(event)

    async def stream_metrics_update(self, plan_id: str, metrics: Dict[str, Any]) -> None:
        """Stream metrics update."""
        event = StreamEvent(event_type=EventType.METRICS_UPDATE.value, plan_id=plan_id, data=metrics)
        await self.stream_event(event)
...
class StreamingExecutionWrapper:
    ...
    def __init__(self, agent: Any, streaming_manager: StreamingManager):
        """
    ...
    async def execute_research_plan_streaming(
        ...
        for i, step in enumerate(steps):
            ...
            for progress in [0.25, 0.5, 0.75]:
                ...
                await self.streaming_manager.stream_step_progress(
                    plan_id=plan_id, step_id=step_id, progress=progress, message=f"Processing {int(progress * 100)}%"
                )
            ...
            await self.streaming_manager.stream_step_completed(
                plan_id=plan_id,
                step_id=step_id,
                result={"status": "success", "quality_score": 0.9, "data": {"result": "Step completed"}},
            )
        ...
        await self.streaming_manager.stream_event(
            StreamEvent(
                ...
                plan_id=plan_id,
                data={"status": "completed", "total_steps": len(steps), "completed_steps": len(steps)},
            )
        )

        return {"status": "completed", "plan_id": plan_id, "total_steps": len(steps), "completed_steps": len(steps)}
...
async def _test_streaming_manager():
    ...
    await manager.stream_plan_started(plan_id="plan_001", plan_data={"query": "Test query", "total_steps": 2})

    await manager.stream_step_started(
        plan_id="plan_001", step_id="step_1", step_data={"action": "search", "agent": "registry"}
    )

    await manager.stream_step_progress(plan_id="plan_001", step_id="step_1", progress=0.5, message="Processing 50%")

    await manager.stream_step_completed(plan_id="plan_001", step_id="step_1", result={"status": "success", "quality": 0.95})

    print("  ✓ Streamed 4 events")
