...
class RetryHandler:
    ...
    def should_retry(self, attempt: int, max_retries: int, exception: Optional[Exception] = None) -> bool:
        """
    ...
    def execute_with_retry(
        ...
        **kwargs,
        ...
        for attempt in range(1, max_retries + 2):  # +1 for initial attempt, +1 for range
            try:
                logger.info(f"Step {step_id}: Attempt {attempt}/{max_retries + 1}")
                ...
                # Success
                if attempt > 1:
                    logger.info(f"Step {step_id}: Success after {attempt - 1} retries")
                ...
                last_exception = e
                logger.warning(f"Step {step_id}: Attempt {attempt} failed: {str(e)}")

                # Check if we should retry
                if not self.should_retry(attempt, max_retries, e):
                    logger.error(f"Step {step_id}: Max retries ({max_retries}) exhausted")
                    raise

                # Calculate delay
                if attempt <= max_retries:  # Don't delay after last retry
                    delay = self.calculate_delay(attempt)
                    logger.info(f"Step {step_id}: Retrying in {delay:.2f}s " f"(strategy: {self.config.strategy.value})")
                    time.sleep(delay)
    ...
    def get_retry_stats(self, attempt: int) -> Dict[str, Any]:
        ...
        delays = [self.calculate_delay(i) for i in range(1, retry_count + 1)]
...
def _test_retry_handler():
    ...
    print("\n[TEST 1] Exponential Backoff Calculation")
    handler = RetryHandler(RetryConfig(base_delay=1.0, backoff_factor=2.0, jitter=False))  # Disable for predictable testing
    ...
    print("\n[TEST 2] Linear Backo")
    handler = RetryHandler(RetryConfig(strategy=RetryStrategy.LINEAR, base_delay=2.0, jitter=False))
    ...
    print("\n[TEST 3] Constant Backo")
    handler = RetryHandler(RetryConfig(strategy=RetryStrategy.CONSTANT, base_delay=3.0, jitter=False))
    ...
    print("\n[TEST 4] Fibonacci Backo")
    handler = RetryHandler(RetryConfig(strategy=RetryStrategy.FIBONACCI, base_delay=1.0, jitter=False))
    ...
    print("\n[TEST 5] Max Delay Cap")
    handler = RetryHandler(RetryConfig(base_delay=10.0, backoff_factor=3.0, max_delay=50.0, jitter=False))
    ...
    # Test 6: Jitter range
    print("\n[TEST 6] Jitter Range")
    handler = RetryHandler(RetryConfig(base_delay=10.0, jitter=True, jitter_range=0.2))  # ±20%

    delays = [handler.calculate_delay(1) for _ in range(100)]
    min_delay = min(delays)
    max_delay = max(delays)

    print("Base delay: 10.0s")
    print(f"Min delay: {min_delay:.2f}s")
    print(f"Max delay: {max_delay:.2f}s")
    print("Expected range: 8.0s - 12.0s (±20%)")
    ...
    result = handler.execute_with_retry(func=success_func, step_id="test_step_1", max_retries=3)
    ...
    handler = RetryHandler(RetryConfig(max_retries=3, base_delay=0.1, jitter=False))  # Fast for testing
    ...
    result = handler.execute_with_retry(func=retry_once_func, step_id="test_step_2", max_retries=3)
    ...
    print("\n[TEST 9] Max Retries Exhausted")
    handler = RetryHandler(RetryConfig(max_retries=2, base_delay=0.05, jitter=False))
    ...
    try:
        result = handler.execute_with_retry(func=always_fail_func, step_id="test_step_3", max_retries=2)
    ...
    print("\n[TEST 10] Context Preservation")
    handler = RetryHandler(RetryConfig(max_retries=3, base_delay=0.05, jitter=False))
    ...
    result = handler.execute_with_retry(func=context_check_func, step_id="test_step_4", max_retries=3, context=initial_context)
    ...
    print("\n[TEST 11] Retry Statistics")
    handler = RetryHandler(RetryConfig(base_delay=1.0, backoff_factor=2.0, jitter=False))
