...
class DwdSimpleWeatherAgent:
    ...
    def find_nearest_station(
        self, latitude: float, longitude: float, max_distance_km: float = 50.0
    ...
    def get_current_weather(self, latitude: float, longitude: float) -> Dict[str, Any]:
        ...
        if not station:
            return {"success": False, "error": "Keine Station gefunden", "data": None}
...
def test_dwd_simple_agent():
    ...
    station = agent.find_nearest_station(latitude=48.1351, longitude=11.5820)
    ...
    weather = agent.get_current_weather(latitude=48.1351, longitude=11.5820)
    ...
    station = agent.find_nearest_station(latitude=52.5200, longitude=13.4050)
