...
class ResearchPlanStorage:
    ...
    def update_plan(self, plan_id: str, updates: Dict[str, Any]) -> bool:
        ...
        if self.using_db:
            try:
                # Only allow updates to known, allowed columns to avoid SQL injection
                allowed_columns = {
                    "research_question",
                    "status",
                    "plan_document",
                ...
                with get_cursor() as cur:
                    # The set_clause is constructed from a whitelist of allowed
                    # columns earlier in this function. Values are parameterized.
                    cur.execute(
                        f"UPDATE research_plans SET {set_clause}, updated_at = CURRENT_TIMESTAMP WHERE plan_id = %(plan_id)s",  # nosec B608  # set_clause validated
    ...
    def update_step(self, step_id: str, updates: Dict[str, Any]) -> bool:
        ...
        if self.using_db:
            try:
                # Validate allowed step columns before constructing SET clause
                allowed_step_columns = {
                    "step_index",
                    "step_name",
                    "step_type",
                    "agent_name",
                ...
                invalid = [k for k in updates.keys() if k not in allowed_step_columns]
                if invalid:
                    raise ValueError(f"Invalid update columns for step: {invalid}")

                set_clause = ", ".join([f"{k} = %({k})s" for k in updates.keys()])
                with get_cursor() as cur:
                    # set_clause validated against allowed fields; parameters are passed separately.
                    cur.execute(
                        f"UPDATE research_plan_steps SET {set_clause} WHERE step_id = %(step_id)s",  # nosec B608  # set_clause validated
    ...
    def list_steps(self, plan_id: str) -> List[Dict[str, Any]]:
        ...
        if self.using_db:
            try:
                with get_cursor() as cur:
                    cur.execute("SELECT * FROM research_plan_steps WHERE plan_id = %s ORDER BY step_index", (plan_id,))
                    results = cur.fetchall()
                    columns = [desc[0] for desc in cur.description]
                    return [dict(zip(columns, row)) for row in results]
            except Exception as e:
