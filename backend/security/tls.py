...
class HSTSMiddleware(BaseHTTPMiddleware):
    ...
    def __init__(
        self, app, max_age: int = 31536000, include_subdomains: bool = True, preload: bool = False, dev_mode: bool = False
    ):
        super().__init__(app)
...
def create_ssl_context(config: TLSConfig) -> ssl.SSLContext:
    ...
    if config.ciphers:
        ...
        logger.info("✅ Custom cipher suite configured")
    else:
...
def add_tls_middleware(app: FastAPI, config: Optional[TLSConfig] = None):
    ...
    if config.enforce_https:
        app.add_middleware(HTTPSRedirectMiddleware, https_port=config.https_port, dev_mode=config.dev_mode)
