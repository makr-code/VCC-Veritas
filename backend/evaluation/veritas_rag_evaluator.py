...
class RAGEvaluator:
    ...
    def __init__(self, pipeline: Any = None, ollama_client: Any = None, strict_mode: bool = False):
        """
    ...
    async def run_evaluation(
        ...
        filtered_cases = self._filter_test_cases(
            filter_category=filter_category, filter_complexity=filter_complexity, filter_ids=filter_ids
        )
        ...
        for i, test_case in enumerate(filtered_cases, 1):
            ...
            if verbose:
                ...
                logger.info(f"   {status} - Score: {result.overall_score:.2f}, " f"Duration: {result.duration_ms:.0f}ms")
    ...
    async def _evaluate_test_case(self, test_case: Dict[str, Any], verbose: bool = False) -> EvaluationResult:
        """
        ...
        """
        start_time = time.time()
        result = EvaluationResult(test_case_id=test_case["id"], query=test_case["question"])
        ...
        try:
            ...
            retrieval_score = self._evaluate_retrieval(
                response=response, expected=test_case["expected_retrieval"], result=result
            )
            ...
            result.overall_score = retrieval_score * 0.3 + context_score * 0.3 + answer_score * 0.4

            # Test-Case passed wenn alle Sub-Metriken passed
            result.passed = result.retrieval_passed and result.context_passed and result.answer_passed
    ...
    async def _run_pipeline(self, query: str) -> Dict[str, Any]:
        ...
        if self.pipeline:
            ...
            try:
                ...
                request = IntelligentPipelineRequest(
                    query_id=f"eval_{hash(query)}", query_text=query, session_id="baseline_evaluation"
                )
        ...
        return {
            ...
            "graph": {"related_entities": ["Entity A", "Entity B"], "relationships": []},
            "meta": {"reranking_applied": True, "duration_ms": 1250},
        }
    ...
    def _evaluate_retrieval(self, response: Dict[str, Any], expected: Dict[str, Any], result: EvaluationResult) -> float:
        """
        ...
        score = (
            sum(precisions) / len(precisions) * 0.4
            + recall * 0.3  # Avg Precision
            + mrr * 0.2
    ...
    async def _evaluate_context(self, response: Dict[str, Any], expected: Dict[str, Any], result: EvaluationResult) -> float:
        """
        ...
        # Context-Score
        score = entity_recall * 0.6 + (1.0 if has_graph_enrichment else 0.5) * 0.4

        return min(1.0, max(0.0, score))

    async def _evaluate_answer(
        self, response: Dict[str, Any], expected: Dict[str, Any], hallucination_triggers: List[str], result: EvaluationResult
    ) -> float:
        """
        ...
        max_len = expected.get("max_length", float("in"))
        length_ok = min_len <= answer_len <= max_len

        # Answer-Score
        hallucination_penalty = len(all_hallucinations) * 0.3  # Starke Penalty!
        score = (
            must_contain_rate * 0.5 + (0.0 if all_hallucinations else 0.3) + (0.2 if length_ok else 0.0)
    ...
    def _compute_summary(self, total_duration: float) -> EvaluationSummary:
        ...
        summary.pass_rate = (
            (summary.passed_test_cases / summary.total_test_cases * 100) if summary.total_test_cases > 0 else 0.0
        )
    ...
    def save_report(self, output_path: str, include_details: bool = True) -> None:
        """
        ...
        report = {"summary": asdict(summary), "results": [asdict(r) for r in self.results] if include_details else []}
...
async def evaluate_golden_dataset(
    dataset_path: str, pipeline: Any = None, output_path: Optional[str] = None, verbose: bool = True
) -> EvaluationSummary:
    """
                ...
                "test_cases": [
                    {
                        ...
                        "expected_answer": {"must_contain": ["Test"], "must_not_contain": ["Fake"], "min_length": 10},
