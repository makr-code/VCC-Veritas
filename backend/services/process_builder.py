...
from backend.models.nlp_models import (
    NLPAnalysisResult, IntentType, EntityType, QuestionType
)
...
class ProcessBuilder:
    ...
    def build_process_tree(self, query: str) -> ProcessTree:
        ...
        logger.info(f"Process tree built: {tree.total_steps} steps, "
                   f"{len(tree.execution_order)} levels")
    ...
    def _create_comparison_steps(self, nlp_result: NLPAnalysisResult) -> List[ProcessStep]:
        ...
        entities = [e for e in nlp_result.entities if e.entity_type in [
            EntityType.ORGANIZATION, EntityType.DOCUMENT, EntityType.PROCEDURE
        ]]
        ...
        if len(entities) >= 2:
            ...
            steps.append(self._create_step(
                ...
                f"Compare differences and similarities",
                StepType.COMPARISON,
    ...
    def _create_procedure_steps(self, nlp_result: NLPAnalysisResult) -> List[ProcessStep]:
        ...
        steps.append(self._create_step(
            ...
            f"Search for requirements and regulations",
            StepType.SEARCH,
            search_params,
            []
        ))
        ...
        steps.append(self._create_step(
            ...
            "Search for necessary forms and documents",
            StepType.SEARCH,
            search_params,
            []
        ))
    ...
    def _create_calculation_steps(self, nlp_result: NLPAnalysisResult) -> List[ProcessStep]:
        ...
        steps.append(self._create_step(
            ...
            "Search for pricing and cost information",
            StepType.SEARCH,
            search_params,
            []
        ))
        ...
        steps.append(self._create_step(
            ...
            "Calculate total cost based on found information",
    ...
    def _create_default_steps(self, template: List[str], 
    ...
    def _create_step(self, name: str, description: str, step_type: StepType,
                    parameters: Dict[str, Any], dependencies: List[str]) -> ProcessStep:
    ...
    def _infer_dependencies(self, steps: List[ProcessStep], 
    ...
    def _estimate_execution_time(self, tree: ProcessTree) -> float:
        ...
        for group in tree.get_parallel_groups():
            ...
            group_time = max(
                time_estimates.get(tree.get_step(sid).step_type, 2.0)
                for sid in group
            )
