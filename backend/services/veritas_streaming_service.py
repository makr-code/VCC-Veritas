...
try:
    from shared.core.veritas_core import BackendResponse, ChatMessage, MessageType, QueueMessage, StatusMessage, ThreadManager
...
try:
    from shared.pipelines.veritas_streaming_progress import ProgressStage, ProgressType, VeritasProgressManager
...
class VeritasStreamingService:
    ...
    def start_streaming_query(
        ...
        try:
            ...
            # API-Request senden
            response = requests.post(f"{self.base_url}/v2/query/stream", json=payload, timeout=10)
            ...
            if response.status_code == 200:
                ...
                return {"success": True, "stream_session_id": stream_session_id, "stream_info": stream_info}
            else:
                error_msg = f"Streaming-Start fehlgeschlagen: {response.status_code}"
                self._send_streaming_message(
                    stream_type=StreamingMessageType.STREAM_ERROR, session_id=session_id, message=error_msg
                )
    ...
    def _start_progress_monitoring(self, stream_session_id: str):
        ...
        # Thread starten
        monitor_thread = threading.Thread(target=monitor_progress, name=f"StreamMonitor-{stream_session_id}", daemon=True)
    ...
    def _send_final_response(self, session_id: str, final_result: Dict[str, Any]):
        ...
        logger.info(
            f"📤 _send_final_response called with final_result keys: {final_result.keys() if final_result else 'EMPTY'}"
        )
        if final_result:
    ...
    def cancel_stream(self, stream_session_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            response = requests.post(
                f"{self.base_url}/cancel/{stream_session_id}", json={"reason": "user_cancelled"}, timeout=5
            )
            ...
            if response.status_code == 200:
                ...
                return {"success": True, "message": cancel_data.get("message", "Stream erfolgreich abgebrochen")}
            else:
                return {
    ...
    def _send_streaming_message(self, stream_type: StreamingMessageType, **kwargs):
        ...
        streaming_msg = StreamingMessage(sender_id="streaming_service", stream_type=stream_type, **kwargs)
...
class StreamingUIMixin:
    ...
    def init_streaming_ui(self, parent_frame):
        ...
        self.streaming_frame = ttk.LabelFrame(parent_frame, text="🔄 Real-time Processing", padding=5)

        # Progress Bar
        self.progress_var = tk.DoubleVar()
        self.progress_bar = ttk.Progressbar(self.streaming_frame, variable=self.progress_var, maximum=100, length=300)
        self.progress_bar.pack(fill=tk.X, pady=2)

        # Status Label
        self.stream_status_var = tk.StringVar(value="Bereit für Streaming...")
        self.stream_status_label = ttk.Label(self.streaming_frame, textvariable=self.stream_status_var, font=("Arial", 8))
        self.stream_status_label.pack(anchor=tk.W)

        # Cancel Button
        self.cancel_button = ttk.Button(
            self.streaming_frame, text="❌ Abbrechen", command=self._cancel_current_stream, state=tk.DISABLED
        )
    ...
    def setup_streaming_integration(self, window_id: str, thread_manager):
        """Setzt Streaming-Integration au"""
    ...
    def _add_stage_reflection(self, reflection_data: Dict[str, Any]):
        ...
        # Status-Emoji
        status_emoji = {"incomplete": "🔴", "partial": "🟡", "complete": "🟢"}.get(status, "⚪")
...
def setup_streaming_chat_tags(chat_display):
    ...
    chat_display.tag_config(
        "reflection", foreground="#9900CC", font=("Arial", 9), background="#F8F0FF", spacing1=4, spacing3=4
    )
...
def get_streaming_preferences() -> Dict[str, bool]:
    ...
    return {"enable_progress": True, "enable_intermediate": True, "enable_thinking": True, "auto_show_ui": True}
