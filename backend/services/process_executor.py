...
from backend.models.process_step import ProcessStep, StepResult, StepStatus, StepType
...
try:
    from backend.models.document_source import CitationConfidence, DocumentSource, SourceCitation
...
try:
    from backend.models.hypothesis import ConfidenceLevel, Hypothesis
...
class ProcessExecutor:
    ...
    def execute_process(self, tree: ProcessTree, progress_callback: Optional["ProgressCallback"] = None) -> Dict[str, Any]:
        """
                ...
                if self.rag_service:
                    try:
                        ...
                        if search_results and "results" in search_results:
                            rag_context = [
                                result.get("content", "")[:200] for result in search_results["results"]  # First 200 chars
                            ]
                    except Exception as e:
                        logger.warning(f"⚠️ RAG context retrieval failed: {e}")

                # Generate hypothesis
                hypothesis = self.hypothesis_service.generate_hypothesis(query=tree.query, rag_context=rag_context)

                logger.info(
                    f"✅ Hypothesis: type={hypothesis.question_type.value}, "
        ...
        for level_num, level_steps in enumerate(execution_plan):
            ...
            level_results = self._execute_parallel_group(tree, level_steps, current_step_num, total_steps, progress_callback)
        ...
        if progress_callback and self.streaming_available:
            progress_callback.emit(
                create_plan_completed_event(total_steps, len(completed_steps), len(failed_steps), execution_time)
            )

        logger.info(
            f"Process execution complete: {len(completed_steps)} completed, "
    ...
    def _execute_parallel_group(
        ...
        if progress_callback and self.streaming_available:
            for i, step_id in enumerate(step_ids):
                ...
                progress_callback.emit(
                    create_step_started_event(
                        step_id, step.name, current_step, total_steps, metadata={"step_type": step.step_type.value}
                    )
                )
        ...
        with ThreadPoolExecutor(max_workers=self.max_workers) as executor:
            ...
            future_to_step = {
                executor.submit(
                    self._execute_step, tree.get_step(step_id), base_step_num + i + 1, total_steps, progress_callback
                ): step_id
                for i, step_id in enumerate(step_ids)
            }
            ...
            for future in as_completed(future_to_step):
                ...
                try:
                    ...
                    results[step_id] = StepResult(success=False, error=f"Exception: {str(e)}")
        ...
        try:
            ...
            if self.use_agents and hasattr(self, "agent_executor"):
                ...
                if progress_callback and self.streaming_available:
                    progress_callback.emit(
                        create_step_progress_event(
                            step.id, step.name, current_step, total_steps, 25.0, message=f"Using agents for {step.name}..."
                        )
                    )
                ...
                if progress_callback and self.streaming_available:
                    progress_callback.emit(
                        create_step_progress_event(
                            step.id, step.name, current_step, total_steps, 90.0, message="Agent execution completed"
                        )
                    )
                ...
                if progress_callback and self.streaming_available:
                    if result.success:
                        progress_callback.emit(
                            create_step_completed_event(
                                step.id, step.name, current_step, total_steps, execution_time, result.data
                            )
                        )
                    else:
                        progress_callback.emit(
                            create_step_failed_event(
                                step.id, step.name, current_step, total_steps, result.error or "Unknown error"
                            )
                        )
            ...
            # Fallback: Mock execution
                ...
                if progress_callback and self.streaming_available:
                    progress_callback.emit(
                        create_step_progress_event(
                            step.id, step.name, current_step, total_steps, 10.0, message=f"Mock execution for {step.name}..."
                        )
                    )
                ...
                if progress_callback and self.streaming_available:
                    progress_callback.emit(
                        create_step_progress_event(
                            step.id, step.name, current_step, total_steps, 50.0, message=f"Processing {step.name}..."
                        )
                    )
                ...
                if progress_callback and self.streaming_available:
                    progress_callback.emit(
                        create_step_progress_event(
                            step.id, step.name, current_step, total_steps, 90.0, message=f"Finalizing {step.name}..."
                        )
                    )
            ...
            if progress_callback and self.streaming_available and not self.use_agents:
                progress_callback.emit(
                    create_step_completed_event(
                        step.id, step.name, current_step, total_steps, result.execution_time, result.data
                    )
                )
            ...
            if progress_callback and self.streaming_available:
                progress_callback.emit(create_step_failed_event(step.id, step.name, current_step, total_steps, str(e)))

            return StepResult(success=False, error=str(e), metadata={"step_id": step.id})
    ...
    def _generate_data_with_documents(self, step: ProcessStep, documents: List["DocumentSource"]) -> Dict[str, Any]:
        """
    ...
    def _generate_mock_data(self, step: ProcessStep) -> Dict[str, Any]:
        ...
        if step_type == StepType.SEARCH:
            ...
            return {"documents_retrieved": 3, "content": f"Mock content for {step.name}", "metadata": params}
            ...
            return {
                "entities": entities,
                "similarities": ["Both have property X", f"Both are regulated"],
            ...
            return {"step_type": step_type.value, "result": f"Mock result for {step.name}", "parameters": params}
    ...
    def _retrieve_documents(
        self, step: ProcessStep, max_results: int = 5, min_relevance: float = 0.3
    ) -> List["DocumentSource"]:
        """
        ...
        try:
            ...
            filters = SearchFilters(max_results=max_results, min_relevance=min_relevance)

            search_result = self.rag_service.hybrid_search(query=query, filters=filters)
            ...
            if self.enable_reranking and self.reranker_service and search_result.results:
                try:
                    ...
                    reranking_results = self.reranker_service.rerank(
                        query=query, documents=docs_for_rerank, top_k=max_results, batch_size=5
                    )
    ...
    def _build_context(self, documents: List["DocumentSource"], max_tokens: int = 2000) -> str:
        """
    ...
    def _extract_citations(self, documents: List["DocumentSource"]) -> List["SourceCitation"]:
        """
        ...
        for doc in documents:
            ...
            citation = SourceCitation(source=doc, confidence=confidence, page_number=page_number, excerpt=doc.get_excerpt(200))
...
if __name__ == "__main__":
    ...
    # Test queries
    test_queries = ["Bauantrag für Stuttgart", "Unterschied zwischen GmbH und AG", "Wie viel kostet ein Bauantrag?"]
