...
class CoordinateTransformer:
    ...
    def __init__(self):
        ...
        self.transformer = Transformer.from_crs(
            "EPSG:25833", "EPSG:4326", always_xy=True  # ETRS89 UTM Zone 33N  # WGS84 (lat/lon)
        )
    ...
    def is_in_bounds(self, lat: float, lon: float, bounds: Optional[str]) -> bool:
        """
...
async def get_bimschg_heatmap(
    ...
    try:
        ...
        for ostwert, nordwert, count in rows:
            ...
            points.append(HeatmapPoint(lat=lat, lon=lon, intensity=float(count)))
...
async def search_location(
    ...
    try:
        ...
        for row in cursor.fetchall():
            ...
            results.append(SearchResult(id=bimschg_id, name=bst_name, type="BImSchG", ort=ort, lat=lat, lon=lon))
        ...
        if len(results) < limit:
            ...
            for row in cursor.fetchall():
                ...
                results.append(
                    SearchResult(id=wka_id, name=betreiber or "Unbekannter Betreiber", type="WKA", ort=ort, lat=lat, lon=lon)
                )
...
async def get_region_statistics(bounds: str = Query(..., description="min_lat,min_lon,max_lat,max_lon")) -> RegionStatistics:
    """
...
async def get_filter_options() -> FilterOptions:
    ...
    try:
        ...
        return FilterOptions(bimschg_categories=bimschg_categories, wka_status=wka_status, orte=orte)
