...
async def login(form_data: OAuth2PasswordRequestForm = Depends()) -> Token:
    ...
    if not ENABLE_AUTH:
        test_token = create_access_token(data={"sub": "dev_admin", "roles": ["admin", "manager", "user", "guest"]})
        return Token(access_token=test_token, token_type="bearer", expires_in=ACCESS_TOKEN_EXPIRE_MINUTES * 60)
    ...
    access_token = create_access_token(
        data={"sub": user.username, "roles": [role.value for role in user.roles]}, expires_delta=access_token_expires
    )
...
async def auth_status() -> Dict:
    ...
    return {
        ...
        "rbac": {"roles": ["admin", "manager", "user", "guest"], "description": "Role - Based Access Control enabled"},
    }
