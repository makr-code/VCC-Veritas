...
try:
    from veritas_chunk_quality_db import get_chunk_quality_scores, get_document_chunks, update_chunk_quality
        ...
        try:
            if self.quality_manager:
                ...
                quality_result = await self.quality_manager.assess_answer_quality(
                    question=request.question, answer=request.answer, sources=request.sources, context=request.context
                )
...
async def get_document_quality(document_id: str, include_chunks: bool = True, quality_threshold: float = 0.7):
    ...
    request = DocumentQualityRequest(
        document_id=document_id, include_chunks=include_chunks, quality_threshold=quality_threshold
    )
    ...
    try:
        if quality_controller.quality_manager:
            ...
            return RAGOptimizationResponse(
                ...
                recommended_settings={"chunk_size": 512, "overlap": 50, "top_k": 5, "temperature": 0.7},
                quality_improvement=0.15,
                performance_impact={"response_time_change": "+5%", "accuracy_improvement": "+15%"},
            )
...
async def get_quality_metrics_summary():
    ...
    try:
        ...
        return {
            ...
            "improvement_trends": {"last_30_days": "+5.2%", "quality_stability": "stable"},
            "top_quality_issues": ["Kurze Chunks mit wenig Kontext", "Inkonsistente Formatierung", "Fehlende Metadaten"],
            "timestamp": datetime.now().isoformat(),
        }
