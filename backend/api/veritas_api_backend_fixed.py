...
def initialize_agent_pipeline():
    ...
    try:
        ...
        agent_coordinator = create_agent_coordinator(registry=agent_registry, pipeline_manager=pipeline_manager)

        # Agent Orchestrator erstellen
        agent_orchestrator = create_agent_orchestrator(agent_coordinator=agent_coordinator, pipeline_manager=pipeline_manager)
    ...
    try:
        ...
        if not AGENT_PIPELINE_AVAILABLE or not agent_orchestrator:
            ...
            return {
                ...
                "sources": [],
                "worker_results": {},
                "processing_metadata": {"processing_time": time.time() - start_time, "system_mode": "legacy"},
            }
        ...
        logger.info(
            f"🎯 Chat-Query verarbeitet: {preprocessing_result['complexity']}/{preprocessing_result['domain']} - {processing_time:.2f}s"
        )
...
async def veritas_agent_query(request: VeritasAgentQueryRequest):
    ...
    try:
        ...
        logger.info(
            f"✅ Agent-Query verarbeitet: {preprocessing_result['complexity']}/{preprocessing_result['domain']} - {processing_time:.2f}s"
        )
...
async def get_available_modes():
    ...
    return {"available_modes": system_manager.get_available_modes(), "default_mode": "VERITAS"}
...
if __name__ == "__main__":
    ...
    # Bind host is configurable to avoid unintentionally exposing the server.
    # Default to localhost for development and require VERITAS_API_HOST=0.0.0.0 in containerized deployments.
    host = os.getenv("VERITAS_API_HOST", "127.0.0.1")
    port = int(os.getenv("VERITAS_API_PORT", "5000"))
    logger.info(f"Starting Veritas API on {host}:{port}")
