...
try:
    from covina_base import BaseWorker, DatabaseWorker, EULexWorker, ExternalAPIWorker, GoogleSearchWorker
    from covina_core import CovinaAgentEngine, WorkerType, get_agent_engine
...
# UDS3 Integration
try:
    from uds3.uds3_admin_types import AdminDocumentType, AdminDomain, AdminLevel
...
try:
    from covina_file_manager import CovinaConfigManager, CovinaFileManager
...
EXTERNAL_DATA_CONFIG = {
    "eu_lex": {"enabled": True, "base_url": "https://eur - lex.europa.eu/", "cache_ttl": 3600},
    "google_search": {"enabled": False, "api_key": None, "search_engine_id": None, "cache_ttl": 1800},  # Benötigt API-Key
    "sql_databases": {
        "legal_db": {"type": "sqlite", "path": "sqlite_db / legal_data.db", "enabled": False},
        "admin_db": {
            "type": "postgresql",
            "host": "localhost",
            "port": 5432,
...
class DocumentRetrievalWorker(BaseWorker):
    ...
    async def _process_internal(self, metadata, user_profile: Dict = None) -> Dict[str, Any]:
        ...
        if not self.rag_available:
            return {"retrieved_docs": [], "summary": "VERITAS RAG nicht verfügbar", "confidence_score": 0.0}
    ...
    async def _analyze_geo_context(self, locations: List[str]) -> Dict[str, Any]:
        ...
        for location in locations:
            if "berlin" in location.lower():
                authorities.append(
                    {"name": "Bezirksamt Berlin", "type": "Bezirksverwaltung", "contact": "https://service.berlin.de"}
                )
                jurisdiction = "Land Berlin"
            elif "münchen" in location.lower():
                authorities.append(
                    {"name": "Landeshauptstadt München", "type": "Stadtverwaltung", "contact": "https://www.muenchen.de"}
                )
                jurisdiction = "Bayern"

        return {"authorities": authorities, "jurisdiction": jurisdiction}
            ...
            "Immissionsschutzrecht": [
                {
                    ...
                    "key_provisions": "§§ 4 - 21 (Genehmigungsbedürftige Anlagen)",
                },
...
class CovinaModuleManagerEnhanced:
        ...
        try:
            ...
            if google_config["enabled"] and google_config["api_key"]:
                google_worker = GoogleSearchWorker(
                    api_key=google_config["api_key"], search_engine_id=google_config["search_engine_id"]
                )
    ...
    def _initialize_specialized_workers(self):
        ...
        try:
            ...
            total_workers = (
                len(ENVIRONMENTAL_WORKERS)
                + len(CONSTRUCTION_WORKERS)
                + len(TRAFFIC_WORKERS)
                + len(FINANCIAL_WORKERS)
                + len(SOCIAL_WORKERS)
            )
    ...
    def configure_external_data_source(self, source_type: str, config: Dict[str, Any]):
        ...
        if source_type == "google_search":
            ...
            if config.get("enabled") and config.get("api_key"):
                try:
                    google_worker = GoogleSearchWorker(
                        api_key=config["api_key"], search_engine_id=config.get("search_engine_id")
                    )
                    self.external_workers["google_search"] = google_worker
    ...
    def get_module(self, module_name: str):
        """Legacy-Methode für Modul-Zugrif"""
