    ...
    try:
        ...
        lineage_graph = {
            ...
            "nodes": [
                ...
                {"node_id": "source_pdf", "type": "source", "name": "PDF Files"},
            ...
            "edges": [
                {"from": "source_pdf", "to": lineage_req.entity_id, "type": "feeds"},
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Data Lineage Abruf fehlgeschlagen: {str(e)}"
        )
...
async def search_data_catalog(
    ...
    **Returns**:
    ...
    try:
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Data Catalog Search fehlgeschlagen: {str(e)}"
        )
    ...
    try:
        ...
        for i in range(min(limit, 12)):
            policy = DataGovernancePolicy(
                ...
                rules=[
                    {"rule_id": f"rule_{j}", "condition": f"condition_{j}", "action": f"action_{j}"}
                    for j in range(3)
                ],
    ...
    **Returns**:
    ...
    try:
        ...
        if action == "check":
            ...
            raise HTTPException(
                status_code=400,
                detail=f"Invalid action: {action}. Use 'check', 'grant', or 'revoke'"
            )
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Access Control Operation fehlgeschlagen: {str(e)}"
        )
...
async def get_governance_audit_log(
    ...
    **Returns**:
    ...
    try:
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Governance Audit-Log Abruf fehlgeschlagen: {str(e)}"
        )
...
async def get_governance_metrics(
    request: Request,
    metric_type: str = Query("all", description="Metric Type (all, quality, usage, compliance)")
):
    """
    ...
    **Returns**:
    ...
    try:
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Metrics-Generierung fehlgeschlagen: {str(e)}"
        )
