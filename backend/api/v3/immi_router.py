...
from .models import ErrorResponse, IMMIGeoData, IMMIQueryRequest, IMMIQueryResponse, IMMIRegulation
...
immi_router = APIRouter(prefix="/immi", tags=["IMMI - Immissionsschutz"])
...
async def immi_query(query_req: IMMIQueryRequest, request: Request):
    """
...
async def get_immi_regulations(
    request: Request, category: Optional[str] = None, search: Optional[str] = None, limit: int = 20
):
    """
    ...
    try:
        ...
        raise HTTPException(status_code=503, detail="IMMI Vorschriften-Abruf noch nicht implementiert")
...
async def get_immi_geodata(
    ...
    try:
        ...
        raise HTTPException(status_code=503, detail="IMMI Geodaten-Abruf noch nicht implementiert")
