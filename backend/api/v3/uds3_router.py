...
async def unified_query(query_req: UDS3QueryRequest, request: Request):
    """
    ...
    if not uds3:
        ...
        if query_req.database_type == "vector":
            ...
            results = [{"node_id": "node_1", "labels": ["Document"], "properties": {"title": "VPB Dokument", "year": 2023}}]
        elif query_req.database_type == "relational":
            results = [{"id": 1, "title": "SQL Result", "created_at": "2025 - 10-18"}]
        elif query_req.database_type == "file":
            results = [{"file_id": "file_123", "name": "document.pd", "size": 1024000, "url": " / files/document.pdf"}]
        ...
        return UDS3QueryResponse(
            ...
            results=results,
            count=len(results),
            metadata={"mode": "demo", "query": query_req.query, "parameters": query_req.parameters or {}},
            duration=duration,
        )
...
async def vector_search(search_req: VectorSearchRequest, request: Request):
    """
    ...
    try:
        ...
        return VectorSearchResponse(
            results=results, count=len(results), query_vector=search_req.query_vector, duration=duration
        )
...
async def graph_query(query_req: GraphQueryRequest, request: Request):
    """
        ...
        nodes = [
            {
                "node_id": "doc_1",
                "labels": ["Document"],
                "properties": {"title": "VPB Dokument zu Windkraft", "year": 2023, "source": "vpb"},
            },
            {"node_id": "entity_1", "labels": ["Entity"], "properties": {"name": "Windkraftanlage", "type": "facility"}},
        ]

        relationships = [
            {"rel_id": "rel_1", "type": "MENTIONS", "from": "doc_1", "to": "entity_1", "properties": {"count": 15}}
        ]

        duration = time.time() - start_time

        return GraphQueryResponse(
            nodes=nodes[: query_req.limit], relationships=relationships[: query_req.limit], count=len(nodes), duration=duration
        )

    # Production: Cypher query via UDS3
    try:
        result = uds3.graph_query(cypher_query=query_req.cypher_query, parameters=query_req.parameters, limit=query_req.limit)
...
async def relational_query(
    query: str = QueryParam(..., description="SQL Query"), parameters: Optional[Dict[str, Any]] = None, request: Request = None
):
    """
...
async def get_file(
    file_id: str, download: bool = QueryParam(False, description="Download file (vs. metadata only)"), request: Request = None
):
    """
    ...
    if not uds3:
        ...
        return {
            "file_id": file_id,
            "name": f"document_{file_id}.pd",
            "size": 1024000,
            "content_type": "application/pdf",
            "created_at": datetime.now().isoformat(),
            "url": f"/files/{file_id}",
            "metadata": {"source": "vpb", "year": 2023, "type": "regulation"},
        }
...
async def bulk_operation(bulk_req: BulkOperationRequest, request: Request):
    """
