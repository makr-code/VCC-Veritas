...
def test_enterprise_models():
    ...
    try:
        from backend.api.v3.models import (  # SAGA Models; Compliance Models; Governance Models
        ...
        saga_step = SAGAStep(step_id="step_1", service="test_service", action="test_action", parameters={"key": "value"})
        print(f"   ✅ SAGAStep erstellt: step_id='{saga_step.step_id}', service={saga_step.service}")

        # Test Compliance Model Creation
        compliance_req = ComplianceCheckRequest(entity_type="document", entity_id="doc_123", rules=["GDPR", "DSGVO"])
        print(
            f"   ✅ ComplianceCheckRequest erstellt: entity_type='{compliance_req.entity_type}', rules={compliance_req.rules}"
        )

        # Test Governance Model Creation
        lineage_req = DataLineageRequest(entity_id="dataset_123", depth=3, direction="both")
