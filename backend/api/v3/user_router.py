...
from backend.api.v3.models import UserFeedback, UserPreferences, UserProfile, UserQueryHistory, UserRegistration
...
async def register_user(registration: UserRegistration, request: Request):
    ...
    if not uds3:
        return UserProfile(
            ...
            last_login=None,
            query_count=0,
            preferences={"theme": "forest", "language": "de", "default_mode": "veritas"},
        )
...
async def get_user_profile(user_id: str = QueryParam(..., description="User ID"), request: Request = None):
    """
    ...
    if not uds3:
        return UserProfile(
            ...
            query_count=156,
            preferences={"theme": "forest", "language": "de", "default_mode": "veritas", "enable_llm_commentary": True},
        )
...
async def get_user_preferences(user_id: str = QueryParam(..., description="User ID"), request: Request = None):
    """
...
async def update_user_preferences(preferences: UserPreferences, request: Request):
    ...
    try:
        updated_prefs = uds3.update_user_preferences(preferences.user_id, preferences.model_dump())
...
async def get_query_history(
    ...
    try:
        ...
        history = uds3.get_query_history(filters=filters, limit=limit, offset=offset)
