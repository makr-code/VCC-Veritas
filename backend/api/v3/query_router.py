    ...
    try:
        ...
        for src in pipeline_result.get("sources", []):
            ...
            sources_metadata.append(SourceMetadata(
                ...
                **{k: v for k, v in src.items() if k not in ['id', 'file', 'page', 'confidence', 'authors', 'title', 'year']}
            ))
...
async def query_stream(query_req: QueryRequest, request: Request):
    ...
    try:
        ...
        if not services["streaming"] or not services["ollama"]:
            raise HTTPException(
                status_code=503,
                detail="Streaming service unavailable"
            )
...
async def query_intelligent(query_req: QueryRequest, request: Request):
    ...
    try:
        ...
        logger.info(f"✅ Intelligent Query completed: {duration:.2f}s, {len(agents_involved)} agents, complexity={metadata.complexity}")
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Intelligent query failed: {str(e)}"
        )
