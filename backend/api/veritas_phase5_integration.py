...
try:
    ...
    from backend.agents.veritas_sparse_retrieval import SparseRetrievalConfig, SparseRetriever
...
async def initialize_phase5_hybrid_search(demo_corpus: Optional[List[Dict[str, str]]] = None) -> bool:
    """
    ...
    try:
        ...
        try:
            ...
            try:
                ...
                uds3_strategy = UDS3PolyglotManager(backend_config=backend_config, enable_rag=True)
            ...
            if uds3_strategy is None:
                ...
                logger.info("   ✅ Mock Dense Retriever initialized (no real vector search)")
            else:
            ...
            uds3_adapter = MockDenseRetriever()
            logger.info("   ✅ Mock Dense Retriever initialized (fallback)")

        # Step 2: Initialize BM25 Sparse Retriever
        logger.info("\n📦 Step 2: Initializing BM25 Sparse Retriever...")

        bm25_config = SparseRetrievalConfig(k1=config["bm25_k1"], b=config["bm25_b"])
        ...
        hybrid_retriever = HybridRetriever(dense_retriever=uds3_adapter, sparse_retriever=bm25_retriever, config=hybrid_config)
        ...
        try:
            ...
            if results and len(results) > 0:
                ...
                logger.info("   ⚠️ No results (expected if corpus not loaded)")
        except Exception as e:
        ...
        logger.info("   ✅ Hybrid Retriever: OK")
        logger.info(f"\n📝 Usage:")
...
def get_hybrid_retriever():
    ...
    if hybrid_retriever is None:
        raise RuntimeError("Hybrid Retriever not initialized! " "Call initialize_phase5_hybrid_search() first.")
...
def get_bm25_retriever():
    ...
    if bm25_retriever is None:
        raise RuntimeError("BM25 Retriever not initialized! " "Call initialize_phase5_hybrid_search() first.")
...
def get_uds3_adapter():
    ...
    if uds3_adapter is None:
        raise RuntimeError("UDS3 Adapter not initialized! " "Call initialize_phase5_hybrid_search() first.")
