...
async def _process_streaming_query(session_id: str, query_id: str, request: VeritasStreamingQueryRequest):
    ...
    logger.info(f"🔍 _process_streaming_query STARTED for session={session_id}, query={request.query[:50]}")
    
    try:
        # Lade StageReflectionService (falls verfügbar)
        reflection_service = None
        if INTELLIGENT_PIPELINE_AVAILABLE and ollama_client is not None:
            try:
                ...
                logger.info(f"✅ Stage Reflection Service aktiv mit Ollama-Client (enabled={reflection_service.reflection_enabled})")
            except Exception as e:
                logger.warning(f"⚠️ Stage Reflection Service nicht verfügbar: {e}")
                import traceback
                traceback.print_exc()
        else:
            logger.info(f"ℹ️ Stage Reflection Service: INTELLIGENT_PIPELINE={INTELLIGENT_PIPELINE_AVAILABLE}, ollama_client={'verfügbar' if ollama_client else 'nicht verfügbar'}")
        ...
        # 🔍 REFLECTION: Hypothesen-Generierung
        if reflection_service and request.enable_llm_thinking:
            try:
                logger.info(f"🔍 Starte Hypothesis Reflection (LLM-enabled={reflection_service.reflection_enabled})")
                hypothesis_reflection = await reflection_service.reflect_on_stage(
                    stage=ReflectionStage.HYPOTHESIS,
                    user_query=request.query,
                    stage_data={
                        'hypotheses': [
                            f"Query betrifft {domain}-Domäne",
        ...
        # DEBUG
        logger.info(f"🔍 DEBUG: INTELLIGENT_PIPELINE_AVAILABLE={INTELLIGENT_PIPELINE_AVAILABLE}")
        logger.info(f"🔍 DEBUG: intelligent_pipeline={intelligent_pipeline is not None}")
        
        if INTELLIGENT_PIPELINE_AVAILABLE and intelligent_pipeline:
            logger.info(f"✅ Nutze Intelligent Pipeline für Agent-Execution")
            ...
            try:
                ...
                logger.info(f"✅ Intelligent Pipeline: {len(agent_results)} Agenten ausgeführt")
                
            except Exception as e:
                logger.error(f"❌ Intelligent Pipeline Fehler: {e}, Fallback auf Mock")
                import traceback
                traceback.print_exc()
            ...
            logger.warning("⚠️ Intelligent Pipeline nicht verfügbar, nutze Mock-Agenten")
...
def _generate_agent_result(agent_type: str, query: str, complexity: str) -> Dict[str, Any]:
    ...
    # Mapping von Agent-Typen zu Such-Kategorien
    agent_to_category = {
...
def _synthesize_final_response(
    ...
    # 🐛 DEBUG: Log conversation_history
    logger.info(f"📚 _synthesize_final_response: conversation_history = {conversation_history is not None}")
    ...
    if conversation_history and len(conversation_history) > 0:
        ...
        conversation_context += "\n"
        logger.info(f"✅ Conversation context created: {len(conversation_context)} chars")
