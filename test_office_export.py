...
3. **Datenbanken**: PostgreSQL, Neo4j, ChromaDB""",
        "timestamp": datetime.now().strftime("%Y-%m-%d %H:%M:%S"),
        "metadata": {"confidence": 92, "duration": 2.3, "sources_count": 3, "agents_count": 2},
        "sources": [
...
5. Quality Checker → User""",
        "timestamp": (datetime.now() - timedelta(hours=1)).strftime("%Y-%m-%d %H:%M:%S"),
        "metadata": {"confidence": 88, "duration": 1.8, "sources_count": 2, "agents_count": 4},
        "sources": [
...
- .py, .js, .ts, .java, .cpp, .c, .h, .cs, .go, .rs, .sql""",
        "timestamp": (datetime.now() - timedelta(days=2)).strftime("%Y-%m-%d %H:%M:%S"),
        "metadata": {"confidence": 95, "duration": 1.2, "sources_count": 1},
        "sources": [{"title": "File Format Documentation", "page": 2, "relevance": 0.98}],
    },
]
...
def test_word_export():
    ...
    try:
        output_path = service.export_to_word(
            chat_messages=SAMPLE_MESSAGES, title="VERITAS Test-Chat", include_metadata=True, include_sources=True
        )
...
def test_excel_export():
    ...
    try:
        output_path = service.export_to_excel(chat_messages=SAMPLE_MESSAGES, feedback_stats=SAMPLE_FEEDBACK_STATS)
...
def test_empty_messages():
    ...
    try:
        output_path = service.export_to_word(chat_messages=[], title="Leerer Chat")
