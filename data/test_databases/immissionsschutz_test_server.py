...
async def health():
    """Health check"""
    health_status = {"status": "healthy", "timestamp": datetime.now().isoformat(), "databases": {}}
...
async def get_anlage(db: Literal["bimschg", "wka"], bst_nr: str, anl_nr: str):
...
async def get_verfahren(verfahren_id: str):
    ...
    # Verfahren
    verfahren = cursor.execute("SELECT * FROM genehmigungsverfahren WHERE verfahren_id = ?", (verfahren_id,)).fetchone()
    ...
    # Bescheide
    bescheide = cursor.execute("SELECT * FROM bescheide WHERE verfahren_id = ?", (verfahren_id,)).fetchall()

    # Auflagen (für alle Bescheide)
    auflagen = []
    for bescheid in bescheide:
        auflagen_rows = cursor.execute("SELECT * FROM auflagen WHERE bescheid_id = ?", (bescheid["bescheid_id"],)).fetchall()
        auflagen.extend([dict(row) for row in auflagen_rows])

    conn.close()

    return {"verfahren": dict(verfahren), "bescheide": [dict(b) for b in bescheide], "auflagen": auflagen}
...
async def get_anlage_complete(bst_nr: str, anl_nr: str, include_messungen: bool = True, include_verfahren: bool = True):
    """
    Vollständige Anlagen-Daten mit allen Relationen
    ...
    try:
        ...
        row = cursor.execute(
            "SELECT * FROM bimschg WHERE CAST(bst_nr AS TEXT) = ? AND anl_nr = ?", (bst_nr, anl_nr)
        ).fetchone()
        conn_bimschg.close()
    ...
    if not anlage_data:
        try:
            ...
            row = cursor.execute("SELECT * FROM wka WHERE bst_nr = ? AND anl_nr = ?", (bst_nr, anl_nr)).fetchone()
            conn_wka.close()
    ...
    if include_verfahren:
        verfahren_rows = cursor.execute(
            "SELECT * FROM genehmigungsverfahren WHERE bst_nr LIKE ? AND anl_nr LIKE ?", (f"%{bst_nr}%", f"%{anl_nr}%")
        ).fetchall()
        verfahren = [Verfahren(**dict(row)) for row in verfahren_rows]

        # Bescheide
        for v in verfahren:
            bescheid_rows = cursor.execute("SELECT * FROM bescheide WHERE verfahren_id = ?", (v.verfahren_id,)).fetchall()
    ...
    if include_messungen:
        messung_rows = cursor.execute(
            "SELECT * FROM messungen WHERE bst_nr LIKE ? AND anl_nr LIKE ? LIMIT 50", (f"%{bst_nr}%", f"%{anl_nr}%")
        ).fetchall()
        messungen = [Messung(**dict(row)) for row in messung_rows]

    # Überwachungen
    ueberwachung_rows = cursor.execute(
        "SELECT * FROM ueberwachung WHERE bst_nr LIKE ? AND anl_nr LIKE ? LIMIT 20", (f"%{bst_nr}%", f"%{anl_nr}%")
    ).fetchall()
    ueberwachungen = [Ueberwachung(**dict(row)) for row in ueberwachung_rows]

    # Mängel
    maengel_rows = cursor.execute(
        "SELECT * FROM maengel WHERE bst_nr LIKE ? AND anl_nr LIKE ? LIMIT 20", (f"%{bst_nr}%", f"%{anl_nr}%")
    ).fetchall()
...
async def statistik_overview():
    ...
    stats = {
        "verfahren": {
            ...
            "in_bearbeitung": cursor.execute(
                "SELECT COUNT(*) FROM genehmigungsverfahren WHERE status = 'in_bearbeitung'"
            ).fetchone()[0],
        },
        "messungen": {
        ...
        "ueberwachung": {
            ...
            "mit_maengeln": cursor.execute(
                "SELECT COUNT(*) FROM ueberwachung WHERE ergebnis IN ('mängel_festgestellt', 'nachbesserung_erforderlich')"
            ).fetchone()[0],
        },
        "maengel": {
            ...
            "kritisch": cursor.execute(
                "SELECT COUNT(*) FROM maengel WHERE schweregrad = 'kritisch' AND status != 'behoben'"
            ).fetchone()[0],
        },
    }
    ...
    return {"timestamp": datetime.now().isoformat(), "statistics": stats}
...
async def search_dokumente(
    ...
    return {"count": len(dokumente), "dokumente": dokumente}
...
async def search_ansprechpartner(
    bst_nr: Optional[str] = None, anl_nr: Optional[str] = None, funktion: Optional[str] = None, aktiv: int = 1, limit: int = 50
):
    ...
    return {"count": len(ansprechpartner), "ansprechpartner": ansprechpartner}
...
async def search_wartung(
    ...
    return {"count": len(wartungen), "wartungen": wartungen}
...
async def search_messreihen(
    ...
    return {"count": len(messreihen), "messreihen": messreihen}
...
async def search_behoerden(behoerde: Optional[str] = None, abteilung: Optional[str] = None, limit: int = 50):
    ...
    return {"count": len(kontakte), "kontakte": kontakte}


@app.get("/compliance/search", tags=["Compliance"])
async def search_compliance(
    bst_nr: Optional[str] = None, anl_nr: Optional[str] = None, ergebnis: Optional[str] = None, limit: int = 50
):
    ...
    return {"count": len(historie), "historie": historie}


@app.get("/anlage-extended/{bst_nr}/{anl_nr}", tags=["Anlagen"])
async def get_anlage_extended(bst_nr: str, anl_nr: str, include_messungen: bool = True, include_dokumente: bool = True):
    """
    ...
    # Verfahren
    cursor_immi.execute("SELECT * FROM genehmigungsverfahren WHERE bst_nr = ? AND anl_nr = ?", (bst_nr, anl_nr))
    ...
    bescheide = []
    for v in verfahren:
        cursor_immi.execute("SELECT * FROM bescheide WHERE verfahren_id = ?", (v["verfahren_id"],))
    ...
    if include_messungen:
        cursor_immi.execute(
            "SELECT * FROM messungen WHERE bst_nr = ? AND anl_nr = ? ORDER BY messdatum DESC LIMIT 100", (bst_nr, anl_nr)
        )
    ...
    # Überwachungen
    cursor_immi.execute(
        "SELECT * FROM ueberwachung WHERE bst_nr = ? AND anl_nr = ? ORDER BY geplant_datum DESC LIMIT 20", (bst_nr, anl_nr)
    )
    ...
    # Mängel
    cursor_immi.execute(
        "SELECT * FROM maengel WHERE bst_nr = ? AND anl_nr = ? ORDER BY festgestellt_datum DESC", (bst_nr, anl_nr)
    )
    ...
    if include_dokumente:
        cursor_immi.execute(
            "SELECT * FROM dokumente WHERE bst_nr = ? AND anl_nr = ? ORDER BY erstellt_datum DESC LIMIT 50", (bst_nr, anl_nr)
        )
    ...
    # Ansprechpartner
    cursor_immi.execute("SELECT * FROM ansprechpartner WHERE bst_nr = ? AND anl_nr = ? AND aktiv = 1", (bst_nr, anl_nr))
    ...
    # Wartungen
    cursor_immi.execute(
        "SELECT * FROM wartung WHERE bst_nr = ? AND anl_nr = ? ORDER BY geplant_datum DESC LIMIT 20", (bst_nr, anl_nr)
    )
    ...
    # Messreihen
    cursor_immi.execute("SELECT * FROM messreihen WHERE bst_nr = ? AND anl_nr = ?", (bst_nr, anl_nr))
...
if __name__ == "__main__":
    ...
    uvicorn.run("immissionsschutz_test_server:app", host="0.0.0.0", port=5001, reload=False, log_level="info")
