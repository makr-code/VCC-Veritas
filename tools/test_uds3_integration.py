...
from backend.database.uds3_integration import get_status, get_uds3_client, graph_query, hybrid_search, vector_search
...
def test_document_retrieval(uds3):
    ...
    try:
        ...
        results = uds3.search_documents(
            selector={"domain": {"$exists": True}}, fields=["_id", "title", "domain", "date"], limit=5
        )
...
def test_hybrid_search(uds3):
    ...
    try:
        results = hybrid_search(query="environmental regulations", top_k=5, domains=["environmental"])
