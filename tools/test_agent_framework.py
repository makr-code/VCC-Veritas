...
class TestDataRetrievalAgent(BaseAgent):
    ...
    def execute_step(self, step: dict, context: dict) -> dict:
        ...
        if step_type == "data_retrieval":
            ...
            result = {
                "status": "success",
                "data": {"analysis": "Test analysis completed", "metrics": {"accuracy": 0.94, "completeness": 0.88}},
            ...
            result = {
                "status": "success",
                "data": {"synthesis": "Test synthesis of findings", "key_points": ["Point 1", "Point 2", "Point 3"]},
    ...
    steps = [
        {
            ...
            "status": "pending",
            "step_config": json.dumps({"databases": ["chromadb"], "max_results": 10, "use_hybrid_search": True}),
        },
        {
            "step_id": f"{plan_id}_step_002",
            "plan_id": plan_id,
            ...
            "agent_type": "TestDataRetrievalAgent",
            "status": "pending",
            "step_config": json.dumps({"metrics": ["PM2.5", "PM10", "NO2"], "threshold_check": True}),
        ...
        },
        {
            ...
            "agent_type": "TestDataRetrievalAgent",
            "status": "pending",
            "step_config": json.dumps({"format": "summary", "include_recommendations": True}),
...
def main():
    ...
    agent = TestDataRetrievalAgent(agent_id="test_agent_001", config={"test_mode": True})
    ...
    for step in steps:
        ...
        storage.update_step(step_id, {"status": "running", "started_at": "now"})
        ...
        try:
            ...
            storage.update_step(
                step_id, {"status": "completed", "completed_at": "now", "result": json.dumps(result.get("data", {}))}
            )
            ...
            storage.update_step(step_id, {"status": "failed", "error_message": str(e)})

    # Complete plan
    storage.update_plan(plan["plan_id"], {"status": "completed", "progress_percentage": 100.0})
