...
class DragDropTestApp:
    ...
    def _setup_ui(self):
        """Setup test UI"""
        # Title
        title = tk.Label(self.root, text="🖱️ VERITAS Drag & Drop Test", font=("Segoe UI", 16, "bold"), pady=10)
        title.pack()

        # Instructions
        instructions = tk.Label(
            self.root, text="📂 Drag & Drop Dateien hier her (max 10, max 50 MB pro Datei)", font=("Segoe UI", 10), fg="#666"
        )
        instructions.pack()
        ...
        self.drop_zone = tk.Text(drop_frame, font=("Consolas", 10), bg="#F5F5F5", wrap=tk.WORD, padx=10, pady=10)
...
""".strip(),
        ...
        self.info_label = tk.Label(self.info_frame, text="Keine Dateien gedroppt", font=("Segoe UI", 9), fg="#666", anchor="w")
        self.info_label.pack(side=tk.LEFT, fill=tk.X, expand=True)

        # Clear Button
        self.clear_btn = tk.Button(self.info_frame, text="❌ Clear", command=self._clear_files, font=("Segoe UI", 9))
        self.clear_btn.pack(side=tk.RIGHT, padx=5)

        # Show Formats Button
        self.formats_btn = tk.Button(
            self.info_frame, text="📋 Zeige alle Formate", command=self._show_formats, font=("Segoe UI", 9)
        )
    ...
    def _setup_drag_drop(self):
        ...
        try:
            self.dnd_handler = DragDropHandler(target_widget=self.drop_zone, on_files_dropped=self._on_files_dropped)
    ...
    def _on_files_dropped(self, files):
        ...
        self.info_label.config(text=f"✅ {total_count} Datei(en) ({total_mb:.2f} MB total)")
    ...
    def _update_display(self, new_files):
        ...
        for i, file in enumerate(new_files, 1):
            # File header
            self.drop_zone.insert(tk.END, f"📎 Datei {i}/{len(new_files)}: {file['name']}\n", "file_header")

            # Details
            self.drop_zone.insert(tk.END, f"   📏 Größe: {file['size_mb']:.2f} MB ({file['size']:,} Bytes)\n")
            self.drop_zone.insert(tk.END, f"   📄 Typ: {file['mime_type']}\n")
            self.drop_zone.insert(tk.END, f"   🔗 Pfad: {file['path']}\n")
            self.drop_zone.insert(tk.END, f"   🔐 Hash: {file['hash'][:16]}...\n")
            self.drop_zone.insert(tk.END, "\n")
    ...
    def _show_formats(self):
        ...
        close_btn = tk.Button(popup, text="Schließen", command=popup.destroy, font=("Segoe UI", 9))
