...
class StreamingAgentTester:
    ...
    async def test_backend_health(self) -> bool:
        ...
        try:
            async with self.session.get(f"{BACKEND_URL}/health", timeout=10) as response:
                ...
                self.test_results["health"] = {"passed": all_passed, "details": health}
    ...
    async def test_agent_registry(self) -> bool:
        ...
        try:
            async with self.session.get(f"{BACKEND_URL}/capabilities", timeout=10) as response:
                ...
                details_lines = [f"Gefunden: {len(found_agents)}/14 Agents", f"Registry-Count: {len(agents)} Agents"]
                ...
                env_agents = [
                    ...
                    if "Environmental" in a
                    or "Atmospheric" in a
                    or "Immissionsschutz" in a
                    or "Boden" in a
                    or "Naturschutz" in a
                    or "Emissionen" in a
                ]
                ...
                data_agents = [
                    a for a in found_agents if "Data" in a or "Database" in a or "Wikipedia" in a or "Technical" in a
                ]
                        ...
                        try:
                            ...
                            if event_str.startswith("data: "):
                                ...
                                if event_type == "agent_complete":  # Changed from 'AGENT_COMPLETE'
                                    ...
                                    event_type == "stage_complete" and data.get("stage") == "completed"
                                ):  # Changed from 'STREAM_COMPLETE'
    ...
    async def test_streaming_multi_agent_query(self) -> bool:
        ...
        try:
            ...
            async with self.session.get(
                        ...
                        try:
                            ...
                            if event_str.startswith("data: "):
                                ...
                                if event_type == "agent_complete":  # Changed from 'AGENT_COMPLETE'
                                    ...
                                    event_type == "stage_start" or event_type == "stage_progress"
                                ):  # Changed from 'STAGE_UPDATE'
                ...
                passed = (
                    len(agent_results) >= 3
                    and pipeline_used  # Mindestens 3 Agents
                    and len(found_relevant) >= 2  # Pipeline wurde genutzt
                    and final_response is not None  # Mindestens 2 relevante Agents
                )
    ...
    async def test_agent_domain_coverage(self) -> bool:
        ...
        self.test_results["domain_coverage"] = {"passed": passed, "results": results, "score": f"{sum(results)}/4"}
    ...
    async def test_pipeline_performance(self) -> bool:
        ...
        try:
            ...
            async with self.session.get(
                f"{BACKEND_URL}{stream_url}", headers={"Accept": "text/event-stream"}, timeout=aiohttp.ClientTimeout(total=30)
            ) as stream_response:
                if stream_response.status != 200:
                ...
                passed = (
                    total_time < 30
                    and first_response_time < 10  # Unter 30 Sekunden total
                    and agent_count >= 1  # Erster Agent unter 10 Sekunden  # Mindestens 1 Agent
                )
