...
class TestRerankerServiceIntegration:
    ...
    def test_reranking_with_mock_llm(self, sample_documents):
        ...
        with patch("backend.services.reranker_service.DirectOllamaLLM") as MockLLM:
            ...
            reranker = RerankerService(model_name="llama3.1:8b", scoring_mode=ScoringMode.COMBINED)

            results = reranker.rerank(query="Wie beantrage ich eine Baugenehmigung?", documents=sample_documents, top_k=5)
    ...
    def test_score_improvements_validation(self, sample_documents):
        ...
        with patch("backend.services.reranker_service.DirectOllamaLLM") as MockLLM:
            ...
            results = reranker.rerank(query="Baugenehmigung beantragen", documents=sample_documents)
    ...
    def test_batch_processing_5_docs(self, sample_documents):
        ...
        with patch("backend.services.reranker_service.DirectOllamaLLM") as MockLLM:
            ...
            reranker = RerankerService()
            reranker.rerank(query="test query", documents=sample_documents, batch_size=5)
    ...
    def test_batch_processing_10_docs(self, large_document_set):
        ...
        with patch("backend.services.reranker_service.DirectOllamaLLM") as MockLLM:
            ...
            reranker.rerank(query="test query", documents=docs_10, batch_size=5)
    ...
    def test_batch_processing_20_docs(self, large_document_set):
        ...
        with patch("backend.services.reranker_service.DirectOllamaLLM") as MockLLM:
            ...
            reranker.rerank(query="test query", documents=large_document_set, batch_size=5)
    ...
    def test_fallback_on_llm_timeout(self, sample_documents):
        ...
        with patch("backend.services.reranker_service.DirectOllamaLLM") as MockLLM:
            ...
            reranker = RerankerService()
            results = reranker.rerank(query="test query", documents=sample_documents)
    ...
    def test_latency_under_2s_for_10_docs(self, large_document_set):
        ...
        with patch("backend.services.reranker_service.DirectOllamaLLM") as MockLLM:
            ...
            reranker.rerank(query="test query", documents=docs_10, batch_size=5)  # 2 batches * 200ms = 400ms
    ...
    def test_real_llm_reranking(self, sample_documents):
        ...
        try:
            reranker = RerankerService(model_name="llama3.1:8b", scoring_mode=ScoringMode.COMBINED, temperature=0.1)

            start_time = time.time()
            results = reranker.rerank(
                query="Wie beantrage ich eine Baugenehmigung in Stuttgart?", documents=sample_documents, top_k=5
            )
            elapsed = time.time() - start_time
    ...
    def test_empty_documents_list(self):
        ...
        reranker = RerankerService()
        results = reranker.rerank(query="test query", documents=[], top_k=5)
    ...
    def test_single_document(self, sample_documents):
        ...
        with patch("backend.services.reranker_service.DirectOllamaLLM") as MockLLM:
            ...
            reranker = RerankerService()
            results = reranker.rerank(query="test query", documents=[sample_documents[0]])
    ...
    def test_top_k_filtering(self, sample_documents):
        ...
        with patch("backend.services.reranker_service.DirectOllamaLLM") as MockLLM:
            ...
            reranker = RerankerService()
            results = reranker.rerank(query="test query", documents=sample_documents, top_k=3)  # Only return top 3
