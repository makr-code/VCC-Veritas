...
def test_query(query: str, model: str = "phi3", agent_mode: str = "intelligent") -> Dict[str, Any]:
    ...
    payload = {"query": query, "model": model, "agent_mode": agent_mode}
...
def main():
    ...
    # Test-Szenarien
    tests = [
        {"name": "Simple Question", "query": "Was ist ein Bauantrag?", "model": "phi3", "expected_budget": "250-500 tokens"},
        {
    ...
    for i, test in enumerate(tests, 1):
        ...
        result = test_query(query=test["query"], model=test["model"])

        results.append({"test": test["name"], "result": result, "expected": test["expected_budget"]})
