...
def create_test_window():
    ...
    # Chat-Text-Widget
    chat_text = tk.Text(root, wrap=tk.WORD, font=("Segoe UI", 10), bg="#ffffff", relief=tk.FLAT, padx=10, pady=10)
    ...
    formatter = ChatDisplayFormatter(
        text_widget=chat_text, parent_window=root, markdown_renderer=None, source_link_handler=None  # Für Test ohne Markdown
    )
...
Die detaillierten Analysen finden Sie in den Quelldokumenten."""
    ...
    metadata = {"confidence": 92, "duration": 2.3, "sources_count": 5, "agents_count": 3}
...
def test_placeholder_animation(formatter):
    ...
    def replace_after_delay():
        ...
        formatter.replace_placeholder_with_response(
            ...
            metadata={"confidence": 88, "duration": 3.2, "sources_count": 2, "agents_count": 1},
        )
...
def test_low_confidence_message(formatter):
    ...
    formatter._render_assistant_message_structured(
        ...
        metadata={"confidence": 45, "duration": 1.1, "sources_count": 1, "agents_count": 1},
...
def test_multiple_messages(formatter):
    ...
    for i in range(5):
        # User-Message
        formatter._render_user_message(
            content=f"Frage {i+1}: Wie funktioniert Feature X?", timestamp_short=f"Heute 14:{30+i}", attachments=[]
        )
