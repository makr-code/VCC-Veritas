...
class TestHybridSearchPipelineE2E:
    ...
    @pytest.mark.asyncio
    async def test_e2e_hybrid_search_flow_mocked(self, mock_uds3, comprehensive_search_results):
        ...
        with patch("backend.services.query_service.RAGService") as MockRAGService, patch(
            ...
            mock_reranker_instance.rerank.return_value = [
                Mock(
                    ...
                    reranked_score=r.relevance_score + 0.03,
                    score_delta=0.03,
                    confidence=0.95,
                    reasoning="Relevant",
                )
    ...
    @pytest.mark.asyncio
    async def test_verwaltung_query_bauantrag(self, mock_uds3, comprehensive_search_results):
        ...
        with patch("backend.services.query_service.RAGService") as MockRAGService:
            ...
            relevant_count = sum(1 for title in titles if any(kw in title for kw in baurecht_keywords))
            assert relevant_count >= 1, "Expected at least 1 Baurecht-related result"

    @pytest.mark.asyncio
    async def test_verwaltung_query_bimschg(self, mock_uds3, comprehensive_search_results):
        """Test with real BImSchG query"""

        # Filter to Umweltrecht
        umwelt_results = [
            r for r in comprehensive_search_results if "Umwelt" in r.metadata.custom_fields.get("rechtsgebiet", "")
        ]
    ...
    @pytest.mark.asyncio
    async def test_e2e_latency_under_10s(self, mock_uds3, comprehensive_search_results):
        ...
        with patch("backend.services.query_service.RAGService") as MockRAGService, patch(
            ...
            request = UnifiedQueryRequest(query="Test latency", mode=QueryMode.HYBRID, max_results=10, enable_reranking=True)
    ...
    @pytest.mark.asyncio
    async def test_ieee_citation_format(self, mock_uds3, comprehensive_search_results):
        ...
        with patch("backend.services.query_service.RAGService") as MockRAGService:
            ...
            request = UnifiedQueryRequest(query="Test", mode=QueryMode.HYBRID, max_results=10, enable_reranking=False)
    ...
    async def test_e2e_with_real_services(self, real_verwaltung_queries):
        ...
        try:
            ...
            for query_data in real_verwaltung_queries:
                request = UnifiedQueryRequest(
                    query=query_data["query"], mode=QueryMode.HYBRID, max_results=10, enable_reranking=True
                )
