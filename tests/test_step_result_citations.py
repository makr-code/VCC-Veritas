...
def test_step_result_with_citations():
    ...
    doc1 = create_mock_document(
        "doc_1", "Bauantragsverfahren Baden-Württemberg", "Ein Bauantrag in Stuttgart erfordert...", relevance=0.92
    )

    doc2 = create_mock_document(
        "doc_2", "Einfamilienhaus Genehmigung", "Die Genehmigung für ein Einfamilienhaus...", relevance=0.85
    )
    ...
    result = StepResult(
        success=True,
        data={"result": "Found information about building permits", "documents_retrieved": 2},
        execution_time=0.5,
        source_citations=citations,
        metadata={"agent_mode": "test", "rag_enabled": True},
    )
