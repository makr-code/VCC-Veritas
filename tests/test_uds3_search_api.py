...
from uds3.uds3_search_api import SearchQuery, SearchResult, SearchType, UDS3SearchAPI, create_search_api
    ...
    def search_similar(self, query_vector: List[float], n_results: int, collection: str = None) -> List[Dict]:
        ...
        return [
            ...
            },
            {
                "id": "doc_vector_2",
                "distance": 0.4,
                "metadata": {"name": "Test Document 2", "content": "Another test document", "document_type": "guideline"},
            },
        ]
...
class MockGraphBackend:
    ...
    def execute_query(self, cypher: str, params: Dict = None) -> List[Dict]:
        ...
        return [
            ...
            {
                "d": MockNode(
                    {"document_id": "doc_graph_2", "name": "Energiegesetz BW", "content": "Energy law", "document_type": "law"}
                ),
                "related_docs": [],
            },
        ]
...
def test_search_result_creation():
    ...
    result = SearchResult(
        document_id="test_doc", content="Test content", metadata={"type": "test"}, score=0.85, source="vector"
    )
...
def test_search_query_weight_normalization():
    ...
    query = SearchQuery(query_text="test", weights={"vector": 0.6, "graph": 0.3, "keyword": 0.3})  # Sum = 1.2
...
async def test_hybrid_search_basic(search_api):
    ...
    query = SearchQuery(
        query_text="Photovoltaik", top_k=10, search_types=["vector", "graph"], weights={"vector": 0.5, "graph": 0.5}
    )
...
async def test_hybrid_search_weighted_scoring(search_api):
    ...
    query = SearchQuery(query_text="test", top_k=10, search_types=["vector", "graph"], weights={"vector": 0.7, "graph": 0.3})
...
async def test_hybrid_search_deduplication(search_api):
    ...
    query = SearchQuery(query_text="test", top_k=10, search_types=["vector", "graph"])
...
async def test_hybrid_search_top_k_limit(search_api):
    ...
    query = SearchQuery(query_text="test", top_k=2, search_types=["vector", "graph"])  # Limit to 2 results
...
async def test_empty_query(search_api):
    ...
    query = SearchQuery(query_text="", top_k=10)
...
async def test_zero_weights(search_api):
    ...
    query = SearchQuery(
        query_text="test", top_k=10, search_types=["vector"], weights={"vector": 0.0, "graph": 0.0}  # All zero
    )
