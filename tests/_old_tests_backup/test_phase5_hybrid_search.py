...
try:
    from backend.agents.veritas_hybrid_retrieval import HybridResult, HybridRetrievalConfig, HybridRetriever
    from backend.agents.veritas_query_expansion import ExpandedQuery, ExpansionStrategy, QueryExpander, QueryExpansionConfig
...
def sparse_retriever(sample_corpus):
    ...
    config = SparseRetrievalConfig(k1=1.5, b=0.75, top_k=5, enable_cache=False)  # Disable für Tests
...
class TestSparseRetrieval:
    ...
    async def test_multi_query_retrieval(self, sparse_retriever):
        ...
        queries = ["BGB", "DIN", "UVP"]
        results = await sparse_retriever.retrieve_multi_query(queries, aggregation="max", top_k=5)
...
class TestReciprocalRankFusion:
    ...
    def test_fusion_with_weights(self):
        ...
        config = RRFConfig(k=60, top_k=10, weights={"dense": 0.7, "sparse": 0.3})
        rrf = ReciprocalRankFusion(config)
...
class TestHybridRetrieval:
    ...
    async def test_initialization(self, mock_dense_retriever, sample_corpus):
        ...
        config = HybridRetrievalConfig(
            dense_top_k=10, sparse_top_k=10, final_top_k=5, enable_sparse=True, enable_query_expansion=False
        )

        retriever = HybridRetriever(dense_retriever=mock_dense_retriever, config=config)
    ...
    async def test_hybrid_retrieval(self, mock_dense_retriever, sample_corpus):
        ...
        config = HybridRetrievalConfig(enable_sparse=True, enable_query_expansion=False)
    ...
    async def test_dense_only_fallback(self, mock_dense_retriever):
        ...
        config = HybridRetrievalConfig(enable_sparse=False, enable_query_expansion=False)
    ...
    async def test_get_stats(self, mock_dense_retriever, sample_corpus):
        ...
        retriever = HybridRetriever(dense_retriever=mock_dense_retriever, config=HybridRetrievalConfig())
        retriever.sparse_retriever.index_documents(sample_corpus)
...
class TestQueryExpansion:
    ...
    def test_initialization(self):
        ...
        config = QueryExpansionConfig(model="llama3.2:3b", num_expansions=2, temperature=0.7)
    ...
    async def test_expand_without_ollama(self):
        ...
        config = QueryExpansionConfig(num_expansions=2, fallback_to_original=True)
    ...
    def test_cache_key_generation(self):
        ...
        key1 = expander._get_cache_key("Test Query", 2, [ExpansionStrategy.SYNONYM, ExpansionStrategy.CONTEXT])

        key2 = expander._get_cache_key("Test Query", 2, [ExpansionStrategy.SYNONYM, ExpansionStrategy.CONTEXT])

        # Gleiche Parameter → gleicher Key
        assert key1 == key2

        key3 = expander._get_cache_key("Different Query", 2, [ExpansionStrategy.SYNONYM, ExpansionStrategy.CONTEXT])
...
class TestPerformance:
    ...
    async def test_hybrid_retrieval_latency(self, mock_dense_retriever, sample_corpus):
        ...
        retriever = HybridRetriever(
            dense_retriever=mock_dense_retriever, config=HybridRetrievalConfig(enable_query_expansion=False)
        )
        retriever.sparse_retriever.index_documents(sample_corpus)
