    ...
    try:
        ...
        for query in test_queries:
            ...
            if result.get("success") and len(result.get("results", [])) > 0:
                print_result(
                    "PASS",
                    f"'{query[:50]}...' → {len(result['results'])} Ergebnisse (Konfidenz: {result.get('confidence', 0):.2f})",
                )
                passed += 1
            else:
...
def test_multi_agent_scenario():
    ...
    try:
        ...
        if (
            verwaltungsrecht_agent
            and len(result1.get("results", [])) > 0
            and immissionsschutz_agent
            and len(result2.get("results", [])) > 0
        ):
            print_result("PASS", "Multi-Agent Koordination erfolgreich!")
            print_result(
                "INFO",
                f"Gesamt: {len(result1.get('results', []))} + {len(result2.get('results', []))} = {len(result1.get('results', [])) + len(result2.get('results', []))} Ergebnisse",
            )
            return True
        else:
...
def test_performance():
    ...
    try:
        ...
        agents = ["VerwaltungsrechtAgent", "RechtsrecherchAgent", "ImmissionsschutzAgent"]
