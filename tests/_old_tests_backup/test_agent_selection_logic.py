...
from backend.agents.agent_registry import AgentDomain, AgentRegistry, get_agent_registry, reset_agent_registry
...
def test_domain_based_selection():
    ...
    test_cases = [
        ...
        {"domain": AgentDomain.KNOWLEDGE, "expected_count": 1, "expected_agents": ["WikipediaAgent"]},  # WikipediaAgent
        {
            "domain": AgentDomain.ATMOSPHERIC,
            "expected_count": 1,  # AtmosphericFlowAgent
            "expected_agents": ["AtmosphericFlowAgent"],
        },
        {"domain": AgentDomain.DATABASE, "expected_count": 1, "expected_agents": ["DatabaseAgent"]},  # DatabaseAgent
    ]
    ...
    for test in test_cases:
        ...
        results.append({"domain": domain.value, "passed": passed, "expected": test["expected_agents"], "found": agents})
...
def test_priority_scoring():
    ...
    # Ergebnis
    sorted_agents = sorted(priority_map.items(), key=lambda x: x[1], reverse=True)
