...
from backend.services.rag_service import RAGService, RankingStrategy, SearchFilters, SearchWeights
...
def test_hybrid_search_ranking(rag_service):
    ...
    result_rrf = rag_service.hybrid_search(query, ranking_strategy=RankingStrategy.RECIPROCAL_RANK_FUSION)
...
def test_document_deduplication(mock_documents):
    ...
    dup_doc = create_mock_document(
        "doc_1", "Duplicate Document", "This is a duplicate", relevance=0.75  # Same ID as first document
    )
...
def test_query_reformulation(executor_with_rag):
    ...
    for step_type, desc, expected_prefix in test_cases:
        step = ProcessStep(id="test", name="Test Step", step_type=step_type, description=desc)
