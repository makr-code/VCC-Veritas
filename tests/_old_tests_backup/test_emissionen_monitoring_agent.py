...
def test_fallback_query():
    ...
    if result and len(result) > 0:
        print_result(
            "PASS", f"Fallback-Query 'TA Luft' → {len(result)} Ergebnisse (Konfidenz: {result[0].get('confidence', 0)})"
        )
        return True
    else:
...
def main():
    ...
    tests = [test_initialization, test_query_capabilities, test_fallback_query, test_get_info, test_search_methods]
