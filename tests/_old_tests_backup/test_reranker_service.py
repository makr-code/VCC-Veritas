...
from services.reranker_service import RerankerService, RerankingResult, ScoringMode
...
def test_rerank_batch_processing(reranker_service):
    ...
    # Create 20 documents
    documents = [
        {"document_id": f"doc{i}", "content": f"Document content {i}", "relevance_score": 0.5 + (i * 0.01)} for i in range(20)
    ]
