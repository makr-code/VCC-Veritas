...
try:
    ...
    from backend.agents.veritas_hybrid_retrieval import HybridRetrievalConfig, HybridRetriever
    from backend.agents.veritas_query_expansion import MultiQueryGenerator, QueryExpander
...
class TestFullPipeline:
    ...
    async def test_hybrid_search_pipeline(self, rag_service):
        ...
        options = RAGQueryOptions(limit_documents=5, enable_hybrid_search=True, enable_reranking=False, hybrid_final_top_k=10)

        result = await rag_service.build_context(query_text="Wie baue ich ein barrierefreies Haus?", options=options)
    ...
    async def test_dense_only_baseline(self, rag_service):
        ...
        options = RAGQueryOptions(limit_documents=5, enable_hybrid_search=False, enable_reranking=False)  # Nur Dense
    ...
    async def test_performance_comparison(self, rag_service):
        ...
        result_dense = await rag_service.build_context(
            query, options=RAGQueryOptions(enable_hybrid_search=False, enable_reranking=False)
        )
        ...
        result_hybrid = await rag_service.build_context(
            query, options=RAGQueryOptions(enable_hybrid_search=True, enable_reranking=False)
        )
...
class TestRealWorldQueries:
    ...
    async def test_legal_query(self, rag_service):
        ...
        result = await rag_service.build_context(
            query_text="§ 242 BGB Treu und Glauben", options=RAGQueryOptions(enable_hybrid_search=True, enable_reranking=False)
        )
    ...
    async def test_complex_multi_topic_query(self, rag_service):
        ...
        result = await rag_service.build_context(
            ...
            options=RAGQueryOptions(enable_hybrid_search=True, enable_reranking=False, limit_documents=10),
        )
...
class TestEdgeCases:
    ...
    async def test_empty_query(self, rag_service):
        """Test: Leere Query."""
        result = await rag_service.build_context(
            query_text="", options=RAGQueryOptions(enable_hybrid_search=True, enable_reranking=False)
        )
    ...
    async def test_very_long_query(self, rag_service):
        ...
        result = await rag_service.build_context(
            query_text=long_query, options=RAGQueryOptions(enable_hybrid_search=True, enable_reranking=False)
        )
...
class TestABComparison:
    ...
    async def test_recall_comparison(self, rag_service):
        ...
        # Dense-Only
        result_dense = await rag_service.build_context(
            query, options=RAGQueryOptions(enable_hybrid_search=False, enable_reranking=False, limit_documents=10)
        )

        # Hybrid
        result_hybrid = await rag_service.build_context(
            query, options=RAGQueryOptions(enable_hybrid_search=True, enable_reranking=False, limit_documents=10)
        )
    ...
    async def test_latency_overhead(self, rag_service):
        ...
        for _ in range(3):
            start = time.time()
            await rag_service.build_context(query, options=RAGQueryOptions(enable_hybrid_search=True, enable_reranking=False))
