...
for i, test_case in enumerate(test_cases, 1):
    ...
    try:
        ...
        print(f"   Intent: {nlp_result.intent.intent_type.value} " f"(confidence: {nlp_result.intent.confidence:.1%})")
        ...
        print(
            f"   ✅ Intent matches expected"
            if intent_match
            else f"   ⚠️  Intent mismatch (expected: {test_case['expected_intent']})"
        )
        ...
        print(
            f"   ✅ Step count matches expected"
            if steps_match
            else f"   ⚠️  Step count mismatch (expected: {test_case['expected_steps']})"
        )
        ...
        # Overall test result
        all_passed = intent_match and steps_match and exec_result["success"] and exec_result["steps_failed"] == 0
...
for i, result in enumerate(results_summary, 1):
    ...
    if "tree" in result:
        print(
            f"      Tree: {result['tree']['steps_created']} steps "
    ...
    if "execution" in result:
        print(
            f"      Exec: {result['execution']['steps_completed']}/{result['execution']['steps_completed']} "
