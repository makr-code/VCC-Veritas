...
try:
    ...
    class StreamingUIMixin:
        ...
        def start_streaming_query(self, query, session_id):
            pass
...
class ChatWindowBaseStreaming:
    ...
    def create_gui(self):
        ...
        # Chat-Display
        self.chat_text = scrolledtext.ScrolledText(main_frame, wrap=tk.WORD, height=20, state=tk.DISABLED)
        ...
        # Send-Button
        send_button = ttk.Button(input_frame, text="Senden", command=self._send_message)
        ...
        def api_request():
            try:
                response = requests.post(
                    ...
                    json={"query": text, "session_id": self.session_id, "llm_model": self.selected_llm},
                    timeout=30,
                )
