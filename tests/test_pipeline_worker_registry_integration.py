...
async def test_pipeline_worker_registry_integration():
    ...
    for i, test in enumerate(test_queries, 1):
        ...
        request = IntelligentPipelineRequest(
            query_text=test["query"], user_context={}, enable_supervisor=False, streaming=False  # Worker Registry nutzen
        )

        try:
            # Nur Agent Selection testen (nicht full pipeline)
            # Direkt _step_agent_selection aufrufen
            analysis_context = {"analysis": {"complexity": "standard", "domain": "general"}, "rag": {"documents": []}}
            ...
            print(f"  ERROR: {e}")
            test_results.append({"query": test["query"], "passed": False, "error": str(e)})
...
async def test_worker_registry_statistics():
    ...
    queries = ["Luftqualitaet", "Chemische Stoffe", "DIN Normen", "Wikipedia Suche", "Atmosphaere"]

    print(f"\nFuehre {len(queries)} Queries durch...")

    for query in queries:
        request = IntelligentPipelineRequest(query_text=query, user_context={}, enable_supervisor=False, streaming=False)

        context = {"analysis": {"complexity": "standard", "domain": "general"}, "rag": {"documents": []}}
