    ...
    try:
        ...
        with httpx.Client(verify=ssl_context) as client:
            ...
            if response.status_code == 200:
                ...
                print_result(
                    "Health endpoint accessible without certificate", False, f"Unexpected status code: {response.status_code}"
                )
                return False

    except Exception as e:
        print_result("Health endpoint accessible without certificate", False, f"Error: {e}")
        return False
    ...
    try:
        ...
        with httpx.Client(verify=ssl_context) as client:
            ...
            if response.status_code in [401, 403]:
                print_result(
                    "API endpoint blocks access without certificate", True, f"Status: {response.status_code} (as expected)"
                )
                return True
            else:
                print_result(
        ...
        print_result("API endpoint blocks access without certificate", True, "Connection blocked at TLS level (expected)")
        return True
    except Exception as e:
        print_result("API endpoint blocks access without certificate", False, f"Unexpected error: {e}")
        return False
...
def test_api_endpoint_with_cert():
    ...
    if not CLIENT_CERT.exists():
        print_result("Client certificate exists", False, f"Certificate not found: {CLIENT_CERT}")
        return False

    if not CLIENT_KEY.exists():
        print_result("Client key exists", False, f"Key not found: {CLIENT_KEY}")
        return False
    ...
    try:
        ...
        with httpx.Client(verify=ssl_context) as client:
            ...
            if response.status_code == 200:
                data = response.json()
                print_result(
                    "API endpoint accessible with certificate", True, f"Service: {data.get('client', {}).get('service')}"
                )
                ...
                if "client" in data and "service" in data["client"]:
                    ...
                    print_result("Response structure valid", False, "Missing expected fields")
                    return False
            else:
                print_result("API endpoint accessible with certificate", False, f"Status: {response.status_code}")
                return False

    except Exception as e:
        print_result("API endpoint accessible with certificate", False, f"Error: {e}")
        return False
...
def test_certificate_info():
    ...
    try:
        ...
        with httpx.Client(verify=ssl_context) as client:
            ...
            if response.status_code == 200:
                data = response.json()
                print_result("Certificate info endpoint accessible", True, f"CN: {data.get('subject', {}).get('common_name')}")
                ...
                print_result("Certificate info endpoint accessible", False, f"Status: {response.status_code}")
                return False

    except Exception as e:
        print_result("Certificate info endpoint accessible", False, f"Error: {e}")
        return False
        ...
        with httpx.Client(verify=ssl_context) as client:
            ...
            if response.status_code == 200:
                ...
                if data.get("authenticated"):
                    print_result("WhoAmI returns authenticated status", True, f"Service: {data.get('service')}")
                    print(f"         Method: {data.get('authentication_method')}")
                    return True
                else:
                    print_result("WhoAmI returns authenticated status", False, "Not authenticated")
                    return False
            else:
                print_result("WhoAmI endpoint accessible", False, f"Status: {response.status_code}")
                return False

    except Exception as e:
        print_result("WhoAmI endpoint accessible", False, f"Error: {e}")
        return False
