...
Beantworte die User-Frage direkt und hilfreich."""
    ...
    def test_user_template_with_json_data(self, template_user):
        ...
        agent_results = {"BImSchG-Agent": {"result": "Das BImSchG regelt Immissionsschutz", "confidence": 0.95}}

        rag_context = {"sources": ["BImSchG § 1", "BImSchG § 22"], "text": "Zweck des Gesetzes ist es..."}
...
Query: {query}"""
    ...
    def test_json_dumps_in_format(self):
        ...
        agent_data = {"agent1": {"metadata": {"key": "value"}, "response": "Text with {curly} braces"}}

        try:
            result = template.format(agent_results=json.dumps(agent_data, indent=2, ensure_ascii=False), query="Test")
...
def run_tests():
    ...
    exit_code = pytest.main([__file__, "-v", "--tb=short", "--color=yes", "-W", "ignore::DeprecationWarning"])
