...
from backend.services.rag_service import RAGService, RankingStrategy, SearchWeights
...
def test_chromadb_connection():
    ...
    try:
        ...
        chromadb = ChromaDBRemoteBackend(host="192.168.178.94", port=8000)
        chromadb.connect()
...
def test_neo4j_connection():
    ...
    try:
        ...
        neo4j = UDS3RelationsCore(uri="bolt://192.168.178.94:7687", user="neo4j", password="neo4j")
...
def test_postgresql_connection():
    ...
    try:
        ...
        postgres = PostgreSQLRelationalBackend(
            host="192.168.178.94", port=5432, user="postgres", password="postgres", database="postgres", schema="public"
        )
        postgres.connect()
...
def test_vector_search(rag: RAGService):
    ...
    try:
        ...
        results = rag.vector_search(query="Bauantrag für Einfamilienhaus", top_k=3)
...
def test_graph_search(rag: RAGService):
    ...
    try:
        ...
        start_time = time.time()
        results = rag.graph_search(query="GmbH Gründung", top_k=3)
        execution_time = (time.time() - start_time) * 1000  # ms
...
def test_relational_search(rag: RAGService):
    ...
    try:
        ...
        start_time = time.time()
        results = rag.relational_search(query="Stuttgart Bauamt", top_k=3)
        execution_time = (time.time() - start_time) * 1000  # ms
...
def test_hybrid_search(rag: RAGService):
    ...
    try:
        ...
        result = rag.hybrid_search(
            ...
            weights=SearchWeights(vector_weight=0.5, graph_weight=0.3, relational_weight=0.2),
...
def test_performance_benchmarks(rag: RAGService):
    ...
    results = {"vector": [], "graph": [], "relational": [], "hybrid": []}
