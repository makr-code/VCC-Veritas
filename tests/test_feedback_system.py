...
async def test_async_submit_feedback():
    ...
    async with FeedbackAPIClient(base_url=BACKEND_URL) as client:
        ...
        response3 = await client.submit_feedback(
            message_id="test_msg_003", rating=0, category="unclear", comment="Could be more concise", user_id="test_user_2"
        )
...
def test_sync_submit_feedback():
    ...
    response = client.submit_feedback(
        message_id="test_msg_sync_001", rating=1, category="helpful", comment="Sync test successful!", user_id="sync_user_1"
    )
