...
class TestThemisDBConfig:
    ...
    def test_custom_config(self):
        ...
        config = ThemisDBConfig(
            host="themis.internal", port=9000, use_ssl=True, api_token="test-token-123", timeout=60, max_retries=5
        )
...
class TestThemisDBAdapter:
    ...
    async def test_vector_search_success(self, adapter):
        ...
        with patch.object(adapter, "_embed", new_callable=AsyncMock) as mock_embed:
            ...
            mock_response.json.return_value = {
                "results": [
                    {"id": "doc1", "document": {"content": "BGB §1 content", "metadata": {"year": 2020}}, "score": 0.95},
                    {"id": "doc2", "document": {"content": "BGB §2 content", "metadata": {"year": 2021}}, "score": 0.87},
                ]
            }
            ...
            with patch.object(adapter.client, "post", new_callable=AsyncMock) as mock_post:
                ...
                results = await adapter.vector_search(query="BGB Vertragsrecht", top_k=2)
    ...
    async def test_graph_traverse(self, adapter):
        ...
        mock_response = MagicMock()
        mock_response.json.return_value = {
            "paths": [{"vertices": ["doc1", "doc2"], "edges": ["cites"]}, {"vertices": ["doc1", "doc3"], "edges": ["cites"]}]
        }
        ...
        with patch.object(adapter.client, "post", new_callable=AsyncMock) as mock_post:
            ...
            results = await adapter.graph_traverse(
                start_vertex="documents/doc1", edge_collection="citations", direction="outbound", max_depth=2
            )
    ...
    async def test_execute_aql(self, adapter):
        ...
        mock_response = MagicMock()
        mock_response.json.return_value = {
            "result": [{"_key": "doc1", "title": "Document 1"}, {"_key": "doc2", "title": "Document 2"}]
        }
        ...
        with patch.object(adapter.client, "post", new_callable=AsyncMock) as mock_post:
            ...
            results = await adapter.execute_aql(
                query="FOR doc IN documents FILTER doc.year >= @year RETURN doc", bind_vars={"year": 2020}
            )
...
class TestAdapterFactory:
    ...
    @patch.dict("os.environ", {"THEMIS_ENABLED": "true", "THEMIS_HOST": "localhost", "USE_UDS3_FALLBACK": "true"})
    ...
    @patch.dict("os.environ", {"THEMIS_ENABLED": "false", "USE_UDS3_FALLBACK": "true"})
    ...
    @patch.dict("os.environ", {"THEMIS_ENABLED": "true", "USE_UDS3_FALLBACK": "true"})
    @patch("backend.adapters.adapter_factory._init_themisdb_adapter")
    @patch("backend.adapters.adapter_factory._init_uds3_adapter")
    ...
    @patch.dict("os.environ", {"THEMIS_ENABLED": "true", "USE_UDS3_FALLBACK": "false"})
    @patch("backend.adapters.adapter_factory._init_themisdb_adapter")
    ...
    async def test_rag_service_with_themis(self):
        ...
        with patch("backend.adapters.adapter_factory._init_themisdb_adapter") as mock_init:
            ...
            mock_adapter.vector_search = AsyncMock(
                return_value=[{"doc_id": "doc1", "content": "Test content", "score": 0.95, "metadata": {}}]
            )
