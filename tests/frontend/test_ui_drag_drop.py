...
class MockDragDropHandler:
    ...
    def _get_all_supported_formats(self):
        ...
        return self.SUPPORTED_DOCUMENTS + self.SUPPORTED_IMAGES + self.SUPPORTED_DATA + self.SUPPORTED_CODE
    ...
    def _validate_files(self, file_paths):
        ...
        for file_path in file_paths:
            ...
            # Valid file
            valid_files.append({"path": str(path), "name": path.name, "size": size, "hash": file_hash})
...
def drag_drop_handler(mock_widget, mock_callback):
    ...
    return MockDragDropHandler(
        target_widget=mock_widget, on_files_dropped=mock_callback, max_file_size=50 * 1024 * 1024, max_files=10
    )
...
def test_handler_initialization(mock_widget, mock_callback):
    ...
    # Act
    handler = MockDragDropHandler(target_widget=mock_widget, on_files_dropped=mock_callback)
...
def test_handler_custom_limits(mock_widget, mock_callback):
    ...
    handler = MockDragDropHandler(
        target_widget=mock_widget, on_files_dropped=mock_callback, max_file_size=10 * 1024 * 1024, max_files=5  # 10 MB
    )
...
def test_validate_file_too_large(drag_drop_handler, temp_export_dir):
    ...
    # Arrange
    handler = MockDragDropHandler(target_widget=MagicMock(), on_files_dropped=MagicMock(), max_file_size=100)  # 100 bytes
...
def test_validate_too_many_files(drag_drop_handler, temp_export_dir):
    ...
    # Arrange
    handler = MockDragDropHandler(target_widget=MagicMock(), on_files_dropped=MagicMock(), max_files=3)
