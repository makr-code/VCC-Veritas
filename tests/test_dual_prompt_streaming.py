...
def test_streaming_with_reflections():
    ...
    try:
        ...
        stream_response = requests.get(
            f"{BACKEND_URL}{stream_url}", stream=True, headers={"Accept": "text/event-stream"}, timeout=120
        )
        ...
        for line in stream_response.iter_lines(decode_unicode=True):
            ...
            try:
                ...
                if event_type == "stage_update":
                    ...
                    print(
                        f"[{timestamp}] {colorize('📍', '1;34')} Stage: {colorize(stage.upper(), '1;34')} | Progress: {progress:.0f}%"
                    )
                ...
                # 🆕 STAGE-REFLECTION (Das neue Feature!)
                    ...
                    print_separator("─")
                    print(
                        f"  Erfüllung: {colorize(f'{completion:.0f}%', '1;33')} | Status: {colorize(status, '1;33')} | Konfidenz: {colorize(f'{confidence:.2f}', '1;33')}"
                    )
                ...
                # Stream Complete
                    ...
                    print(
                        f"  • {colorize(f'Stage-Reflections: {reflection_count}', '1;35')} {colorize('← NEUE FEATURE!', '1;33')}"
                    )
