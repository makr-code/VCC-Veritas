...
def main():
    ...
    parser.add_argument("-k", "--keyword", type=str, help="Run only tests matching keyword")
    parser.add_argument("--json", action="store_true", help="Run only JSON extraction tests")
    ...
    pytest_args = ["tests/", "--color=yes", "-W", "ignore::DeprecationWarning"]
    ...
    if args.coverage:
        pytest_args.extend(["--cov=backend", "--cov-report=term-missing", "--cov-report=html"])
