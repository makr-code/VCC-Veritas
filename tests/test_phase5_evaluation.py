...
try:
    from backend.evaluation.veritas_rag_evaluator import EvaluationResult, RAGEvaluator, RetrievalMetrics
...
class Phase5Evaluator:
    ...
    def _create_test_cases(self) -> List[EvaluationTestCase]:
        ...
        return [
            ...
            EvaluationTestCase(
                ...
                relevance_scores={"bgb_242": 1.0, "baurecht_overview": 0.7, "construction_process": 0.3},
                category="legal",
            ...
            EvaluationTestCase(
                ...
                relevance_scores={"din_18040_1": 1.0, "accessibility_law": 0.9, "sustainable_building": 0.3},
                category="technical",
            ...
            EvaluationTestCase(
                ...
                expected_doc_ids=["din_18040_1", "accessibility_law"],
                relevance_scores={"din_18040_1": 1.0, "accessibility_law": 0.9, "baurecht_overview": 0.4},
                category="technical",
            ...
            EvaluationTestCase(
                ...
                relevance_scores={"uvpg_3a": 1.0, "environmental_impact": 0.8, "sustainable_building": 0.5},
            ...
            EvaluationTestCase(
                ...
                expected_doc_ids=["sustainable_building", "din_18040_1", "uvpg_3a", "environmental_impact", "green_building"],
                relevance_scores={
            ...
            EvaluationTestCase(
                ...
                expected_doc_ids=["energy_efficiency", "sustainable_building", "baurecht_overview", "construction_process"],
                relevance_scores={
    ...
    async def evaluate_configuration(
        self, config_name: str, enable_hybrid: bool, enable_query_expansion: bool
    ) -> BenchmarkResult:
        """
        ...
        for test_case in self.test_cases:
            ...
            start = time.time()
            result = await self.rag_service.build_context(query_text=test_case.query, options=options)
            ...
            # Metriken berechnen
            ndcg = self._calculate_ndcg(retrieved_ids[:10], test_case.relevance_scores, k=10)

            mrr = self._calculate_mrr(retrieved_ids, set(test_case.expected_doc_ids))

            recall = self._calculate_recall(retrieved_ids[:10], set(test_case.expected_doc_ids))

            precision = self._calculate_precision(retrieved_ids[:5], set(test_case.expected_doc_ids))
    ...
    def _calculate_ndcg(self, retrieved_ids: List[str], relevance_scores: Dict[str, float], k: int = 10) -> float:
        """Berechnet NDCG@K."""
    ...
    def _calculate_mrr(self, retrieved_ids: List[str], expected_ids: Set[str]) -> float:
    ...
    def _calculate_recall(self, retrieved_ids: List[str], expected_ids: Set[str]) -> float:
        """Berechnet Recall@K."""
    ...
    def _calculate_precision(self, retrieved_ids: List[str], expected_ids: Set[str]) -> float:
    ...
    def print_comparison(self, results: List[BenchmarkResult]) -> None:
        ...
        for result in results:
            ...
            if baseline and result.configuration != "Baseline":
                ndcg_delta = (
                    ((result.ndcg_at_10 - baseline.ndcg_at_10) / baseline.ndcg_at_10 * 100) if baseline.ndcg_at_10 > 0 else 0
                )
        ...
        if baseline:
            ...
            if hybrid:
                ndcg_improvement = (
                    ((hybrid.ndcg_at_10 - baseline.ndcg_at_10) / baseline.ndcg_at_10 * 100) if baseline.ndcg_at_10 > 0 else 0
                )
            ...
            if hybrid_qe and hybrid:
                ndcg_improvement_qe = (
                    ((hybrid_qe.ndcg_at_10 - hybrid.ndcg_at_10) / hybrid.ndcg_at_10 * 100) if hybrid.ndcg_at_10 > 0 else 0
                )
