    ...
    def test_websocket_search_success(self, client):
        ...
        with patch("backend.adapters.adapter_factory.get_database_adapter") as mock_adapter:
            ...
            mock_adapter.return_value.vector_search = AsyncMock(
                return_value=[
                    {"id": "doc1", "content": "Machine learning tutorial", "score": 0.95},
                    {"id": "doc2", "content": "Deep learning guide", "score": 0.88},
                ]
            )
            ...
            with client.websocket_connect("/api/v3/ws/search?client_id=test_client") as websocket:
                ...
                # Send search query
                websocket.send_json({"action": "search", "query": "machine learning", "top_k": 5, "collection": "documents"})
...
class TestWebSocketAdapterStatus:
    ...
    async def test_websocket_status_updates(self, client):
        ...
        """
        with patch("backend.adapters.adapter_factory.get_database_adapter") as mock_adapter, patch(
            "backend.adapters.adapter_factory.is_themisdb_available"
        ) as mock_themis, patch("backend.adapters.adapter_factory.is_uds3_available") as mock_uds3:
            # Mock Availability
            ...
            mock_adapter_instance.get_stats = MagicMock(
                return_value={"query_count": 100, "avg_latency_ms": 45.2, "success_rate": 0.95}
            )
...
class TestWebSocketLogs:
    ...
    def test_websocket_log_level_change(self, client):
        ...
        with client.websocket_connect("/api/v3/ws/logs?client_id=test&log_level=INFO") as websocket:
            ...
            # Change log level
            websocket.send_json({"action": "change_level", "level": "DEBUG"})
...
class TestWebSocketGraphTraverse:
    ...
    def test_websocket_graph_traversal(self, client):
        ...
        with patch("backend.adapters.adapter_factory.get_database_adapter") as mock_adapter:
            ...
            mock_adapter.return_value = AsyncMock()
            mock_adapter.return_value.graph_traverse = AsyncMock(
                return_value={
                    "vertices": [{"_key": "doc1", "label": "Document"}, {"_key": "doc2", "label": "Document"}],
                    "edges": [{"_from": "doc1", "_to": "doc2", "type": "cites"}],
                }
            )
            ...
            with client.websocket_connect("/api/v3/ws/graph/traverse?client_id=test") as websocket:
                ...
                websocket.send_json(
                    {"action": "traverse", "start_vertex": "doc1", "edge_collection": "citations", "max_depth": 2}
                )
...
class TestWebSocketConnectionManager:
    ...
    def test_multiple_connections_same_client(self, client):
        ...
        """
        with client.websocket_connect("/api/v3/ws/search?client_id=multi_client") as ws1, client.websocket_connect(
            "/api/v3/ws/search?client_id=multi_client"
        ) as ws2:
    ...
    def test_connection_count_endpoint(self, client):
        ...
        """
        with client.websocket_connect("/api/v3/ws/search?client_id=test1") as ws1, client.websocket_connect(
            "/api/v3/ws/search?client_id=test2"
        ) as ws2:
...
class TestWebSocketErrorHandling:
    ...
    def test_websocket_adapter_error(self, client):
        ...
        with patch("backend.adapters.adapter_factory.get_database_adapter") as mock_adapter:
            ...
            mock_adapter.return_value = AsyncMock()
            mock_adapter.return_value.vector_search = AsyncMock(side_effect=Exception("Database connection failed"))
            ...
            with client.websocket_connect("/api/v3/ws/search?client_id=test") as websocket:
                ...
                # Send search
                websocket.send_json({"action": "search", "query": "test", "top_k": 5})
