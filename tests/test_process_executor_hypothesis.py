...
from models.hypothesis import ConfidenceLevel, Hypothesis, QuestionType
from models.nlp_models import Entity, EntityType, Intent, IntentType, NLPAnalysisResult, QueryParameters
from models.nlp_models import QuestionType as NLPQuestionType
...
def simple_nlp_analysis():
    ...
    return NLPAnalysisResult(
        ...
        intent=Intent(
            intent_type=IntentType.PROCEDURE_QUERY, confidence=0.95, keywords=["Bauantrag", "Einfamilienhaus", "Stuttgart"]
        ),
        entities=[Entity(text="Stuttgart", entity_type=EntityType.LOCATION, start_pos=37, end_pos=46, confidence=0.98)],
        parameters=QueryParameters(location="Stuttgart", document_type="Bauantrag"),
...
def simple_process_tree(simple_nlp_analysis):
    ...
    tree = ProcessTree(
        query="Bauantrag für Einfamilienhaus in Stuttgart", nlp_analysis=simple_nlp_analysis, metadata={"source": "test"}
    )
...
def test_executor_hypothesis_with_rag_context(simple_process_tree, mock_hypothesis):
    ...
    with patch("services.process_executor.HypothesisService") as mock_service_class, patch(
        "services.process_executor.RAGService"
    ) as mock_rag_class:
        # Setup mocks
        mock_service = Mock()
        mock_service.generate_hypothesis.return_value = mock_hypothesis
...
def test_executor_hypothesis_with_low_confidence(simple_process_tree):
    ...
    low_conf_hypothesis = Hypothesis(
        ...
        information_gaps=[Mock(gap_type="location", severity=Mock(value="critical"))],
