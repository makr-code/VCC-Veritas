...
async def test_batch_search_with_filters(rag_service, sample_queries):
    ...
    filters = SearchFilters(max_results=5, min_relevance=0.7)

    results = await rag_service.batch_search(sample_queries, filters=filters)
...
async def test_batch_search_vector_only(rag_service, sample_queries):
    ...
    results = await rag_service.batch_search(sample_queries, search_method=SearchMethod.VECTOR)

    for result in results:
        assert SearchMethod.VECTOR in result.search_methods_used, "Should use vector search method"


@pytest.mark.asyncio
async def test_batch_search_with_weights(rag_service, sample_queries):
    """Test batch search with custom weights"""
    weights = SearchWeights(vector_weight=0.6, graph_weight=0.3, relational_weight=0.1)

    results = await rag_service.batch_search(sample_queries, weights=weights)
...
async def test_batch_search_error_handling(rag_service):
    ...
    problematic_queries = ["Normal query", "", "A" * 10000, "Test query"]  # Empty query  # Very long query
