...
class Phase5Config:
    ...
    def from_environment(cls) -> "Phase5Config":
        ...
        return cls(
            ...
            query_expansion_strategy=ExpansionStrategy(get_str("VERITAS_QUERY_EXPANSION_STRATEGY", "multi_perspective")),
            ...
            # Deployment
            deployment_stage=DeploymentStage(get_str("VERITAS_DEPLOYMENT_STAGE", "development")),
    ...
    def validate(self) -> list[str]:
        ...
        if abs(total_weight - 1.0) > 0.01:
            errors.append(f"Dense + Sparse weights must sum to 1.0, got {total_weight:.2f}")

        # Validate rollout percentage
        if not 0 <= self.rollout_percentage <= 100:
            errors.append(f"Rollout percentage must be 0-100, got {self.rollout_percentage}")
        ...
        if self.query_expansion_variants < 1:
            errors.append(f"query_expansion_variants must be >= 1, got {self.query_expansion_variants}")
...
def get_config() -> Phase5Config:
    ...
    has_env_vars = any(key.startswith("VERITAS_") for key in os.environ.keys())
...
if __name__ == "__main__":
    ...
    print(
        f"   Features: Hybrid={DEVELOPMENT_CONFIG.enable_hybrid_search}, " f"QE={DEVELOPMENT_CONFIG.enable_query_expansion}\n"
    )

    print("2. Staging Phase 1 Config:")
    print(f"   {STAGING_PHASE1_CONFIG}")
    print(
        f"   Features: Hybrid={STAGING_PHASE1_CONFIG.enable_hybrid_search}, "
    ...
    print(
        f"   Features: Hybrid={STAGING_PHASE2_CONFIG.enable_hybrid_search}, "
        f"QE={STAGING_PHASE2_CONFIG.enable_query_expansion}\n"
    )

    print("4. Production Config:")
    print(f"   {PRODUCTION_CONFIG}")
    print(f"   Features: Hybrid={PRODUCTION_CONFIG.enable_hybrid_search}, " f"QE={PRODUCTION_CONFIG.enable_query_expansion}\n")
