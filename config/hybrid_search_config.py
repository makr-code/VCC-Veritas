...
class SearchWeights:
    ...
    def to_dict(self) -> Dict[str, float]:
        ...
        return {"vector": self.vector, "graph": self.graph, "relational": self.relational}

    @classmethod
    def from_dict(cls, data: Dict[str, float]) -> "SearchWeights":
        """Create from dictionary."""
        return cls(vector=data.get("vector", 0.6), graph=data.get("graph", 0.2), relational=data.get("relational", 0.2))
...
def load_config_from_env() -> HybridSearchConfig:
    ...
    try:
        ...
        ranking_strategy = RankingStrategy.from_string(os.getenv("HYBRID_RANKING_STRATEGY", "reciprocal_rank_fusion"))
        ...
        # RRF Configuration
        rrf_config = RRFConfig(k=int(os.getenv("HYBRID_RRF_K", "60")))

        # Re-Ranking Configuration
        reranking_config = ReRankingConfig(
            enabled=os.getenv("RERANKING_ENABLED", "true").lower() == "true",
            model_name=os.getenv("RERANKING_MODEL", "llama3.1:8b"),
            scoring_mode=ScoringMode.from_string(os.getenv("RERANKING_SCORING_MODE", "combined")),
...
def get_preset_config(preset: str) -> HybridSearchConfig:
    ...
    if preset == "balanced":
        ...
        return HybridSearchConfig(
            weights=SearchWeights(vector=0.8, graph=0.1, relational=0.1), ranking_strategy=RankingStrategy.WEIGHTED_COMBINATION
        )
        ...
        return HybridSearchConfig(
            ...
            reranking_config=ReRankingConfig(enabled=True, batch_size=10, scoring_mode=ScoringMode.COMBINED),  # Larger batches
            ranking_strategy=RankingStrategy.RECIPROCAL_RANK_FUSION,
        )
        ...
        return HybridSearchConfig(
            ...
            reranking_config=ReRankingConfig(enabled=True, batch_size=5, timeout_seconds=30, max_retries=2),
            ranking_strategy=RankingStrategy.RECIPROCAL_RANK_FUSION,
        )

    else:
        raise ValueError(
            f"Unknown preset '{preset}'. Available: " "balanced, vector_heavy, graph_heavy, fast, accurate, production"
        )
