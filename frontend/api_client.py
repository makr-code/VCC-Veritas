...
class UnifiedResponse:
    ...
    def from_dict(cls, data: Dict[str, Any]) -> "UnifiedResponse":
        ...
        sources = [SourceMetadata.from_dict(src) for src in data.get("sources", [])]
...
class VeritasAPIClient:
    ...
    def __init__(self, base_url: str = "http://localhost:5000", timeout: int = 60, session_id: Optional[str] = None):
        """
    ...
    def ask(self, query: str, model: str = "llama3.2", temperature: float = 0.7, max_tokens: int = 1000) -> UnifiedResponse:
        """
    ...
    def rag_query(
        self, query: str, model: str = "llama3.2", temperature: float = 0.7, max_tokens: int = 2000, top_k: int = 5
    ) -> UnifiedResponse:
        """
...
def create_api_client(base_url: str = "http://localhost:5000", session_id: Optional[str] = None) -> VeritasAPIClient:
...
if __name__ == "__main__":
    ...
    print("🔍 Test Query...")
    try:
        response = client.query(query="Was regelt das BImSchG?", mode="rag", model="llama3.2")
