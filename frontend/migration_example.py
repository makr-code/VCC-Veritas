...
from frontend.api_client import ResponseMetadata, SourceMetadata, UnifiedResponse, VeritasAPIClient
...
class VeritasApp:
    ...
    def __init__(self, root):
        ...
        self.api_client = VeritasAPIClient(base_url=API_BASE_URL, session_id=self.session_id)  # "http://localhost:5000"
...
def send_message(self, message=None, llm=None):
    ...
    try:
        ...
        self.api_status_label.config(text="✅")
        self.status_var.set(
            f"✅ Antwort erhalten ({unified_response.metadata.duration:.2f}s, " f"{len(unified_response.sources)} Quellen)"
        )
...
def _process_unified_response(self, response: UnifiedResponse):
    ...
    self.add_assistant_message(content=content, sources=sources, metadata=metadata)  # List[SourceMetadata]
...
def _get_conversation_history(self) -> Optional[List[Dict[str, str]]]:
    ...
    try:
        ...
        recent_messages = (
            self.chat_session.messages[-10:] if len(self.chat_session.messages) > 10 else self.chat_session.messages
        )
...
def add_assistant_message(self, content: str, sources: List[SourceMetadata] = None, metadata: ResponseMetadata = None):
    """
...
def _fetch_available_modes(self):
    ...
    return [
        ...
        {"key": "ask", "display": "Simple Ask", "description": "Direct LLM ohne RAG", "features": ["direct_llm", "fast"]},
    ]
