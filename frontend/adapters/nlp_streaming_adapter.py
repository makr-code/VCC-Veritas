...
class NLPStreamingAdapter:
    ...
    def process_query_with_streaming(
        self, query: str, on_complete: Optional[Callable] = None, on_error: Optional[Callable] = None
    ) -> str:
        """
        ...
        # Store session info
        self.active_sessions[session_id] = {"query": query, "started_at": datetime.now().isoformat(), "status": "running"}

        # Display query in UI
        self._queue_ui_update({"type": "text", "text": f"\n{'='*80}\n", "tags": "separator"})
        self._queue_ui_update({"type": "text", "text": f"🔍 Query: {query}\n", "tags": "query"})
        self._queue_ui_update({"type": "text", "text": f"{'='*80}\n\n", "tags": "separator"})

        # Update status
        self._queue_ui_update({"type": "status", "status": f"Processing: {query[:50]}..."})
        ...
        def process_thread():
            try:
                ...
                # Build process tree
                self._queue_ui_update({"type": "text", "text": "📊 Building process tree...\n", "tags": "info"})

                tree = self.process_builder.build_process_tree(query)

                self._queue_ui_update({"type": "text", "text": f"   ✅ {len(tree.steps)} steps planned\n\n", "tags": "success"})

                # Execute with streaming
                self._queue_ui_update({"type": "text", "text": "🚀 Executing process...\n\n", "tags": "info"})
                ...
                # Update status
                self._queue_ui_update({"type": "status", "status": "Ready"})
                ...
                # Update status
                self._queue_ui_update({"type": "status", "status": "Error occurred"})
    ...
    def _on_progress_event(self, event: ProgressEvent):
        ...
        if event_type == "plan_started":
            self._queue_ui_update({"type": "progress", "percentage": 0})
            ...
            # Update progress bar
            self._queue_ui_update({"type": "progress", "percentage": event.percentage})

            # Display progress message
            if event.message:
                self._queue_ui_update(
                    {"type": "text", "text": f"   ⏳ {event.percentage:.0f}%: {event.message}\n", "tags": "progress"}
                )
            ...
            # Reset progress bar
            self._queue_ui_update({"type": "progress", "percentage": 100})

    def _display_result(self, result: Dict[str, Any], session_id: str):
        """Display execution result."""
        self._queue_ui_update({"type": "text", "text": f"\n{'─'*80}\n", "tags": "separator"})

        self._queue_ui_update({"type": "text", "text": "📊 Execution Summary:\n", "tags": "header"})
        ...
        for stat in stats:
            self._queue_ui_update({"type": "text", "text": f"{stat}\n", "tags": "info"})

        # Display final result data
        if result.get("data"):
            self._queue_ui_update({"type": "text", "text": "\n📋 Result Data:\n", "tags": "header"})

            for key, value in result["data"].items():
                self._queue_ui_update({"type": "text", "text": f"   {key}: {value}\n", "tags": "result"})
...
if __name__ == "__main__":
    ...
    # Create adapter
    adapter = NLPStreamingAdapter(text_widget=text_widget, status_label=status_label, progress_bar=progress_bar, root=root)
    ...
    # Info label
    info_label = ttk.Label(frame, text="Enter a query and click 'Send Query' to test NLP streaming", foreground="gray")
