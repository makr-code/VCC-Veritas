...
class BackendAPIClient:
    ...
    def __init__(self, base_url: str = "http://127.0.0.1:5000/api/v3", timeout: int = 60, max_retries: int = 3):
        """
    ...
    def get_capabilities(self) -> Optional[Dict]:
        ...
        """
        try:
            # Backend v4 exposes capabilities under /system/capabilities
    ...
    def get_question_modes(self) -> List[Dict]:
        ...
        """
        try:
            # Backend v4 exposes available modes under /system/modes
            response = requests.get(f"{self.base_url}/system/modes", timeout=10)
    ...
    def send_query(
        ...
        try:
            ...
            response = requests.post(f"{self.base_url}{endpoint}", json=self._request_to_dict(request), timeout=self.timeout)
    ...
    def start_streaming_query(
        ...
        # Ensure session
        if not self.ensure_session():
            return {"success": False, "error": "Session-Erstellung fehlgeschlagen"}
