...
class MarkdownRenderer:
    ...
    def _render_citation(self, part: str) -> bool:
        ...
        if citation_match:
            ...
            self.text_widget.tag_bind(cite_tag, "<Button-1>", lambda e, cid=cite_id: self._scroll_to_source(cid))

            # Hover-Effects: Hand-Cursor + Tooltip
            self.text_widget.tag_bind(cite_tag, "<Enter>", lambda e: self.text_widget.config(cursor="hand2"))
            self.text_widget.tag_bind(cite_tag, "<Leave>", lambda e: self.text_widget.config(cursor=""))
    ...
    def _scroll_to_source(self, source_num: str) -> None:
        ...
        try:
            ...
            self.text_widget.after(2000, lambda: self.text_widget.tag_config(source_tag, background=""))  # 2000ms = 2s
    ...
    def _add_copy_button(self, code_text: str, position: str) -> None:
        ...
        try:
            ...
            copy_btn = tk.Label(self.text_widget, text="📋", font=("Segoe UI", 8), foreground="#666", padx=2, pady=0)
    ...
    def set_link_callback(self, callback):
        ...
        for link_tag, link_url in self.link_handlers.items():
            self.text_widget.tag_bind(link_tag, "<Button-1>", lambda e, url=link_url: callback(url))
    ...
    def parse_rag_response(content: str) -> Dict[str, Any]:
        ...
        """
        sections = {"main_answer": "", "sources": [], "agents": {}, "suggestions": [], "metadata": {}}
    ...
    def render_code_block(self, code: str, language: Optional[str] = None, add_copy_button: bool = True) -> None:
        ...
        if self.syntax_highlighter:
            self.syntax_highlighter.highlight_multiline_block(
                code, language=language, insert_position=tk.END, add_background=True
            )
        else:
...
def setup_markdown_tags(text_widget: tk.Text) -> None:
    ...
    # Code
    text_widget.tag_configure("md_code", font=("Consolas", 9), background="#f0f0f0", foreground="#c7254e")

    # Headings
    text_widget.tag_configure("md_heading1", font=("Segoe UI", 14, "bold"), foreground="#2c3e50")
    text_widget.tag_configure("md_heading2", font=("Segoe UI", 12, "bold"), foreground="#34495e")
    text_widget.tag_configure("md_heading3", font=("Segoe UI", 11, "bold"), foreground="#7f8c8d")

    # List Items
    text_widget.tag_configure("md_list_item", foreground="#555555")

    # Blockquotes
    text_widget.tag_configure("md_quote", font=("Segoe UI", 10, "italic"), foreground="#7f8c8d", lmargin1=20, lmargin2=20)

    # ✨ Feature #2: Tabellen-Tags
    text_widget.tag_configure("table_header", font=("Courier New", 9, "bold"), foreground="#2c3e50")

    text_widget.tag_configure("table_cell", font=("Courier New", 9), foreground="#34495e")

    text_widget.tag_configure("table_cell_alt", font=("Courier New", 9), foreground="#34495e", background="#f9f9f9")

    text_widget.tag_configure("table_border", font=("Courier New", 9), foreground="#95a5a6")
