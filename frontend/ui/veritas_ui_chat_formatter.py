...
try:
    from .veritas_ui_icons import VeritasIcons, format_with_icon, get_source_icon
...
class ChatDisplayFormatter:
        ...
        for idx, msg in enumerate(chat_messages):
            ...
            # === USER MESSAGE ===
                ...
                self._render_user_message(
                    content=content, timestamp_short=timestamp_short, timestamp_full=timestamp_full, attachments=attachments
                )
    ...
    def _render_user_message(
        self, content: str, timestamp_short: str = "", timestamp_full: str = "", attachments: List[Dict] = None
    ) -> None:
        """
    ...
    def _render_assistant_message_structured(
        self, content: str, timestamp_short: str = "", timestamp_full: str = "", metadata: Dict = None, message_id: str = None
    ) -> None:
        """
        ...
        if self.enable_modern_ui and self.assistant_layout:
            ...
            logger.info(
                f"🔍 RAW metadata['sources_metadata']: {metadata.get('sources_metadata', 'NOT FOUND')[:500] if metadata else 'NO METADATA'}"
            )
            ...
            self.assistant_layout.render_assistant_message(
                content=content, metadata=metadata, sources=sources, enable_citations=True  # IEEE-Citations aktiviert
            )
    ...
    def replace_placeholder_with_response(self, message_id: str, content: str, metadata: Dict = None) -> None:
        """
    ...
    def _create_feedback_widget(self, message_id: str) -> tk.Frame:
        ...
        # Haupt-Frame
        feedback_frame = tk.Frame(self.text_widget, bg="#F5F5F5", relief="flat", padx=5, pady=5)

        # Label
        label = tk.Label(feedback_frame, text="War diese Antwort hilfreich?", font=("Segoe UI", 8), bg="#F5F5F5", fg="#666")
    ...
    def _on_feedback_thumbs_up(self, message_id: str, widget: tk.Frame) -> None:
        ...
        thank_you_label = tk.Label(
            widget, text="✓ Danke für Ihr Feedback! 👍", font=("Segoe UI", 9, "bold"), bg="#F5F5F5", fg="#27ae60"
        )
    ...
    def _on_feedback_comment(self, message_id: str) -> None:
        ...
        try:
            ...
            comment = simpledialog.askstring(
                "Feedback-Kommentar", "Möchten Sie einen Kommentar hinzufügen?", parent=self.parent_window
            )
    ...
    def _submit_feedback_to_backend(
        self, message_id: str, rating: int, category: Optional[str] = None, comment: Optional[str] = None
    ) -> None:
        """
            ...
            def submit_async():
                try:
                    response = self.feedback_api.submit_feedback(
                        message_id=message_id, rating=rating, category=category, comment=comment, user_id="anonymous"
                    )

                    if response.get("success"):
                        logger.info(
                            f"✅ Feedback erfolgreich an Backend gesendet: {message_id} (ID: {response.get('feedback_id')})"
                        )
                    else:
    ...
    def _handle_toolbar_feedback(self, message_id: str, feedback_type: str) -> None:
        ...
        try:
            ...
            self._feedback_states[message_id] = {"type": feedback_type, "submitted": True, "timestamp": time.time()}
    ...
    def _handle_toolbar_raw(self, raw_response: str) -> None:
        ...
        try:
            ...
            if hasattr(self, "dialog_manager") and self.dialog_manager:
                self.dialog_manager.show_info(title="Raw Response", message=raw_response)
    ...
    def _insert_collapsible_details(self, sections: Dict) -> None:
        ...
        self._setup_collapsible_toggle(details_header_start, details_header_end, details_content_start, details_content_end)
                ...
                for part in parts:
                    ...
                    if re.match(url_pattern, part) or re.match(file_pattern, part):
                        ...
                        if self.source_link_handler:
                            ...
                            self.text_widget.tag_bind(link_tag, "<Button-1>", create_click_handler(i, part))
    ...
    def _add_source_hover_tooltip(self, tag_name: str, source_name: str, metadata: Dict[str, Any]) -> None:
        """
    ...
    def _insert_suggestion_link(self, index: int, suggestion: str) -> None:
        ...
        if hasattr(self, "suggestion_click_callback") and self.suggestion_click_callback:
            ...
            self.text_widget.tag_bind(suggestion_tag, "<Button-1>", create_click_handler(suggestion))
        else:
                    ...
                    if self.source_link_handler:
                        ...
                        def create_click_handler(idx, url):
                            def handler(e):
                                ...
                                # Verzögerung damit Scroll-Animation abläuft
                                self.parent_window.after(
                                    550, lambda: self.source_link_handler.open_source_link(url)  # 500ms Scroll + 50ms Buffer
                                )

                            return handler

                        self.text_widget.tag_bind(link_tag, "<Button-1>", create_click_handler(index, part))
    ...
    def _setup_collapsible_toggle(self, header_start: str, header_end: str, content_start: str, content_end: str) -> None:
        """
    ...
    def _expand_details(
        self, header_start: str, header_end: str, content_start: str, content_end: str, animation_state: Dict
    ) -> None:
...
def setup_chat_tags(text_widget: tk.Text) -> None:
    ...
    """
    # Timestamp
    text_widget.tag_configure("timestamp", font=("Segoe UI", 8), foreground="#999")

    # User
    text_widget.tag_configure("user", font=("Segoe UI", 10), foreground="#2980b9")

    # Assistant
    text_widget.tag_configure("assistant", font=("Segoe UI", 10), foreground="#27ae60")
    text_widget.tag_configure("assistant_main", font=("Segoe UI", 10), foreground="#000")

    # System
    text_widget.tag_configure("system", font=("Segoe UI", 9, "italic"), foreground="#95a5a6")

    # Header
    text_widget.tag_configure("header", font=("Segoe UI", 10, "bold"), foreground="#34495e")

    # Metadata
    text_widget.tag_configure("metadata", font=("Segoe UI", 9), foreground="#7f8c8d")

    # Confidence
    text_widget.tag_configure("confidence_high", font=("Segoe UI", 9, "bold"), foreground="#27ae60")
    text_widget.tag_configure("confidence_med", font=("Segoe UI", 9), foreground="#f39c12")
    text_widget.tag_configure("confidence_low", font=("Segoe UI", 9), foreground="#e74c3c")
    ...
    # Source
    text_widget.tag_configure("source", font=("Segoe UI", 9), foreground="#555")

    # Agent
    text_widget.tag_configure("agent", font=("Segoe UI", 9, "bold"), foreground="#8e44ad")

    # Clickable
    text_widget.tag_configure("clickable", font=("Segoe UI", 10, "bold"), foreground="#3498db")
    text_widget.tag_configure("clickable_link", font=("Segoe UI", 9), foreground="#3498db", underline=True)

    # Collapsed
    text_widget.tag_configure("collapsed", font=("Segoe UI", 9), foreground="#95a5a6")
    ...
    # Message-Separator
    text_widget.tag_configure("message_separator", spacing1=10, spacing3=10)  # Abstand zwischen Messages

    # Attachment-Link (klickbar)
    text_widget.tag_configure("attachment_link", font=("Segoe UI", 8), foreground="#0066CC", underline=True)

    # Metriken-Compact (Inline horizontal)
    text_widget.tag_configure("metrics_compact", font=("Segoe UI", 8), foreground="#666", spacing1=2, spacing3=5)

    # Processing-Placeholder (pulsierend)
    text_widget.tag_configure("processing_placeholder", font=("Segoe UI", 10, "italic"), foreground="#999")

    # === RAW RESPONSE TAGS (Debug-View) ===
    text_widget.tag_configure("raw_header", font=("Segoe UI", 9, "bold"), foreground="#555")

    text_widget.tag_configure("raw_param", font=("Courier New", 8), foreground="#666", lmargin1=20)

    text_widget.tag_configure(
        "raw_content", font=("Courier New", 8), foreground="#333", background="#FAFAFA", lmargin1=20, wrap="word"
    )

    text_widget.tag_configure("raw_separator", font=("Courier New", 8), foreground="#CCC")

    # ✨ v3.19.0: Suggestion-Link Tags (klickbare Follow-up-Fragen)
    text_widget.tag_configure("suggestion_prefix", font=("Segoe UI", 9), foreground="#666")

    text_widget.tag_configure(
        "suggestion_link", font=("Segoe UI", 9, "bold"), foreground="#0066CC", spacing1=2, spacing3=2  # Blau (Link-Farbe)
    )

    text_widget.tag_configure("raw_warning", font=("Segoe UI", 8), foreground="#FF6600", lmargin1=20)

    text_widget.tag_configure("raw_tip", font=("Segoe UI", 8, "italic"), foreground="#0066CC", lmargin1=20)

    # ✨ v3.16.0: Typing Indicator Tag
    text_widget.tag_configure("typing_indicator", font=("Segoe UI", 10), foreground="#9E9E9E", spacing1=5, spacing3=5)
