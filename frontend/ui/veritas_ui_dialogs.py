...
class DialogManager:
    ...
    def load_chat(self) -> Optional[List[Dict]]:
        ...
        filename = filedialog.askopenfilename(
            initialdir=self.chats_dir, filetypes=[("JSON-Dateien", "*.json"), ("Alle Dateien", "*.*")]
        )
    ...
    def show_all_chats_dialog(self) -> None:
        ...
        ttk.Label(header_frame, text=f"📋 {len(recent_chats)} Chat(s) gefunden", font=("Segoe UI", 12, "bold")).pack(
            side=tk.LEFT
        )
        ...
        chat_listbox = tk.Listbox(list_frame, yscrollcommand=scrollbar.set, font=("Segoe UI", 9))
