...
class MarkerCluster:
    ...
    def add_marker(self, lat: float, lon: float, data: Dict):
        ...
        grid_key = (round(lat / self.grid_size) * self.grid_size, round(lon / self.grid_size) * self.grid_size)
    ...
    def get_clusters(self, min_zoom: int = 10) -> List[Dict]:
        ...
        for (lat, lon), markers in self.clusters.items():
            if len(markers) > 1:
                # Cluster
                result.append({"lat": lat, "lon": lon, "count": len(markers), "markers": markers, "is_cluster": True})
            else:
                # Einzelner Marker
                marker = markers[0]
...
class IMMIMapWidget(tk.Frame):
    ...
    def _create_sidebar(self):
        ...
        # Header
        header = ttk.Label(sidebar, text="🗺️ IMMI Karte", font=("Segoe UI", 14, "bold"))
        header.pack(pady=10)
        ...
        bimschg_check = ttk.Checkbutton(
            filter_frame, text="🏭 BImSchG-Anlagen", variable=self.bimschg_var, command=self._on_filter_change
        )
        ...
        wka_check = ttk.Checkbutton(
            filter_frame, text="🌬️ Windkraftanlagen", variable=self.wka_var, command=self._on_filter_change
        )
        ...
        cluster_check = ttk.Checkbutton(
            filter_frame, text="📍 Marker-Clustering", variable=self.cluster_var, command=self._on_clustering_change
        )
        ...
        reload_btn = ttk.Button(action_frame, text="🔄 Neu laden", command=self._reload_markers)
        reload_btn.pack(fill="x", pady=2)

        center_btn = ttk.Button(action_frame, text="📍 Brandenburg zentrieren", command=self._center_brandenburg)
        center_btn.pack(fill="x", pady=2)

        search_btn = ttk.Button(action_frame, text="🔍 Suchen...", command=self._show_search_dialog)
        ...
        for category, color in self.BIMSCHG_COLORS.items():
            item = ttk.Label(legend_frame, text=f"● {category}", foreground=color, font=("Segoe UI", 9))
        ...
        for status, color in self.WKA_COLORS.items():
            if status != "default":
                item = ttk.Label(wka_legend, text=f"● {status}", foreground=color, font=("Segoe UI", 9))
                item.pack(anchor="w", pady=1)
    ...
    def _create_map(self):
        ...
        self.map_widget.add_right_click_menu_command(
            label="🔍 Details anzeigen", command=self._on_map_right_click, pass_coords=True
        )
    ...
    def _create_statusbar(self):
        ...
        self.status_label = ttk.Label(statusbar, text="Bereit | 0 Marker geladen", font=("Segoe UI", 9))
        self.status_label.pack(side="left", padx=5)

        self.coord_label = ttk.Label(statusbar, text="Position: --", font=("Segoe UI", 9))
    ...
    def _apply_clustering(self, markers: List[Dict]) -> List[Dict]:
        ...
        for marker in markers:
            self.clusterer.add_marker(marker["lat"], marker["lon"], marker)
...
"""
        ...
        # Close-Button
        close_btn = ttk.Button(info_window, text="Schließen", command=info_window.destroy)
    ...
    def _show_cluster_info(self, cluster_data: Dict):
        ...
        # Header
        header = ttk.Label(info_window, text=f"📍 {count} Anlagen an diesem Standort", font=("Segoe UI", 12, "bold"))
        header.pack(pady=10)
    ...
    def _show_search_results(self, query: str, results: List[Dict]):
        ...
        ttk.Label(result_window, text=f"🔍 {len(results)} Ergebnisse für '{query}'", font=("Segoe UI", 11, "bold")).pack(
            pady=10
        )
        ...
        ttk.Button(result_window, text="Schließen", command=result_window.destroy).pack(pady=5)
    ...
    def _show_error(self, message: str):
        ...
        error_label = ttk.Label(self, text=f"❌ {message}", font=("Segoe UI", 12), foreground="red", justify="center")
