...
class HybridMetadataWidget(ttk.Frame):
    ...
    def _create_widgets(self):
        ...
        ttk.Label(methods_frame, text="Suchverfahren:", font=("Segoe UI", 9), foreground="#7f8c8d").pack(
    ...
    def _create_method_indicators(self, parent):
        ...
        # Method Icons
        method_icons = {"vector": "🔍", "graph": "🕸️", "relational": "📊", "keyword": "🔤"}
        ...
        for method in methods:
            ...
            # Method Badge
            badge = ttk.Label(
                parent, text=f"{icon} {name}", font=("Segoe UI", 9), foreground="#3498db", background="#ecf0f1", padding=(5, 2)
            )
    ...
    def _create_score_display(self, parent):
        ...
        # Original Score
        ttk.Label(parent, text=f"Original Score: {original:.4f}", font=("Consolas", 9), foreground="#7f8c8d").pack(
            side=tk.LEFT, padx=(0, 10)
        )

        # Arrow
        ttk.Label(parent, text="→", font=("Segoe UI", 10), foreground="#95a5a6").pack(side=tk.LEFT, padx=5)

        # Reranked Score
        ttk.Label(parent, text=f"Reranked: {reranked:.4f}", font=("Consolas", 9, "bold"), foreground="#27ae60").pack(
            side=tk.LEFT, padx=(0, 10)
        )
    ...
    def _create_delta_badge(self, parent, delta: float):
        ...
        # Badge Frame
        badge_frame = tk.Frame(parent, bg=color, relief=tk.RAISED, bd=1)
        badge_frame.pack(side=tk.LEFT, padx=5)

        # Icon + Text
        badge_label = tk.Label(
            badge_frame, text=f"{icon} {text}", font=("Consolas", 9, "bold"), bg=color, fg="white", padx=8, pady=2
        )
        badge_label.pack()
    ...
    def _create_strategy_display(self, parent):
        ...
        # Label
        strategy_label = ttk.Label(parent, text=f"Ranking: {display_name}", font=("Segoe UI", 9), foreground="#8e44ad")
...
class HybridSourcesPanel(ttk.Frame):
    ...
    def _create_panel(self):
        ...
        ttk.Label(
            title_frame, text=f"📚 Quellen ({len(self.sources)})", font=("Segoe UI", 12, "bold"), foreground="#2c3e50"
        ).pack(side=tk.LEFT)
    ...
    def _create_source_card(self, parent, index: int, source: Dict[str, Any]):
        ...
        # Card Frame
        card = ttk.Frame(parent, relief=tk.RAISED, borderwidth=1, style="SourceCard.TFrame")
        ...
        ttk.Label(header, text=f"[{index}]", font=("Segoe UI", 10, "bold"), foreground="#3498db").pack(side=tk.LEFT)
...
def create_hybrid_metadata_display(parent: tk.Widget, sources: List[Dict[str, Any]]) -> HybridSourcesPanel:
    """
...
def apply_hybrid_metadata_styles(root: tk.Tk):
    ...
    # Hybrid Meta Frame
    style.configure("HybridMeta.TFrame", background="#f8f9fa", relief=tk.FLAT)

    # Source Card Frame
    style.configure("SourceCard.TFrame", background="white", relief=tk.RAISED, borderwidth=1)
