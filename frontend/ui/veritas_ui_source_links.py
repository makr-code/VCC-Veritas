...
class SourceLinkHandler:
    ...
    def show_source_preview(self, source_name: str) -> None:
        ...
        try:
            ...
            ttk.Label(header_frame, text="📄 Quellen-Information", font=("Segoe UI", 12, "bold")).pack(anchor=tk.W)

            ttk.Label(header_frame, text=source_name, font=("Segoe UI", 9), foreground="#666").pack(anchor=tk.W)

            # Info-Text
            info_text = scrolledtext.ScrolledText(dialog, wrap=tk.WORD, font=("Segoe UI", 10), bg="#f9f9f9", fg="#333")
            ...
            # Schließen-Button
            ttk.Button(dialog, text="Schließen", command=dialog.destroy).pack(pady=(0, 10))
...
Weitere Informationen könnten über das Backend abgerufen werden."""
    ...
    def create_hover_tooltip(
        self, widget: tk.Widget, source_name: str, preview_text: str = None, metadata: Dict[str, Any] = None
    ) -> "SourceTooltip":
        """
...
class SourceTooltip:
    ...
    def _fetch_source_snippet(self, source_id: str) -> Optional[str]:
        ...
        try:
            ...
            response = requests.post(
                f"{API_BASE_URL}/database/get_snippet", json={"source_id": source_id, "max_length": 300}, timeout=2
            )
    ...
    def show_tooltip(self, event=None) -> None:
        ...
        frame = tk.Frame(tw, background="#2c3e50", borderwidth=2, relief="solid", padx=1, pady=1)
        frame.pack()
...
def create_clickable_source_link(
    text_widget: tk.Text, link_text: str, link_url: str, handler: SourceLinkHandler, tag_prefix: str = "source_link"
) -> str:
    """
    ...
    # Click-Handler
    text_widget.tag_bind(link_tag, "<Button-1>", lambda e: handler.open_source_link(link_url))
