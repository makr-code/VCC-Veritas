...
class ThemisWebSocketClient:
    ...
    async def vector_search(
        self, query: str, top_k: int = 5, collection: str = "documents", threshold: Optional[float] = None
    ):
        """
        ...
        async with websockets.connect(url) as websocket:
            ...
            # Sende Search Query
            search_request = {"action": "search", "query": query, "top_k": top_k, "collection": collection}
    ...
    async def monitor_adapter_status(self, interval: int = 5, duration: Optional[int] = None):
        """
    ...
    async def stream_logs(self, log_level: str = "INFO", duration: Optional[int] = None):
        """
    ...
    async def graph_traverse(
        self, start_vertex: str, edge_collection: str = "edges", direction: str = "outbound", max_depth: int = 3
    ):
        """
...
async def main():
    ...
    print("\n\n### 1. Vector Search ###\n")
    try:
        results = await client.vector_search(query="machine learning best practices", top_k=3, collection="documents")
    ...
    try:
        await client.monitor_adapter_status(interval=3, duration=10)
    except Exception as e:
    ...
    try:
        await client.stream_logs(log_level="INFO", duration=10)
    except Exception as e:
    ...
    print("\n\n### 4. Graph Traversal ###\n")
    try:
        graph_data = await client.graph_traverse(start_vertex="doc123", edge_collection="citations", max_depth=2)
